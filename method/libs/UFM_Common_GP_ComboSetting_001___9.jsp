...
<%
      ...
      SELECT ICD_DEPT_CD,  ICD_DEPT_NM, ICD_SUBD_NM FROM IT_COMP_DEPT 
                  WHERE ICD_COMP_CD = :sLarge AND ICD_MNGT_YR = :sSmall
                    AND ICD_HALF_CD = :sLevel
                    AND ICD_USE_YN = 'Y'
                  ORDER BY ICD_DEPT_CD 

		*/

		sql = " SELECT ICD_DEPT_CD, ICD_DEPT_NM, ICD_SUBD_NM";
		sql += " FROM IT_COMP_DEPT";
		sql += " WHERE ICD_COMP_CD = '" + SLARGE + "'";
		sql += " AND ICD_MNGT_YR = '" + SSMALL + "'";
		sql += " AND ICD_HALF_CD = '" + SLEVEL + "'";
		sql += " AND ICD_USE_YN = 'Y'";
		sql += " ORDER BY ICD_DEPT_CD";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : UFM_Common_GP_ComboSetting_001___9 \n";
			G_INFO += "설명 : 구분값에 따른 콤보박스 정보 로딩___9 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='ICD_DEPT_CD' rs:number='1' rs:writeunknown='true' rs:basetable='IT_COMP_DEPT' rs:basecolumn='ICD_DEPT_CD'>
			<s:datatype dt:type='string' dt:maxLength='4' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='ICD_DEPT_NM' rs:number='2' rs:writeunknown='true' rs:basetable='IT_COMP_DEPT' rs:basecolumn='ICD_DEPT_NM'>
			<s:datatype dt:type='string' dt:maxLength='100' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='ICD_SUBD_NM' rs:number='3' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_COMP_DEPT'
			 rs:basecolumn='ICD_SUBD_NM'>
			<s:datatype dt:type='string' dt:maxLength='100'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='4' rs:rowid='true' rs:basetable='IT_COMP_DEPT' rs:basecolumn='ROWID'
