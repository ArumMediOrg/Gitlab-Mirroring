...
SELECT SUBSTR(:APLY_DT, 1, 4) EXAM_YR, LTRIM(RTRIM(A.IIM_ITEM_CD)) EXAM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) EXAM_NM, 
       SUBSTR(C.IIC_STDTO_KD, 3, 1) || '0000' AS EXAM_KD, 
 ...
 WHERE C.IIC_EXAM_YR = SUBSTR(:APLY_DT, 1, 4)
   AND SUBSTR(C.IIC_STDTO_KD, 3, 1) <> '0'
 ...
 ORDER BY CASE WHEN SUBSTR(C.IIC_STDTO_KD, 3, 1) = '9' THEN '0' ELSE SUBSTR(C.IIC_STDTO_KD, 3, 1) END, A.IIM_ITEM_CD
		*/

		sql = " SELECT SUBSTR('" + APLY_DT + "', 1, 4) EXAM_YR, LTRIM(RTRIM(A.IIM_ITEM_CD)) EXAM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) EXAM_NM, SUBSTR(C.IIC_STDTO_KD, 3, 1) || '0000' AS EXAM_KD, '" + EXAM_DT + "' PRCE_DT, '1' CALC_CD, B.IIP_GBSG_PR STDT_PR, B.IIP_GASN_PR GASN_PR";
		...
		sql += " WHERE C.IIC_EXAM_YR = SUBSTR('" + APLY_DT + "', 1, 4)";
		sql += " AND SUBSTR(C.IIC_STDTO_KD, 3, 1) <> '0'";
		...
		sql += " ORDER BY CASE WHEN SUBSTR(C.IIC_STDTO_KD, 3, 1) = '9' THEN '0' ELSE SUBSTR(C.IIC_STDTO_KD, 3, 1) END, A.IIM_ITEM_CD";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStdt_AcptO_UP_StdtItem_003 \n";
			G_INFO += "설명 : 학교밖 확진검사 검사항목 조회 \n";
			G_INFO += "\n\n";
