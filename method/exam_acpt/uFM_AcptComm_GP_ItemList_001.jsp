...
SELECT LTRIM(RTRIM(A.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) ITEM_NM,
       F_RPAD(A.IIM_GBHC_KD, 11, '0') AS GBHC_KD,
 ...
 :PARAM_TX
		*/
		...
		sql = " SELECT LTRIM(RTRIM(A.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) ITEM_NM, F_RPAD(A.IIM_GBHC_KD, 11, '0') AS GBHC_KD, CASE '" + CALC_CD + "' WHEN '1' THEN B.IIP_JCSG_PR WHEN '2' THEN B.IIP_HESG_PR WHEN '3' THEN B.IIP_GBSG_PR WHEN '4' THEN B.IIP_SBSG_PR WHEN '5' THEN B.IIP_TGSG_PR ELSE B.IIP_JCSG_PR END AS ITEM_PR, B.IIP_GASN_PR HOLI_PR";
		sql += " FROM IT_ITEM A, IT_ITEM_PRICE B";
		sql += " WHERE UPPER(A.IIM_USE_YN) = 'Y' ";
		sql += PARAM_TX;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_001 \n";
			G_INFO += "설명 : 2018 이전 건보 호출 검사항목 조회 \n";
			G_INFO += "\n\n";
