...
WHERE 0=0 
        AND EEA_EXAM_DT >= :DTEdtEXAM_SDT
        AND EEA_EXAM_DT <= :DTEdtEXAM_EDT
        AND EEA_ORDER_YN <> 'C' 

if :cmbPlaceFlg = '1' then
        AND SUBSTR(EEA_PLCE_CD, 1, 2)= :cmbPlace
...
ORDER BY EEA_EXAM_DT, EEA_EXAM_SQ 
		...
		*/
		...
		sql += " WHERE 0=0";
		sql += " AND EEA_EXAM_DT >= '" + DTEDTEXAM_SDT + "'";
		sql += " AND EEA_EXAM_DT <= '" + DTEDTEXAM_EDT + "'";
		sql += " AND EEA_ORDER_YN <> 'C'";

		if(CMBPLACEFLG.equals("1")) {
			sql += " AND SUBSTR(EEA_PLCE_CD, 1, 2)= '" + CMBPLACE + "'";
		}
		sql += " ORDER BY EEA_EXAM_DT, EEA_EXAM_SQ";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UF_SqlSet_001 \n";
			G_INFO += "설명 : 검사결과 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ROWID' rs:number='144' rs:rowid='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
