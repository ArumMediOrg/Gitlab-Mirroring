...
<%
		...
		String MS_AGE = htMethod.get("MS_AGE");

		//
		if(CHOL_KD == null) { CHOL_KD = ""; }
		if(SEX_KD == null) { SEX_KD = ""; }
		if(MS_AGE == null) { MS_AGE = ""; }
...
WHERE CDR_DANR_KD = '1'                                
   ...
   AND (TO_NUMBER(CDR_LOW_AGE) <= :Ms_Age AND 
             TO_NUMBER(CDR_HIGH_AGE) >= :Ms_Age) 
		*/
		...
		sql += " AND (TO_NUMBER(CDR_LOW_AGE) <= '" + MS_AGE + "' AND TO_NUMBER(CDR_HIGH_AGE) >= '" + MS_AGE + "')";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uAutoBrain_uAutoPanBrain_008 \n";
			G_INFO += "설명 : 복합상태 위험도 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " CHOL_KD : " + CHOL_KD + " \n";
			G_INFO += " SEX_KD : " + SEX_KD + " \n";
			G_INFO += " MS_AGE : " + MS_AGE + " \n";
			G_INFO += "\n\n";
