...
<%
		...
		sql += " 	FROM (";
		sql += "		SELECT MAX('1') GB, SUM(CNT) CNT, MAX(AA) AA,MAX(BB) BB";
		sql += " 		FROM (";
		sql += "			SELECT 0 CNT, '1' GB, 0 AA,0 BB";
		sql += " 			FROM DUAL";

		sql += " 			UNION ALL";

		sql += "			SELECT SUM(BB) CNT ,'1' BB";
		sql += "			, CASE WHEN EEA_SEX_CD='1' THEN COUNT(*) END AA";
		sql += "			, CASE WHEN EEA_SEX_CD='2' THEN COUNT(*) END AB";
		sql += " 			FROM (";
		sql += "				SELECT *";
		sql += " 				FROM (";
		sql += "					SELECT 1 BB, EEA_SEX_CD";
		sql += "					, CASE WHEN NVL(EEA_CUST_NO, ' ') = ' ' THEN TO_CHAR(ROW_NUMBER() OVER(PARTITION BY EEA_CUST_NO ORDER BY EEA_CUST_NO)) ELSE EEA_CUST_NO END EEA_CUST_NO";
		...
		sql += "				) FF";
		sql += " 				GROUP BY BB,EEA_SEX_CD ,EEA_CUST_NO";
		sql += "			) DD";
		...
		sql += " 		FROM (";
		sql += "			SELECT 0 CNT, '2' GB, 0 AA, 0 BB";
		sql += " 			FROM DUAL";

		sql += " 			UNION ALL";

		sql += "			SELECT SUM(BB) CNT";
		sql += "			, '1' BB";
		sql += "			, CASE WHEN EEA_SEX_CD='1' THEN COUNT(*) END AA";
		sql += "			, CASE WHEN EEA_SEX_CD='2' THEN COUNT(*) END AB";
		sql += " 			FROM (";
		...
		sql += ") T3";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uHERESULTPRT_UDP_SEL_SUJIN_001 \n";
			G_INFO += "설명 : 건강진단 받은 근로자 정보 로딩 \n";
			G_INFO += "\n\n";
