...
SELECT B.IIM_KNME_NM AS ICI_ITEM_NM,                                                            
       CASE WHEN ICI_ITKD_CD  IS NULL OR ICI_ITKD_CD  = '' THEN ICI_ITKD_CD                   
            ELSE '[' + ICI_ITKD_CD  + ']' + dbo.GetSP_COMMON_cdToNm('HM99G',A.ICI_ITKD_CD)
            END ICI_ITKD_NM, A.*                                                                
  FROM IT_COMP_DEPT_ITEM A                                                                      
INNER JOIN IT_ITEM B ON A.ICI_SBCD_CD = B.IIM_SBCD_CD                                           
                    AND B.IIM_USE_YN <> 'N'                                                   
:sSQL_ADD
		*/
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCOMP_DEPT_TRNS_GridTargetClick_002 \n";
			G_INFO += "설명 : 사업장부서관리 이월 검사항목 정보 조회 \n";
			G_INFO += "\n\n";
