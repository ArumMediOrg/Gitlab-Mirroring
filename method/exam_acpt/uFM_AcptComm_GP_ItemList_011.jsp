...
SELECT LTRIM(RTRIM(A.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) ITEM_NM,
       CASE WHEN SUBSTR(:EXAM_KD, 1, 1) = '9' THEN F_ITEM_CALL('C', :EXAM_YR, A.IIM_ITEM_CD)  
                                                  ELSE F_ITEM_CALL('H', :EXAM_YR, A.IIM_ITEM_CD) END AS GBHC_KD, 
       CASE WHEN :EXAM_KD = '85' THEN CASE WHEN SUBSTR(:CALC_CD, 2, 1) = '0' THEN 0
                                               ELSE CASE WHEN SUBSTR(:CALC_CD, 1, 1) = 'Y' THEN B.IIP_SBSG_PR 
                                               ELSE CASE WHEN SUBSTR(:CALC_CD, 2, 1) = '4' THEN B.IIP_SBSG_PR ELSE B.IIP_TGSG_PR END END 
                                          END                                                
                                     ELSE CASE :CALC_CD WHEN '0' THEN 0 
                                                            WHEN '1' THEN B.IIP_JCSG_PR 
 ...
 WHERE A.IIM_ITEM_CD IN (:ITEM_CD) 
 ...
 ORDER BY A.IIM_EXGN_CD, A.IIM_SORT_SQ, A.IIM_ITEM_CD 
		*/
		...
		sql = " SELECT LTRIM(RTRIM(A.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) ITEM_NM";
		sql += ", CASE WHEN SUBSTR('" + EXAM_KD + "', 1, 1) = '9' THEN F_ITEM_CALL('C', '" + EXAM_YR + "', A.IIM_ITEM_CD) ELSE F_ITEM_CALL('H', '" + EXAM_YR + "', A.IIM_ITEM_CD) END AS GBHC_KD";
		sql += ", CASE WHEN '" + EXAM_KD + "' = '85' THEN";
		sql += "	CASE WHEN SUBSTR('" + CALC_CD + "', 2, 1) = '0' THEN 0";
		sql += "	ELSE";
		sql += " 		CASE WHEN SUBSTR('" + CALC_CD + "', 1, 1) = 'Y' THEN";
		sql += "			B.IIP_SBSG_PR";
		sql += "		ELSE";
		sql += "			CASE WHEN SUBSTR('" + CALC_CD + "', 2, 1) = '4' THEN B.IIP_SBSG_PR ELSE B.IIP_TGSG_PR END";
		sql += " 		END";
		sql += "	END";
		sql += "  ELSE";
		sql += " 	CASE '" + CALC_CD + "' WHEN '0' THEN 0";
		sql += " 	WHEN '1' THEN B.IIP_JCSG_PR";
		sql += "	WHEN '2' THEN B.IIP_HESG_PR";
		sql += "	WHEN '3' THEN B.IIP_GBSG_PR";
		sql += "	WHEN '4' THEN B.IIP_SBSG_PR";
		sql += "	WHEN '5' THEN B.IIP_TGSG_PR";
		sql += "	ELSE B.IIP_JCSG_PR";
		sql += "  END";
		sql += " END AS ITEM_PR";
		sql += ", B.IIP_GASN_PR HOLI_PR";
		sql += " FROM IT_ITEM A, IT_ITEM_PRICE B";
		sql += " WHERE A.IIM_ITEM_CD IN ('" + ITEM_CD + "')";
		...
		sql += " ORDER BY A.IIM_EXGN_CD, A.IIM_SORT_SQ, A.IIM_ITEM_CD";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_011 \n";
			G_INFO += "설명 : 개별항목, 종속항목, 표적(혈액,개별)추가항목, 암검사항목 수가 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='GBHC_KD' rs:number='3' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM_PRICE'
			 rs:basecolumn='IIP_GASN_PR'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='ITEM_PR' rs:number='4' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM'
			 rs:basecolumn='ROWID'>
			<s:datatype dt:type='number' rs:dbtype='varnumeric' dt:maxLength='38' rs:scale='255' rs:precision='38'/>
		</s:AttributeType>
		<s:AttributeType name='HOLI_PR' rs:number='5' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM_PRICE'
			 rs:basecolumn='ROWID'>
