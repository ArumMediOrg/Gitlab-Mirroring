...
from ct_common 
where ccn_level = '0' 
      and (ccn_full_nm like '%'||:full_nm||'%' 
      or  ccn_large like '%'||:large||'%') 
 order by ccn_large 
		*/

		sql = " SELECT *";
		sql += " FROM CT_COMMON";
		sql += " WHERE CCN_LEVEL = '0'";
		sql += " AND (CCN_FULL_NM LIKE '%" + FULL_NM + "%' OR CCN_LARGE LIKE '%" + LARGE + "%')";
		sql += " ORDER BY CCN_LARGE";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCode_Mngt_btnSearchLClick_001 \n";
			G_INFO += "설명 : 코드정보 로딩 \n";
			G_INFO += "\n\n";
