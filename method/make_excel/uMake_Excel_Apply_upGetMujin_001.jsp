...
if :edtSearch = '' then
begin
  WHERE IIM_ITEM_CD LIKE 'M%'
end else if :edtSearch <> '' then
begin
  WHERE (IIM_KNME_NM LIKE '%' || :edtSearch || '%'
        OR  IIM_ENME_NM LIKE '%' || :edtSearch || '%')
        And IIM_ITEM_CD LIKE 'M%'
end

        And IIM_USE_YN = 'Y'             
        And IIM_KNME_NM Not Like '%영유%'
        And IIM_KNME_NM Not Like '%선원%'
        And IIM_KNME_NM Not Like '%선박%'
		*/

		sql = " SELECT IIM_ITEM_CD, IIM_KNME_NM";
		sql += " FROM IT_ITEM";
 		if(EDTSEARCH.equals("")) {
 			sql += " WHERE IIM_ITEM_CD LIKE 'M%'";
		}
		else if(! EDTSEARCH.equals("")) {
			sql += " WHERE (IIM_KNME_NM LIKE '%" + EDTSEARCH + "%'";
			sql += " OR IIM_ENME_NM LIKE '%" + EDTSEARCH + "%')";
			sql += " AND IIM_ITEM_CD LIKE 'M%'";
		}
		sql += " AND IIM_USE_YN = 'Y'";
		sql += " AND IIM_KNME_NM NOT LIKE '%영유%'";
		sql += " AND IIM_KNME_NM NOT LIKE '%선원%'";
		sql += " AND IIM_KNME_NM NOT LIKE '%선박%'";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uMake_Excel_Apply_upGetMujin_001 \n";
			G_INFO += "설명 : 문진 코드정보 로딩 \n";
			G_INFO += "\n\n";
