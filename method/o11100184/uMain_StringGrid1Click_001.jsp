...
SELECT distinct                                                            
 A.ERI_EXAM_DT, A.ERI_EXAM_SQ, A.ERI_OCS_CD, A.ERI_OCSC_SNO                
FROM ET_RSLT_ITEM A                                                        
WHERE A.ERI_EXAM_DT = :EXAM_DT
  AND A.ERI_EXAM_SQ = :EXAM_SQ
  AND NVL(A.ERI_OCS_CD, ' ') <> ' '                                 
  AND NVL(A.ERI_CNCL_YN, 'N') <> 'Y'
		*/

		sql = " SELECT DISTINCT A.ERI_EXAM_DT, A.ERI_EXAM_SQ, A.ERI_OCS_CD, A.ERI_OCSC_SNO";
		sql += " FROM ET_RSLT_ITEM A";
		sql += " WHERE A.ERI_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND A.ERI_EXAM_SQ = '" + EXAM_SQ + "'";
		sql += " AND NVL(A.ERI_OCS_CD, ' ') <> ' '";
		sql += " AND NVL(A.ERI_CNCL_YN, 'N') <> 'Y'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uMain_StringGrid1Click_001 \n";
			G_INFO += "설명 : 결과정보 로딩 \n";
			G_INFO += "\n\n";
