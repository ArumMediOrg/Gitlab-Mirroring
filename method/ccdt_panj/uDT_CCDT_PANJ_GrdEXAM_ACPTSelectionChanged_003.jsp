...
FROM ET_RSLT_ITEM A, 
      (SELECT * 
        ...
        WHERE ROWNUM = 1) B 
WHERE A.ERI_EXAM_DT = B.EEA_EXAM_DT 
   ...
   AND A.ERI_CNCL_YN <> 'Y' 
		...
		*/
		...
		sql += " 	WHERE ROWNUM = 1";
		sql += ") B";
		sql += " WHERE A.ERI_EXAM_DT = B.EEA_EXAM_DT";
		sql += " AND A.ERI_EXAM_SQ = B.EEA_EXAM_SQ";
		sql += " AND A.ERI_CNCL_YN <> 'Y'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDT_CCDT_PANJ_GrdEXAM_ACPTSelectionChanged_003 \n";
			G_INFO += "설명 : 특정검사결과정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='EEA_EXAM_DT' rs:number='70'>
			<s:datatype dt:type='string' dt:maxLength='10' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='EEA_EXAM_SQ' rs:number='71'>
			<s:datatype dt:type='string' dt:maxLength='6' rs:maybenull='false'/>
		</s:AttributeType>
