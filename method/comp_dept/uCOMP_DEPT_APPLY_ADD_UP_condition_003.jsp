...
select '[2]' || GetSP_COMMON_cdToNm('HM99G','2') VIP_ITKD_NM, *
from VIEW_IT_ITEM_PRICE WHERE NVL(VIP_SBCD_CD, ' ') <> ' ' AND VIP_APLY_DT <= :APLY_DT

if :CmbBoxKind_KD <> '00' then
    AND VIP_EXGN_CD = :CmbBoxKind_KD

if :EdtKind_NM <> '' then
    AND VIP_KNME_NM LIKE :EdtKind_NM||'%'

ORDER BY VIP_SBCD_CD
		*/

		sql = " SELECT '[2]' || GETSP_COMMON_CDTONM('HM99G','2') VIP_ITKD_NM, VIEW_IT_ITEM_PRICE.*";
		sql += " FROM VIEW_IT_ITEM_PRICE";
		sql += " WHERE NVL(VIP_SBCD_CD, ' ') <> ' '";
		sql += " AND VIP_APLY_DT <= '" + APLY_DT + "'";

		if(! CMBBOXKIND_KD.equals("00")) {
			sql += " AND VIP_EXGN_CD = '" + CMBBOXKIND_KD + "'";
		}
		if(! EDTKIND_NM.equals("")) {
			sql += " AND VIP_KNME_NM LIKE '" + EDTKIND_NM + "%'";
		}

		sql += " ORDER BY VIP_SBCD_CD";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCOMP_DEPT_APPLY_ADD_UP_condition_003 \n";
			G_INFO += "설명 : 검사항목조회(검사분류별) \n";
			G_INFO += "\n\n";
