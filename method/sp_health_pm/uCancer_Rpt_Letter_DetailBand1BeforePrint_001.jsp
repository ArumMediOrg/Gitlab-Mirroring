...
SELECT A.*, B.*, C.*, D.*, E.*, F.*, G.*, M.*
     , H.IAU_PENL_NM AS STMC_DT,  I.IAU_PENL_NM AS COLO_DT
     , J.IAU_PENL_NM AS LIVER_DT, K.IAU_PENL_NM AS BRST_DT
     , L.IAU_PENL_NM AS CRVC_DT,  N.IAU_PENL_NM AS LUNG_DT
  FROM ET_EXAM_ACPT A
 INNER JOIN IT_CUSTOMER B ON
  ...
  LEFT OUTER JOIN IT_AUTH_USER H ON
       H.IAU_LICEN1_NO = ECS_LICEN_CD
  LEFT OUTER JOIN IT_AUTH_USER I ON
       I.IAU_LICEN1_NO = ECC_LICEN_CD
  LEFT OUTER JOIN IT_AUTH_USER J ON
       J.IAU_LICEN1_NO = ECL_LICEN_CD
  LEFT OUTER JOIN IT_AUTH_USER K ON
       K.IAU_LICEN1_NO = ECB_LICEN_CD
  LEFT OUTER JOIN IT_AUTH_USER L ON
       L.IAU_LICEN1_NO = ECV_LICEN_CD
  LEFT OUTER JOIN IT_AUTH_USER N ON
       N.IAU_LICEN1_NO = ECG_LICEN_CD
   ...
   AND (EEA_STOMA_CD <> '0' OR EEA_COLON_CD <> '0' OR EEA_LIVER_CD <> '0'
    OR  EEA_BAST_CD  <> '0' OR EEA_UTER_CD  <> '0' OR NVL(EEA_LUNG_CD, '0') <> '0')
		*/
		...
		sql += " AND ECG_EXAM_SQ = EEA_EXAM_SQ LEFT OUTER JOIN IT_AUTH_USER H";
		sql += " ON H.IAU_LICEN1_NO = ECS_LICEN_CD LEFT OUTER JOIN IT_AUTH_USER I";
		sql += " ON I.IAU_LICEN1_NO = ECC_LICEN_CD LEFT OUTER JOIN IT_AUTH_USER J";
		sql += " ON J.IAU_LICEN1_NO = ECL_LICEN_CD LEFT OUTER JOIN IT_AUTH_USER K";
		sql += " ON K.IAU_LICEN1_NO = ECB_LICEN_CD LEFT OUTER JOIN IT_AUTH_USER L";
		sql += " ON L.IAU_LICEN1_NO = ECV_LICEN_CD LEFT OUTER JOIN IT_AUTH_USER N";
		...
		sql += " OR NVL(EEA_LUNG_CD, '0') <> '0')";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCancer_Rpt_Letter_DetailBand1BeforePrint_001 \n";
			G_INFO += "설명 : 검사결과정보 로딩 \n";
			G_INFO += "\n\n";
