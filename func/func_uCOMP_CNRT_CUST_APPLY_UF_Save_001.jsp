...
<%!
	public String func_uCOMP_CNRT_CUST_APPLY_UF_Save_001(Statement argStmtExec, HashMap argHtMethod) throws Exception {
		...
		try {
			...
			String NUSE_DTT = (String) argHtMethod.get("NUSE_DTT");
			...
			if(SMODE.equals("INSERT")) {
				...
				sql += " ECT_1GOJU_CD, ECT_1FROM_DT, ECT_1TO_DT, ECT_2GOJU_CD, ECT_2FROM_DT, ECT_2TO_DT, ECT_MEMO_TX, ECT_RSVT_DT, ECT_QSTN_NO, ";
				sql += " ECT_PID_EN, ECT_NUSE_ID, ECT_NUSE_DTT, ECT_INPT_ID, ECT_INPT_DTT, ECT_HLTH_KD, ECT_HTSB_YN, ECT_CANCER_YN, ECT_ORAL_YN, ";
				...
				sql += ", '" + NUSE_ID + "' ";
				sql += ", TO_DATE('" + NUSE_DTT + "', 'YYYY-MM-DD')";
				sql += ", '" + USER_ID + "'";
				sql += ", SYSDATE ";
				sql += ", '" + HLTH_KD + "'";
				sql += ", '" + HTSB_YN + "'";
				...
				sql += "  ECT_CUST_NM = '" + CUST_NM + "'";
				sql += ", ECT_SEX_CD = '" + SEX_CD + "'";
				sql += ", ECT_BITH_DT = '" + BITH_DT + "'";
				sql += ", ECT_PENL_ID = '" + PENL_ID + "'";
				sql += ", ECT_MOBL_NO = '" + MOBL_NO + "'";
				sql += ", ECT_TEL_NO = '" + TEL_NO + "'";
				sql += ", ECT_EMAIL_AR = '" + EMAIL_AR + "'";
				sql += ", ECT_ZIP_CD = '" + ZIP_CD + "'";
				sql += ", ECT_ZIP_SQ = '" + ZIP_SQ + "'";
				sql += ", ECT_ZIP_AR = '" + ZIP_AR + "'";
				sql += ", ECT_ROAD_AR = '" + ROAD_AR + "'";
				sql += ", ECT_NHIS_YN = '" + NHIS_YN + "'";
				sql += ", ECT_NHIC_TX = '" + NHIC_TX + "'";
				sql += ", ECT_DEPT_NM = '" + DEPT_NM + "'";
				sql += ", ECT_HALF_CD = '" + HALF_CD + "'";
				sql += ", ECT_DEPT_CD = '" + DEPT_CD + "'";
				sql += ", ECT_ADD_YN = '" + ADD_YN + "'";
				sql += ", ECT_BLDG_NO = '" + BLDG_NO + "'";
				sql += ", ECT_EMPL_CD = '" + EMPL_CD + "'";
				sql += ", ECT_PROF_SQ = '" + PROF_SQ + "'";
				sql += ", ECT_COTK_YN = '" + COTK_YN + "'";
				sql += ", ECT_USE_YN = '" + USE_YN + "'";
				sql += ", ECT_ENTRY_DT = '" + ENTRY_DT + "'";
				sql += ", ECT_JUIP_DT = '" + JUIP_DT + "'";
				sql += ", ECT_NCHR_HH = '" + NCHR_HH + "'";
				sql += ", ECT_DIVI_CD = '" + DIVI_CD + "'";
				sql += ", ECT_WORK_LT = '" + WORK_LT + "'";
				sql += ", ECT_GOJU_CD = '" + GOJU_CD + "'";
				sql += ", ECT_JIJO_CD = '" + JIJO_CD + "'";
				sql += ", ECT_1GOJU_CD = '" + GOJU1_CD + "'";
				sql += ", ECT_1FROM_DT = '" + FROM1_DT + "'";
				sql += ", ECT_1TO_DT = '" + TO1_DT + "'";
				sql += ", ECT_2GOJU_CD = '" + GOJU2_CD + "'";
				sql += ", ECT_2FROM_DT = '" + FROM2_DT + "'";
				sql += ", ECT_2TO_DT = '" + TO2_DT + "'";
				sql += ", ECT_MEMO_TX = '" + MEMO_TX + "'";
				sql += ", ECT_RSVT_DT = '" + RSVT_DT + "'";

				if(G_ENCRT_KD.equals("2")) {
						sql += ", ECT_PID_EN = ECHELONDB.ECL_ENCRYPT('" + PID_EN + "')";
				}
				else {
						sql += ", ECT_PID_EN = '" + PID_EN + "'";
				}

				sql += ", ECT_NUSE_ID = '" + NUSE_ID + "'";
				sql += ", ECT_NUSE_DTT = SYSDATE";
				sql += ", ECT_MODI_ID = '" + USER_ID + "'";
				sql += ", ECT_MODI_DTT = SYSDATE";
				sql += ", ECT_HLTH_KD = '" + HLTH_KD + "'";
				sql += ", ECT_HTSB_YN =  '" + HTSB_YN + "'";
				sql += ", ECT_CANCER_YN = '" + CANCER_YN + "'";
				sql += ", ECT_ORAL_YN = '" + ORAL_YN + "'";
				sql += ", ECT_TOTAL_YN = '" + TOTAL_YN + "'";
				sql += ", ECT_SMS_YN = '" + SMS_YN + "'";
