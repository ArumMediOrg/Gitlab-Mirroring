...
SELECT A.ERI_ITEM_CD, B.IIM_KNME_NM
FROM ET_RSLT_ITEM A INNER JOIN IT_ITEM B ON A.ERI_ITEM_CD = B.IIM_ITEM_CD AND (B.IIM_EXGN_CD = 'M' OR ERI_ITEM_CD = 'S0250')
WHERE ERI_EXAM_DT = :EXAM_DT
  AND ERI_EXAM_SQ = :EXAM_SQ
		*/

		sql = " SELECT A.ERI_ITEM_CD, B.IIM_KNME_NM";
		sql += " FROM ET_RSLT_ITEM A INNER JOIN IT_ITEM B";
		sql += " ON A.ERI_ITEM_CD = B.IIM_ITEM_CD";
		sql += " AND (B.IIM_EXGN_CD = 'M' OR ERI_ITEM_CD = 'S0250')";
		sql += " WHERE ERI_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND ERI_EXAM_SQ = '" + EXAM_SQ + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uQT_QUESTION_PAST_GridExam_ListSel_001 \n";
			G_INFO += "설명 : 과거문진항목조회 \n";
			G_INFO += "\n\n";
