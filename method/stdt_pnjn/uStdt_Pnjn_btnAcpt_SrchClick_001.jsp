<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ page import="java.util.HashMap"%>
<%@ include file="/inc_prg/common.jsp"%>
<%
	// 전달자료
	HashMap<String, Object> htData = null;
	HashMap<String, String> htMethod = null;

	// DB객체
	Statement stmtList = null;

	ResultSet rsList = null;

	CRs cRsList = null;

	//
	String G_INFO = "";

	try {
%>
<%@ include file="/inc_prg/connect.jsp"%>
<%
		// 전달자료
		htData = (HashMap<String, Object>)request.getAttribute("htData");
		htMethod = (HashMap<String, String>)request.getAttribute("htMethod_1");

		String EXAM_SDT = htMethod.get("EXAM_SDT");
		String EXAM_EDT = htMethod.get("EXAM_EDT");
		String COMP_CD = htMethod.get("COMP_CD");
		String DIVI_CD = htMethod.get("DIVI_CD");
		String DIVI_CS = htMethod.get("DIVI_CS");
		String CLSS_KD = htMethod.get("CLSS_KD");
		String PSNL_NM = htMethod.get("PSNL_NM");
		String DEPT_NM = htMethod.get("DEPT_NM");
		String PARAM_TX = htMethod.get("PARAM_TX");
		String ORDER_BY = htMethod.get("ORDER_BY");

		//
		if(EXAM_SDT == null) { EXAM_SDT = ""; }
		if(EXAM_EDT == null) { EXAM_EDT = ""; }
		if(COMP_CD == null) { COMP_CD = ""; }
		if(DIVI_CD == null) { DIVI_CD = ""; }
		if(DIVI_CS == null) { DIVI_CS = ""; }
		if(CLSS_KD == null) { CLSS_KD = ""; }
		if(PSNL_NM == null) { PSNL_NM = ""; }
		if(DEPT_NM == null) { DEPT_NM = ""; }
		if(PARAM_TX == null) { PARAM_TX = ""; }
		if(ORDER_BY == null) { ORDER_BY = ""; }

		// DB객체
		stmtList = connect.createStatement();

		sql = " SELECT A.EEA_EXAM_DT, A.EEA_EXAM_SQ, A.EEA_ORAL_YN, A.EEA_FOOD_YN, A.EEA_CHART_NO, A.EEA_ACPT_TX, A.EEA_RFID_NO ";
		sql += ", CASE WHEN A.EEA_ORAL_YN = '3' THEN '-' ELSE C.ESP_PNJN_TT END AS ESP_TOTAL_PJ";
		sql += ", CASE WHEN A.EEA_ORAL_YN = '2' THEN '-' ELSE C.ESP_PNJN_05 END AS ESP_ORAL_PJ";
		sql += ", NVL(C.ESP_EPNJN_YN, 'N') ESP_EPNJN_YN, NVL(C.ESP_OPNJN_YN, 'N') ESP_OPNJN_YN, A.EEA_PSNL_NM, B.ICR_BIRH_DT, A.EEA_CUST_NO, A.EEA_DIVI_CS, A.EEA_SEX_CD, A.EEA_DEPT_NM";

		sql += ", CASE WHEN A.EEA_RSLT_CD = '0' THEN '대기' ELSE";
		sql += "	CASE WHEN A.EEA_RSLT_CD = '1' THEN '입력중' ELSE '완료' END";
		sql += " END AS EEA_RSLT_NM";

		sql += ", CASE WHEN A.EEA_DIVI_CD = '1' THEN '초-' WHEN A.EEA_DIVI_CD = '2' THEN '중-' ELSE '고-' END || A.EEA_DIVI_CS || '/' || A.EEA_CLSS_KD || '/' || A.EEA_DIVI_NO AS EEA_STDT_KD";

		sql += ", NVL(";
		sql += "	(";
		sql += "		SELECT EPM_MEMO_SQ";
		sql += " 		FROM ET_PSNL_MEMO";
		sql += " 		WHERE EPM_CUST_NO = A.EEA_CUST_NO";
		sql += " 		AND ROWNUM = 1";
		sql += "	), '0') EPM_MEMO_SQ";

		sql += ", F_PID(B.ICR_PENL_ID) EEA_PSNL_ID, F_TEL_FORMAT(B.ICR_MOBL_NO) EEA_MOBL_NO";
		sql += ", F_COMP_FIND(A.EEA_COMP_CD) || CASE WHEN LTRIM(A.EEA_DEPT_NM) <> ' ' THEN '/' || LTRIM(A.EEA_DEPT_NM) ELSE '' END AS EEA_COMP_NM";
		sql += " FROM IT_CUSTOMER B, ET_EXAM_ACPT A LEFT OUTER JOIN ET_STDT_PNJN C";
		sql += " ON C.ESP_EXAM_DT = A.EEA_EXAM_DT";
		sql += " AND C.ESP_EXAM_SQ = A.EEA_EXAM_SQ";
		sql += " WHERE A.EEA_EXAM_DT BETWEEN '" + EXAM_SDT + "'";
		sql += " AND '" + EXAM_EDT + "'";
		sql += " AND A.EEA_EXAM_CD = '31001'";
		sql += " AND A.EEA_ORDER_YN <> 'C'";
		sql += " AND B.ICR_CUST_NO = A.EEA_CUST_NO";

		if(! COMP_CD.equals("")) {
			sql += " AND A.EEA_COMP_CD = '" + COMP_CD + "'";
		}

		if(! DIVI_CD.equals("0")) {
			sql += " AND A.EEA_DIVI_CD = '" + DIVI_CD + "'";
		}

		if(! DIVI_CS.equals("")) {
			sql += " AND A.EEA_DIVI_CS = '" + DIVI_CS + "'";
		}

		if(! CLSS_KD.equals("")) {
			sql += " AND A.EEA_CLSS_KD = '" + CLSS_KD + "'";
		}

		if(! PSNL_NM.equals("")) {
			sql += " AND A.EEA_PSNL_NM LIKE '%" + PSNL_NM + "%'";
		}

		if(! DEPT_NM.equals("")) {
			sql += " AND A.EEA_DEPT_NM LIKE '%" + DEPT_NM + "%'";
		}

		sql += PARAM_TX;
		sql += ORDER_BY;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStdt_Pnjn_btnAcpt_SrchClick_001 \n";
			G_INFO += "설명 : 학생 검진 접수 조회(결과/판정) \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " EXAM_SDT : " + EXAM_SDT + " \n";
			G_INFO += " EXAM_EDT : " + EXAM_EDT + " \n";
			G_INFO += " COMP_CD : " + COMP_CD + " \n";
			G_INFO += " DIVI_CD : " + DIVI_CD + " \n";
			G_INFO += " DIVI_CS : " + DIVI_CS + " \n";
			G_INFO += " CLSS_KD : " + CLSS_KD + " \n";
			G_INFO += " PSNL_NM : " + PSNL_NM + " \n";
			G_INFO += " DEPT_NM : " + DEPT_NM + " \n";
			G_INFO += " PARAM_TX : " + PARAM_TX + " \n";
			G_INFO += " ORDER_BY : " + ORDER_BY + " \n";
			G_INFO += "\n\n";

			G_INFO += "질의문 : " + sql + " \n";
			G_INFO += "-->";

		rsList = stmtList.executeQuery(sql);
		cRsList = new CRs(rsList);

		out.clear();		// include된 파일안의 공백 제거
		response.addHeader("Content-type", "text/xml");
%><?xml version="1.0" encoding="UTF-8"?>

<%= G_INFO%>

<nurionXml>
	<resultCode>200</resultCode>
	<resultXml>
		<xml xmlns:s='uuid:BDC6E3F0-6DA3-11d1-A2A3-00AA00C14882'
			xmlns:dt='uuid:C2F41010-65B3-11d1-A29F-00AA00C14882'
			xmlns:rs='urn:schemas-microsoft-com:rowset'
			xmlns:z='#RowsetSchema'>

<s:Schema id='RowsetSchema'>
<%
		for (colCnt = 1; colCnt <= rsmd.getColumnCount(); colCnt++){
			String dataType = "string";
			String maxLength = "4000";

			if (rsmd.getColumnTypeName(colCnt).equals("BLOB")){
				dataType = "bin.hex";
				maxLength = "2147483647";
				//<s:datatype dt:type='bin.hex' dt:maxLength='2147483647' rs:long='true'/>
			} else if (rsmd.getColumnTypeName(colCnt).equals("CLOB")){
				maxLength = "1073741823";
				//<s:datatype dt:type='string' dt:maxLength='1073741823' rs:long='true'/>
			}
%>
		<s:AttributeType name='<%= rsmd.getColumnName(colCnt)%>' rs:number='<%= Integer.toString(colCnt)%>' rs:writeunknown='true' rs:basetable='DUAL' rs:basecolumn='<%= rsmd.getColumnName(colCnt)%>'>
			<s:datatype dt:type='<%= dataType%>' dt:maxLength='<%= maxLength%>' <% if (! maxLength.equals("4000")) { %> rs:long='true' <% } %>/>
		</s:AttributeType>
<%
		}
%>
		<s:AttributeType name='ROWID' rs:number='<%= Integer.toString(colCnt)%>' rs:rowid='true' rs:writeunknown='true' rs:basetable='DUAL'
			 rs:basecolumn='ROWID' rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:extends type='rs:rowbase'/>
	</s:ElementType>
</s:Schema>		<rs:data>
<%
		int cnt = 0;
		while(cRsList.next()) {

			cnt++;

			String ROWID_T = cRsList.getString("ROWID");
%>
			<z:row
<%
			for (colCnt = 1; colCnt <= rsmd.getColumnCount(); colCnt++){
				String tempData = cRsList.getString(rsmd.getColumnName(colCnt));
				if(! tempData.equals("")) {
%>
		 			<%= rsmd.getColumnName(colCnt)%>='<%= tempData%>'
<%
				}
			}
%>
				ROWID='<%= cnt%>'
			/>
<%
		}
%>
		</rs:data>
		</xml>
	</resultXml>
	<errorMsg></errorMsg>
</nurionXml>

<%
	} catch (Exception e) {

		out.clear();		// include된 파일안의 공백 제거
		response.addHeader("Content-type", "text/xml");
%><?xml version="1.0" encoding="UTF-8"?>

<%= G_INFO%>

<nurionXml>
	<resultCode>400</resultCode>
	<resultXml></resultXml>
	<errorMsg><![CDATA[<%= e.toString()%>]]></errorMsg>
	<sql><![CDATA[<%= sql%>]]></sql>
</nurionXml>

<%
	} finally {

		if(rsList != null) {
			rsList.close();
			rsList = null;
		}

		if(stmtList != null) {
			stmtList.close();
			stmtList = null;
		}
%>
<%@ include file="/inc_prg/disconnect.jsp"%>
<%
	}
%>
