...
<%
		...
		sql += " 							WHERE emi_mark_kd = epm_mark_kd AND emi_mark_cd = epm_mark_cd AND emi_mark_sq = epm_mark_sq)) EXAM_RSLT_CNT1 ";
		sql += " 	,(SELECT CASE WHEN count(*) > 0 THEN 'V' ELSE '' END FROM et_rslt_item ";
		sql += " 		WHERE eri_exam_dt = substr(eea.EEA_EXAM_NO2,1,10) AND eri_exam_sq = substr(eea.EEA_EXAM_NO2,12,6) ";
		sql += " 		AND eri_item_cd IN (SELECT emi_item_cd FROM ET_PACK_MARK_item ";
		sql += " 							WHERE emi_mark_kd = epm_mark_kd AND emi_mark_cd = epm_mark_cd AND emi_mark_sq = epm_mark_sq)) EXAM_RSLT_CNT2 ";
		sql += " 	,(SELECT CASE WHEN count(*) > 0 THEN 'V' ELSE '' END FROM et_rslt_item ";
		sql += " 		WHERE eri_exam_dt = substr(eea.EEA_EXAM_NO3,1,10) AND eri_exam_sq = substr(eea.EEA_EXAM_NO3,12,6) ";
		sql += " 		AND eri_item_cd IN (SELECT emi_item_cd FROM ET_PACK_MARK_item ";
		sql += " 							WHERE emi_mark_kd = epm_mark_kd AND emi_mark_cd = epm_mark_cd AND emi_mark_sq = epm_mark_sq)) EXAM_RSLT_CNT3 ";
		sql += " 	,(SELECT CASE WHEN count(*) > 0 THEN 'V' ELSE '' END FROM et_rslt_item ";
		sql += " 		WHERE eri_exam_dt = substr(eea.EEA_EXAM_NO4,1,10) AND eri_exam_sq = substr(eea.EEA_EXAM_NO4,12,6) ";
		sql += " 		AND eri_item_cd IN (SELECT emi_item_cd FROM ET_PACK_MARK_item ";
		sql += " 							WHERE emi_mark_kd = epm_mark_kd AND emi_mark_cd = epm_mark_cd AND emi_mark_sq = epm_mark_sq)) EXAM_RSLT_CNT4 ";
		sql += " 	,(SELECT CASE WHEN count(*) > 0 THEN 'V' ELSE '' END FROM et_rslt_item ";
		sql += " 		WHERE eri_exam_dt = substr(eea.EEA_EXAM_NO5,1,10) AND eri_exam_sq = substr(eea.EEA_EXAM_NO5,12,6) ";
		sql += " 		AND eri_item_cd IN (SELECT emi_item_cd FROM ET_PACK_MARK_item ";
		sql += " 							WHERE emi_mark_kd = epm_mark_kd AND emi_mark_cd = epm_mark_cd AND emi_mark_sq = epm_mark_sq)) EXAM_RSLT_CNT5 ";
		sql += " FROM ( ";
