...
<%
		...
		String EDTIIM_KNME_NM = htMethod.get("EDTIIM_KNME_NM");
		String CHKBOXUSE = htMethod.get("CHKBOXUSE");

		//
		if(EDTIIM_KNME_NM == null) { EDTIIM_KNME_NM = ""; }
...
if :EdtIIM_KNME_NM <> '' then
begin
   AND (IIM_KNME_NM like '%' || :EdtIIM_KNME_NM || '%'
   OR IIM_ENME_NM like '%' || :EdtIIM_KNME_NM || '%'
   OR IIM_CNME_NM like '%' || :EdtIIM_KNME_NM || '%'
   OR IIM_JNME_NM like '%' || :EdtIIM_KNME_NM || '%'
   OR IIM_SNME_NM like '%' || :EdtIIM_KNME_NM || '%)'
end;

if :ChkBoxUSE = 'N'  then
   AND IIM_USE_YN = 'Y'

ORDER BY IIM_ITEM_CD ';
		*/

		sql = " SELECT *";
		sql += " FROM IT_ITEM";
		sql += " WHERE IIM_EXGN_CD = 'M'";

		if(! EDTIIM_KNME_NM.equals("")) {
			sql += " AND (";
			sql += "	IIM_KNME_NM LIKE '%" + EDTIIM_KNME_NM + "%'";
			sql += " 	OR IIM_ENME_NM LIKE '%" + EDTIIM_KNME_NM + "%'";
			sql += " 	OR IIM_CNME_NM LIKE '%" + EDTIIM_KNME_NM + "%'";
			sql += " 	OR IIM_JNME_NM LIKE '%" + EDTIIM_KNME_NM + "%'";
			sql += " 	OR IIM_SNME_NM LIKE '%" + EDTIIM_KNME_NM + "%'";
			sql += ")";
		}
		...
		sql += " ORDER BY IIM_ITEM_CD";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uIT_ITEM_QUESTION_SBtnQueryClick_001 \n";
			G_INFO += "설명 : 검사항목 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " EDTIIM_KNME_NM : " + EDTIIM_KNME_NM + " \n";
