...
SELECT SUBSTR(:EXAM_DT, 1, 4) EXAM_YR, 
       ...
       '000000' || SUBSTR(C.IIC_STDT_KD, 7, 1) EXAM_KD, 
 ...
 WHERE C.IIC_EXAM_YR = SUBSTR(:EXAM_DT, 1, 4)
   AND SUBSTR(NVL(C.IIC_STDT_KD, '0000000'), 7, 1) <> '0'
 ...
 ORDER BY A.IIM_EXGN_CD, A.IIM_SORT_SQ, A.IIM_ITEM_CD
		*/
		...
		sql = " SELECT SUBSTR('" + EXAM_DT + "', 1, 4) EXAM_YR, LTRIM(RTRIM(A.IIM_ITEM_CD)) EXAM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) EXAM_NM, '000000' || SUBSTR(C.IIC_STDT_KD, 7, 1) EXAM_KD, '" + EXAM_DT + "' PRCE_DT, '1' CALC_CD, B.IIP_STSG_PR EXAM_PR";
		sql += " FROM IT_ITEM_CALL C, IT_ITEM A, IT_ITEM_PRICE B";
		sql += " WHERE C.IIC_EXAM_YR = SUBSTR('" + EXAM_DT + "', 1, 4)";
		sql += " AND SUBSTR(NVL(C.IIC_STDT_KD, '0000000'), 7, 1) <> '0'";
		...
		sql += " ORDER BY A.IIM_EXGN_CD, A.IIM_SORT_SQ, A.IIM_ITEM_CD";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStdt_Acpt_UP_StdtItem_002 \n";
			G_INFO += "설명 : 학생 구강검진 검사항목 조회 \n";
			G_INFO += "\n\n";
