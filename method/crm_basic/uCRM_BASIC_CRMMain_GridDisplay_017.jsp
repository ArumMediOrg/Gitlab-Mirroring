...
<%
		...
		sql = " Select DISTINCT A.EEA_EXAM_DT  As EXAM_DT,   A.EEA_EXAM_SQ As EXAM_SQ, ";
		sql += " 	A.EEA_CHART_NO As CHART_NO,  A.EEA_PSNL_NM As PSNL_NM, ";
		sql += " 	B.ICR_PENL_ID  As PENL_ID,   A.EEA_CUST_NO As CUST_NO, ";
		sql += " 	A.EEA_RSVN_NO  As RSVN_NO, ";
		sql += " 	A.EEA_MOBL_NO  AS PHONE_NUM, A.EEA_ACPT_TM AS EXAM_TM ";
		sql += " (SELECT COUNT(*) FROM ST_ADVICE WHERE SAE_EXAM_DT = A.EEA_EXAM_DT";
		sql += " AND SAE_EXAM_SQ = A.EEA_EXAM_SQ AND SAE_USE_YN <> 'N') AS RESE_CNT,";
		sql += " (SELECT COUNT(*) FROM ST_ADVICE WHERE SAE_EXAM_DT = A.EEA_EXAM_DT";
		sql += " AND SAE_EXAM_SQ = A.EEA_EXAM_SQ AND SAE_USE_YN <> 'N' AND SAE_ADVI_STEP = '1') AS COMP_CNT";
		...
		sql += " Left Outer Join PT_FIR_PANJUNG   H On A.EEA_EXAM_DT = H.PFP_EXAM_DT And A.EEA_EXAM_SQ = H.PFP_EXAM_SQ ";
		sql += " Left Outer Join PT_LABOR_PANJUNG J On A.EEA_EXAM_DT = J.PLP_EXAM_DT And A.EEA_EXAM_SQ = J.PLP_EXAM_SQ ";
		...
		sql += SSQL_ADD1;
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCRM_BASIC_CRMMain_GridDisplay_017 \n";
			G_INFO += "설명 : 상담관리-접수조회 \n";
			G_INFO += "\n\n";
