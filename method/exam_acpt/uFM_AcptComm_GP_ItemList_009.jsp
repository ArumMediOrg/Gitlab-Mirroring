...
SELECT LTRIM(RTRIM(B.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(B.IIM_KNME_NM)) ITEM_NM, 
       '00000000000' GBHC_KD, 
       A.ECA_AGEA_PR ITEM_PR, 0 HOLI_PR 
  FROM ET_COMP_AGEA A, IT_ITEM B 
 WHERE A.ECA_COMP_CD = :COMP_CD
   AND A.ECA_MNGT_YR = :MNGT_YR
   AND A.ECA_ITEM_CD IN (:ITEM_CD) 
   AND UPPER(A.ECA_USE_YN) = 'Y'
   AND B.IIM_ITEM_CD = A.ECA_ITEM_CD 
 ORDER BY A.ECA_ITEM_CD
		*/

		sql = " SELECT LTRIM(RTRIM(B.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(B.IIM_KNME_NM)) ITEM_NM, '00000000000' GBHC_KD, A.ECA_AGEA_PR ITEM_PR, 0 HOLI_PR";
		sql += " FROM ET_COMP_AGEA A, IT_ITEM B";
		sql += " WHERE A.ECA_COMP_CD = '" + COMP_CD + "'";
		sql += " AND A.ECA_MNGT_YR = '" + MNGT_YR + "'";
		sql += " AND A.ECA_ITEM_CD IN ('" + ITEM_CD + "')";
		sql += " AND UPPER(A.ECA_USE_YN) = 'Y'";
		sql += " AND B.IIM_ITEM_CD = A.ECA_ITEM_CD";
		sql += " ORDER BY A.ECA_ITEM_CD";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_009 \n";
			G_INFO += "설명 : 사업장 항목별 계약단가 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ITEM_PR' rs:number='4' rs:writeunknown='true' rs:basetable='ET_COMP_AGEA' rs:basecolumn='ECA_AGEA_PR'>
		...
		<s:AttributeType name='ROWID' rs:number='6' rs:rowid='true' rs:basetable='ET_COMP_AGEA' rs:basecolumn='ROWID'
