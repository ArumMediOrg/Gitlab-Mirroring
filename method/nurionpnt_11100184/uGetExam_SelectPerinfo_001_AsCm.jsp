...
<%
		...
		sql = " SELECT A.EEA_EXAM_DT, A.EEA_EXAM_SQ, A.EEA_ACPT_CD, A.EEA_CUST_NO, A.EEA_PSNL_NM, A.EEA_EXAM_CD, A.EEA_CHART_NO, C.ICY_COMP_NM, A.EEA_DEPT_NM, A.EEA_PSNL_AGE,";
		...
		sql += " SUBSTRING(B.ICR_PENL_ID,1,6) ICR_BIRH_DT, A.EEA_COMP_CD, (C.ICY_ZIP_AR||' '||C.ICY_ROAD_AR) ICY_COMP_AR,";
		sql += " CASE WHEN A.EEA_COMP_CD IS NULL OR A.EEA_COMP_CD  = '' THEN ''  else '['||A.EEA_COMP_CD||']'||C.ICY_COMP_NM  END EEA_COMP_NM,";
		sql += " CASE WHEN A.EEA_EXAM_CD IS NULL OR A.EEA_EXAM_CD  = '' THEN A.EEA_EXAM_CD  else '['||A.EEA_EXAM_CD||']'||GetCOMMON_LvCdToNm('0401',A.EEA_EXAM_CD,'5',5) END ICR_EXAM_NM, ERI_RSLT_VL";
		...
		sql += " LEFT OUTER JOIN ET_RSLT_ITEM D ON A.EEA_EXAM_DT = D.ERI_EXAM_DT AND A.EEA_EXAM_SQ = D.ERI_EXAM_SQ AND D.ERI_ITEM_CD = 'Z0116'";
		sql += " WHERE 0=0";
		...
		sql += SSQL_ADD;
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uGetExam_SelectPerinfo_001_AsCm \n";
			G_INFO += "설명 : 조회 버튼(아산충무) \n";
			G_INFO += "\n\n";
