...
Select * from (
   SELECT ICR_PENL_ID, EEA_CUPN_NO, EEA_EXAM_DT EEA_BUSI_YR,
          (replace(EEA_EXAM_DT,'-','') || EEP_EXAM_SQ ) RECENO,
          EEA_CUST_NO,  EEA_CHART_NO,
          EEA_PSNL_NM,
          ICR_BIRH_DT,
          EEA_EXAM_DT,
          EEP_EXAM_SQ,
          EEP_PSNL_PR,
          NVL(EEA_DISC_PR,0) EEA_DISC_PR,
          EEA_EXAM_CD, EEA_RECE_NO ,
         (select a.icy_comp_nm from it_company a where eea_comp_cd = a.icy_comp_cd) icy_comp_nm ,
      NVL((SELECT
     (SELECT  NVL(SUM(RRM_RESER_PR),0) RRM_RESER_PR FROM RT_RECE_MEDICHK
                            WHERE RRM_RECE_NO=RRT_RECE_NO)
        FROM RT_RSVT WHERE  RRT_RSVN_NO=EEA_RSVN_NO ),0) RESER_PR,
      NVL((SELECT
     (SELECT  NVL(SUM(RRM_SALE_PR),0) RRM_SALE_PR FROM RT_RECE_MEDICHK
                            WHERE RRM_RECE_NO=RRT_RECE_NO)
        FROM RT_RSVT WHERE  RRT_RSVN_NO=EEA_RSVN_NO ),0) RRM_SALE_PR,
     (SELECT max(RRT_RECE_NO) FROM RT_RSVT
                            WHERE RRT_RECE_NO=EEA_RSVN_NO) RRT_RECE_NO ,
       NVL(EEP_PSNL_PR - ( select  (NVL(SUM(RRM_SALE_PR),0)   + NVL(EEA_DISC_PR,0) + NVL(SUM(RRM_TRUNC_PR),0) + NVL(SUM(RRM_RECE_PR),0)-NVL(SUM(RRM_OVERPAY_PR),0)) - NVL(SUM(RRM_RETURN_PR ),0),
       NVL(EEA_COMPR_PR,0) EEA_COMPR_PR, NVL(EEA_DISC_CPR,0) EEA_DISC_CPR
from RT_RECE_MEDICHK
            where RRM_RECE_NO=A.EEA_RECE_NO and RRM_OCCU_KD in ('20','70','71') ),0)  RMH_NOPAY_PR,
       NVL(( select NVL(SUM(RRM_RECE_PR),0)   from RT_RECE_MEDICHK
            where RRM_RECE_NO=A.EEA_RECE_NO and RRM_OCCU_KD in ('20','70','71')),0)  RMH_RECE_PR,
      NVL(( select NVL(SUM(RRM_SALE_PR),0)   from RT_RECE_MEDICHK
            where RRM_RECE_NO=A.EEA_RECE_NO and RRM_OCCU_KD in ('20','70','71')),0)  RMH_SALE_PR,
      NVL(( select NVL(sum(RRM_TRUNC_PR),0)   from RT_RECE_MEDICHK
            where RRM_RECE_NO=A.EEA_RECE_NO and RRM_OCCU_KD in ('20','70','71')),0)  RMH_SERVI_PR,
     (SELECT max(CCN_FULL_NM )  CCN_FULL_NM
        FROM CT_COMMON AA,     RT_RECE_MEDICHK BB
       WHERE BB.RRM_PAY_KD=AA.CCN_SMALL
         AND BB.RRM_RECE_NO=EEA_RECE_NO
         AND AA.CCN_LARGE='0920' ) RRM_PAY_NM ,
          (SELECT CCN_FULL_NM
            FROM CT_COMMON
           WHERE CCN_LARGE='0401'
                 AND CCN_SMALL=A.EEA_EXAM_CD
          ) EEA_EXAM_NM,
          CASE
              WHEN EEA_RECE_KD ='2'
              THEN '수납'
              ...
              THEN '환불'
              ELSE '미납'
          END EEA_RECE_KD, EEA_RECE_KD RECE_GBN,
          EEA_MODI_DTT
     FROM ET_EXAM_ACPT A
      LEFT OUTER JOIN
          (SELECT EEP_EXAM_DT,
                 EEP_EXAM_SQ,
                 SUM(EEP_PSNL_PR) EEP_PSNL_PR
            FROM ET_EXAM_PRICE
           GROUP BY EEP_EXAM_DT,
                 EEP_EXAM_SQ
          ) B
          ON A.EEA_EXAM_DT= B.EEP_EXAM_DT
          AND A.EEA_EXAM_SQ=B.EEP_EXAM_SQ
      LEFT OUTER JOIN IT_CUSTOMER D
          ON A.EEA_CUST_NO = D.ICR_CUST_NO
    WHERE 0=0
 ...
 || :sSQL_ADD
		*/
		...
		sql += " FROM RT_RSVT";
		sql += " WHERE RRT_RECE_NO=EEA_RSVN_NO) RRT_RECE_NO , NVL(EEP_PSNL_PR - ( SELECT (NVL(SUM(RRM_SALE_PR),0) + NVL(EEA_DISC_PR,0) + NVL(SUM(RRM_TRUNC_PR),0) + NVL(SUM(RRM_RECE_PR),0)-NVL(SUM(RRM_OVERPAY_PR),0)) - NVL(SUM(RRM_RETURN_PR ),0)";
		sql += " FROM RT_RECE_MEDICHK";
		...
		sql += SSQL_ADD;
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : ubT_Rece_Up_afLoadper_001 \n";
			G_INFO += "설명 : 수납대상내역조회(직불) \n";
			G_INFO += "\n\n";
