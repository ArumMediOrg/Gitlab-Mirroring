...
<%
		...
		String CMBCODE_S1 = htMethod.get("CMBCODE_S1");
		String CMBCODE_S2 = htMethod.get("CMBCODE_S2");
		String CMBPANCD_S1 = htMethod.get("CMBPANCD_S1");
		String CMBPANCD_S2 = htMethod.get("CMBPANCD_S2");
		String EDTFRT_SEARCH_S1 = htMethod.get("EDTFRT_SEARCH_S1");
		String EDTFRT_SEARCH_S2 = htMethod.get("EDTFRT_SEARCH_S2");

		//
		if(LCODE == null) { LCODE = ""; }
		if(CMBCODE_S1 == null) { CMBCODE_S1 = ""; }
		if(CMBCODE_S2 == null) { CMBCODE_S2 = ""; }
		if(CMBPANCD_S1 == null) { CMBPANCD_S1 = ""; }
		if(CMBPANCD_S2 == null) { CMBPANCD_S2 = ""; }
		if(EDTFRT_SEARCH_S1 == null) { EDTFRT_SEARCH_S1 = ""; }
		if(EDTFRT_SEARCH_S2 == null) { EDTFRT_SEARCH_S2 = ""; }
...
if :cmbCode_S1 <> '' then
      AND CCF_CODE = :cmbCode_S2

if :cmbPanCd_S1 <> '' then
      AND CCF_PAN_CD = :cmbPanCd_S2

if :edtFrt_Search_S1 <> '' then
      AND CCF_PANVIEW Like '%' || :edtFrt_Search_S2 || '%' 

ORDER BY CCF_CODENM, CCF_PAN_CD, CCF_PAN_SQ 
		*/

		sql = " SELECT *";
		sql += " FROM CT_COMM_FIRVIEW";
		sql += " WHERE CCF_LCODE = '" + LCODE + "'";
		if(!CMBCODE_S1.equals("")) {
			sql += " AND CCF_CODE = '" + CMBCODE_S2 + "'";
		}
		if(!CMBPANCD_S1.equals("")) {
			sql += " AND CCF_PAN_CD = '" + CMBPANCD_S2 + "'";
		}		
		if(!EDTFRT_SEARCH_S1.equals("")) {
			sql += " AND CCF_PANVIEW LIKE '%" + EDTFRT_SEARCH_S2 + "%'";
		}		
		sql += " ORDER BY CCF_CODENM, CCF_PAN_CD, CCF_PAN_SQ";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uManage_Finding_upGetOpin_001 \n";
			G_INFO += "설명 : 1차판정 정보 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " LCODE : " + LCODE + " \n";
			G_INFO += " CMBCODE_S1 : " + CMBCODE_S1 + " \n";
			G_INFO += " CMBCODE_S2 : " + CMBCODE_S2 + " \n";
			G_INFO += " CMBPANCD_S1 : " + CMBPANCD_S1 + " \n";
			G_INFO += " CMBPANCD_S2 : " + CMBPANCD_S2 + " \n";
			G_INFO += " EDTFRT_SEARCH_S1 : " + EDTFRT_SEARCH_S1 + " \n";
			G_INFO += " EDTFRT_SEARCH_S2 : " + EDTFRT_SEARCH_S2 + " \n";
			G_INFO += "\n\n";
