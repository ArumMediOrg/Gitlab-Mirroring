...
SELECT SCANDATE, CHART_NO AS EXAM_NO, FORMNAME AS DOCUMENT, FILENAME, SERIAL, REQUEST_DATE, EXAM_NAME
 FROM ARUM_TOTAL
 WHERE REQUEST_DATE BETWEEN :First_date  AND :Second_date 
   AND CHART_NO = :CHART_NO
   AND SENDYN = 'Y'
 ORDER BY REQUEST_DATE , SERIAL ,FILENAME
		*/

		sql = " SELECT SCANDATE, CHART_NO AS EXAM_NO, FORMNAME AS DOCUMENT, FILENAME, SERIAL, REQUEST_DATE, EXAM_NAME";
		sql += " FROM ARUM_TOTAL";
		sql += " WHERE REQUEST_DATE BETWEEN '" + FIRST_DATE + "'";
		sql += " AND '" + SECOND_DATE + "'";
		sql += " AND CHART_NO = '" + CHART_NO + "'";
		sql += " AND SENDYN = 'Y'";
		sql += " ORDER BY REQUEST_DATE , SERIAL ,FILENAME";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : omr_selectsearch_005 \n";
			G_INFO += "설명 : OMR 검색버튼 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='REQUEST_DATE' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='ARUM_TOTAL'
