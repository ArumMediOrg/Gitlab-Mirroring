...
<%
		...
		sql += " F_PID(ICR.ICR_PENL_ID) AS ICR_PENL_ID, A.EEA_PSNL_AGE,    ";
		sql += " CASE WHEN A.EEA_SEX_CD = '1' THEN 'M' ELSE 'F' END AS EEA_SEX_CD, ";
		sql += " ICY.ICY_COMP_NM, A.EEA_TEL_NO, A.EEA_MOBL_NO,  ";
		sql += " A.EEA_ZIP_AR, A.EEA_ROAD_AR, ICR.ICR_CPENL_NM, ICR.ICR_PENL_ID AS PID_RAW ";
		sql += " FROM ET_EXAM_ACPT A JOIN ( SELECT EEA_CUST_NO";
		sql += " FROM ET_EXAM_ACPT";
		sql += " WHERE EEA_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND EEA_EXAM_SQ = '" + EXAM_SQ + "') B";
		sql += " ON A.EEA_CUST_NO = B.EEA_CUST_NO";
		...
		sql += " ORDER BY EEA_EXAM_DT DESC";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uGetExamPaperInfo_TPaperInfoCreate_001 \n";
			G_INFO += "설명 : 환자정보 로딩 \n";
			G_INFO += "\n\n";
