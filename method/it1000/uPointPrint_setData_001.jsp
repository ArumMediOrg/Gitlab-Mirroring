...
<%
		...
		sql += " from ";
		sql += " ( ";
		sql += " 		select A.rrm_rece_no ";
		sql += " 		,sum(nvl(A.rrm_rece_pr,0)) TsumPrice ";
		sql += " 		,sum(nvl(A.rrm_return_pr,0)) TreturnPrice ";
		sql += " 		,sum(nvl(A.rrm_rece_pr,0)-nvl(A.rrm_return_pr, 0)) TotalPRICE ";
		sql += " 		from ";
		sql += " 		( ";
		sql += " 				select  AA.rrm_rece_no, AA.rrm_rece_pr, ";
		sql += " 								AA.rrm_return_pr ";
		sql += " 				from rt_rece_medichk AA ";
		sql += " 				where (AA.rrm_pay_kd = '05') ";
		sql += " 				and (AA.rrm_input_dtt < '" + TODT + "') ";
		sql += " 				and (AA.rrm_input_dtt >= '" + FROMDT + "') ";
		sql += " 		) A ";
		sql += " 		group by A.RRM_RECE_NO ";
		sql += " ) RECE_INFO join RT_KUMC_POINT B ON ";
		...
		sql += " ) group by RKP_LOCAL_NM, RKP_EMPL_NO, RKP_EMPL_NM, RKP_DEPT_NM ";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uPointPrint_setData_001 \n";
			G_INFO += "설명 : 병원 포인트 사용현황 인쇄 \n";
			G_INFO += "\n\n";
