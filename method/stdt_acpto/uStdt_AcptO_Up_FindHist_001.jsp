...
SELECT EEA_EXAM_DT || '일 ' || EEA_EXAM_SQ || '번, ' ||
       CASE WHEN SUBSTR(NVL(EEA_HLTH_KD, ''00000''), 1, 1) = '1' THEN '-확진검사' ELSE '' END AS SAME_NM 
  FROM ET_EXAM_ACPT 
 ...
 ORDER BY EEA_EXAM_DT DESC, EEA_EXAM_SQ DESC
		*/
		...
		sql = " SELECT EEA_EXAM_DT || '일 ' || EEA_EXAM_SQ || '번, ' || CASE WHEN SUBSTR(NVL(EEA_HLTH_KD, '00000'), 1, 1) = '1' THEN '-확진검사' ELSE '' END AS SAME_NM";
		...
		sql += " ORDER BY EEA_EXAM_DT DESC, EEA_EXAM_SQ DESC";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStdt_AcptO_Up_FindHist_001 \n";
			G_INFO += "설명 : 당해연도에 학교밖검진 여부 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='SAME_NM' rs:number='1' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
			 rs:basecolumn='SAME_NM'>
			<s:datatype dt:type='string' dt:maxLength='32'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='2' rs:rowid='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
