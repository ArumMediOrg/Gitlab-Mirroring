...
<%
		...
		sql = " SELECT A.ERI_ITEM_CD, ";
		sql += " CASE WHEN B.IIM_EXGN_CD = 'I' THEN ";
		sql += " (Select ERI_RSLT_VL from ET_RSLT_ITEM Where ERI_EXAM_DT = '" + EXAM_DT + "' And ERI_EXAM_SQ = '" + EXAM_SQ + "' And ERI_ITEM_CD = 'N0042') ";
		sql += " ELSE A.ERI_RSLT_VL END ERI_RSLT_VL, ";
		  sql += " A.ERI_PACS_RSLT, B.IIM_RSLT_KD, B.IIM_EXGN_CD, B.ITEM_PR, B.GASN_PR FROM ET_RSLT_ITEM A ";
          sql += " INNER JOIN ";
          sql += " (SELECT C.IIM_ITEM_CD, C.IIM_KNME_NM, C.IIM_EXGN_CD, C.IIM_RSLT_KD, ";
          ...
          sql += " CASE WHEN 'IIC_HLTH_KD' = 'IIC_STDT_KD' THEN 0 ELSE B.IIP_GASN_PR END AS GASN_PR ";
          sql += " FROM IT_ITEM_CALL A, IT_ITEM_PRICE B, IT_ITEM C ";
          sql += " WHERE IIC_EXAM_YR = '" + EXAM_YR + "' AND ";
          ...
          sql += " AND B.IIP_APLY_DT = GETITEM_PRICE_APPLYDATE(A.IIC_ITEM_CD, '" + EXAM_YR + "' || '-12-31', 'ITEM_CD') ";
          ...
          sql += " AND ERI_CNCL_YN <> 'Y' ";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : UChungfile2020_JSON_UDP_UINT_CHUNGPER_004 \n";
			G_INFO += "설명 : 일반검진 건보수가 조회 \n";
			G_INFO += "\n\n";
