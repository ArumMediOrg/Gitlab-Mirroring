...
SELECT ECI_ITEM_CD, ECI_ITEM_PR, ECI_CNRT_PR FROM ET_COMP_CNRT_PROF_ITEM
WHERE ECI_COMP_CD =  :COMP_CD
  AND ECI_MNGT_YR =  :MNGT_YR
  AND ECI_CNRT_SQ =  :CNRT_SQ
  AND ECI_PROF_SQ <> :PROF_SQ
GROUP BY ECI_ITEM_CD, ECI_ITEM_PR, ECI_CNRT_PR
		*/

		sql = " SELECT ECI_ITEM_CD, ECI_ITEM_PR, ECI_CNRT_PR";
		...
		sql += " GROUP BY ECI_ITEM_CD, ECI_ITEM_PR, ECI_CNRT_PR";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCOMP_CNRT_PROF_APPLY_FormShow_002 \n";
			G_INFO += "설명 : 사업장 검진유형 검사항목리스트 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ECI_ITEM_PR' rs:number='2' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_COMP_CNRT_PROF_ITEM'
			 rs:basecolumn='ECI_ITEM_PR'>
			<s:datatype dt:type='number' rs:dbtype='numeric' dt:maxLength='19' rs:scale='0' rs:precision='10' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='ECI_CNRT_PR' rs:number='3' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_COMP_CNRT_PROF_ITEM'
			 rs:basecolumn='ECI_CNRT_PR'>
			<s:datatype dt:type='number' rs:dbtype='numeric' dt:maxLength='19' rs:scale='0' rs:precision='10' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='4' rs:rowid='true' rs:basetable='ET_COMP_CNRT_PROF_ITEM' rs:basecolumn='ROWID'
