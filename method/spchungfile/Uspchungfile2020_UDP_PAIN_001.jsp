...
SELECT EEA_EXAM_DT,EEA_EXAM_SQ,EEA_PSNL_NM, SSM_EXAM_DT, SSM_SYMP_SQ,SSM_SYSQ_CD,
       SSM_SYSQ_NM ,'' PAIN_SYMPTMS_REMARK
  FROM ET_EXAM_ACPT A LEFT OUTER JOIN ST_SYMPTOM B   ON B.SSM_EXAM_DT=A.EEA_EXAM_DT AND B.SSM_EXAM_SQ=A.EEA_EXAM_SQ and SSM_EXAM_CHA = '1' AND SSM_SYMP_CD='2'
 ...
 WHERE 0=0
   ...
   ||:sSQL_ADD
		*/
		...
		sql = " SELECT EEA_EXAM_DT,EEA_EXAM_SQ,EEA_PSNL_NM, SSM_EXAM_DT, SSM_SYMP_SQ,SSM_SYSQ_CD, SSM_SYSQ_NM ,'' PAIN_SYMPTMS_REMARK";
		...
		sql += " AND SSM_SYMP_CD='2' LEFT OUTER JOIN ST_BASE D";
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : Uspchungfile2020_UDP_PAIN_001 \n";
			G_INFO += "설명 : 특검청구 현재증상 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='PAIN_SYMPTMS_REMARK' rs:number='8' rs:nullable='true'>
...
<%
		...
		while(cRsList.next()) {
			...
			String PAIN_SYMPTMS_REMARK_T = cRsList.getString("PAIN_SYMPTMS_REMARK");
...
<%
			...
			if(! PAIN_SYMPTMS_REMARK_T.equals("")) {
%>
		 		PAIN_SYMPTMS_REMARK='<%= PAIN_SYMPTMS_REMARK_T%>'
<%
			}
%>
				ROWID='<%= cnt%>'
