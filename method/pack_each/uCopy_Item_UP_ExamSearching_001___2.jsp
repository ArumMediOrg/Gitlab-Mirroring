...
<%
      ...
      select epk_exam_cd||'-'||epk_pack_sq epk_pack_no, epk_exam_cd, epk_pack_sq, epk_pack_nm 
                  from et_pack 
                 where epk_exam_cd like :exam_cd||'%'
      if :edPack_Nm <> '' then
                 and epk_pack_nm like '%'||:edPack_Nm||'%'
		...
		*/
		...
		sql = " SELECT EPK_EXAM_CD||'-'||EPK_PACK_SQ EPK_PACK_NO, EPK_EXAM_CD, EPK_PACK_SQ, EPK_PACK_NM";
		sql += " FROM ET_PACK";
		sql += " WHERE EPK_EXAM_CD LIKE '" + EXAM_CD + "%'";

		if(!EDPACK_NM.equals("")) {
			sql += " AND EPK_PACK_NM LIKE '%" + EDPACK_NM + "%'";
		}
		...
		sql += " ORDER BY EPK_EXAM_CD, EPK_PACK_SQ";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCopy_Item_UP_ExamSearching_001___2 \n";
			G_INFO += "설명 : 묶음코드 로딩___2 \n";
			G_INFO += "\n\n";
