...
SELECT A.*, B.IIM_ITEM_CD, B.IIM_KNME_NM, B.IIM_SNME_NM, B.IIM_REJH_EX
FROM ET_RSLT_ITEM A INNER JOIN IT_ITEM B ON A.ERI_ITEM_CD = B.IIM_ITEM_CD
WHERE A.ERI_EXAM_DT = :EXAM_DT
  AND A.ERI_EXAM_SQ = :EXAM_SQ
  AND A.ERI_ITEM_CD IN ('A0001','A0002','A0012','A0013')
  AND A.ERI_CNCL_YN = 'N'

		*/

		sql = " SELECT A.*, B.IIM_ITEM_CD, B.IIM_KNME_NM, B.IIM_SNME_NM, B.IIM_REJH_EX";
		...
		sql += " AND A.ERI_ITEM_CD IN ('A0001','A0002','A0012','A0013')";
		sql += " AND A.ERI_CNCL_YN = 'N'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStudent_stats_physical_UP_Calculation_001 \n";
			G_INFO += "설명 : 검사결과정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='IIM_ITEM_CD' rs:number='70' rs:writeunknown='true' rs:basetable='IT_ITEM' rs:basecolumn='IIM_ITEM_CD'>
			<s:datatype dt:type='string' dt:maxLength='10' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='IIM_KNME_NM' rs:number='71' rs:writeunknown='true' rs:basetable='IT_ITEM' rs:basecolumn='IIM_KNME_NM'>
			<s:datatype dt:type='string' dt:maxLength='200' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='IIM_SNME_NM' rs:number='72' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM'
			 rs:basecolumn='IIM_SNME_NM'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='IIM_REJH_EX' rs:number='73' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM'
			 rs:basecolumn='IIM_REJH_EX'>
			<s:datatype dt:type='string' dt:maxLength='400'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='74' rs:rowid='true' rs:basetable='ET_RSLT_ITEM' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c74' rs:name='ROWID' rs:number='75' rs:rowid='true' rs:basetable='IT_ITEM' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
