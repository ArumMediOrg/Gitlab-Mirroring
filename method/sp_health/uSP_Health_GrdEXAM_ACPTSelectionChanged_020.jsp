...
SELECT CASE WHEN SPB_GOJU_CD IS NULL OR SPB_GOJU_CD = '' THEN ''
            ELSE GetCOMMON_LvCdToNm('0808',SPB_GOJU_CD,'5',5) END SPB_GOJU_NM, A.*
  FROM ST_PASTJOB A
 WHERE SPB_EXAM_DT  = :EXAM_DT
   ...
   AND SPB_EXAM_CHA = :EXAM_CHA
		*/
		...
		sql = " SELECT CASE WHEN SPB_GOJU_CD IS NULL";
		sql += " OR SPB_GOJU_CD = '' THEN '' ELSE GETCOMMON_LVCDTONM('0808',SPB_GOJU_CD,'5',5) END SPB_GOJU_NM, A.*";
		sql += " FROM ST_PASTJOB A";
		...
		sql += " AND SPB_EXAM_CHA = '" + EXAM_CHA + "'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_Health_GrdEXAM_ACPTSelectionChanged_020 \n";
			G_INFO += "설명 : 특검과거직력조회(공정명포함) \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='SPB_GOJU_NM' rs:number='1' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PASTJOB'
			 rs:basecolumn='SPB_GOJU_NM'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_EXAM_DT' rs:number='2' rs:writeunknown='true' rs:basetable='ST_PASTJOB' rs:basecolumn='SPB_EXAM_DT'>
			<s:datatype dt:type='string' dt:maxLength='10' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_EXAM_SQ' rs:number='3' rs:writeunknown='true' rs:basetable='ST_PASTJOB' rs:basecolumn='SPB_EXAM_SQ'>
			<s:datatype dt:type='string' dt:maxLength='10' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_EXAM_CHA' rs:number='4' rs:writeunknown='true' rs:basetable='ST_PASTJOB' rs:basecolumn='SPB_EXAM_CHA'>
			<s:datatype dt:type='string' dt:maxLength='10' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_GOJU_SQ' rs:number='5' rs:writeunknown='true' rs:basetable='ST_PASTJOB' rs:basecolumn='SPB_GOJU_SQ'>
			<s:datatype dt:type='number' rs:dbtype='varnumeric' dt:maxLength='38' rs:scale='255' rs:precision='38' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_GOJU_CD' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PASTJOB'
			 rs:basecolumn='SPB_GOJU_CD'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_YEAR_VAL' rs:number='7' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PASTJOB'
		...
		<s:AttributeType name='SPB_ETC_TX' rs:number='10' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PASTJOB'
			 rs:basecolumn='SPB_ETC_TX'>
			<s:datatype dt:type='string' dt:maxLength='20'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_INPT_ID' rs:number='11' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PASTJOB'
			 rs:basecolumn='SPB_INPT_ID'>
			<s:datatype dt:type='string' dt:maxLength='20'/>
		</s:AttributeType>
		<s:AttributeType name='SPB_INPT_DTT' rs:number='12' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PASTJOB'
			 rs:basecolumn='SPB_INPT_DTT'>
			<s:datatype dt:type='dateTime' rs:dbtype='timestamp' dt:maxLength='16' rs:scale='0' rs:precision='19' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='13' rs:rowid='true' rs:basetable='ST_PASTJOB' rs:basecolumn='ROWID'
...
<%
		...
		while(cRsList.next()) {
			...
			String SPB_GOJU_NM_T = cRsList.getString("SPB_GOJU_NM");
...
%>
			<z:row
<%
			if(! SPB_GOJU_NM_T.equals("")) {
%>
		 		SPB_GOJU_NM='<%= SPB_GOJU_NM_T%>'
<%
			}
