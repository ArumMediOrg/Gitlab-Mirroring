...
<%
		...
		String CCF_LCODE = htMethod.get("CCF_LCODE");
		String CMBCODE = htMethod.get("CMBCODE");
		String CMBPANCD = htMethod.get("CMBPANCD");
		String EDTSEARCH = htMethod.get("EDTSEARCH");

		//
		if(CCF_LCODE == null) { CCF_LCODE = ""; }
		if(CMBCODE == null) { CMBCODE = ""; }
		if(CMBPANCD == null) { CMBPANCD = ""; }
...
SELECT  *
 ...
 WHERE CCF_LCODE = :CCF_LCODE

if :cmbCode <> '' then
begin
  AND CCF_CODE = :cmbCode
end;
if :cmbPanCd <> '' then
begin
  AND CCF_PAN_CD = :cmbPanCd
end;
if :edtSearch <> '' then
begin
  AND CCF_PANVIEW Like '%':edtSearch'%'
end;
...
ORDER BY CCF_CODENM, CCF_PAN_CD, CCF_PAN_SQ
		*/
		...
		sql += " WHERE CCF_LCODE = '" + CCF_LCODE + "'";

		if(!CMBCODE.equals("")) {
			sql += " AND CCF_CODE = '" + CMBCODE + "'";
		}

		if(!CMBPANCD.equals("")) {
			sql += " AND CCF_PAN_CD = '" + CMBPANCD + "'";
		}

		if(!EDTSEARCH.equals("")) {
			sql += " AND CCF_PANVIEW LIKE '%" + EDTSEARCH + "%'";
		}
		...
		sql += " ORDER BY CCF_CODENM, CCF_PAN_CD, CCF_PAN_SQ";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFind_Opin_upGetRslt_001 \n";
			G_INFO += "설명 : 1차판정소견조회(소견내역조회) \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " CCF_LCODE : " + CCF_LCODE + " \n";
			G_INFO += " CMBCODE : " + CMBCODE + " \n";
			G_INFO += " CMBPANCD : " + CMBPANCD + " \n";
