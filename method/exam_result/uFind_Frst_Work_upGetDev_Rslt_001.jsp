...
SELECT  CSC_SMALL,CSC_FULL_NM
  FROM CT_SP_COMMON
 WHERE 0=0
   AND CSC_LARGE='HM05H' AND  CSC_ETC1_CD='1'

if  :edtSearch <> '' then
begin
  AND (UPPER(CSC_FULL_NM) LIKE '%':edtSearch'%'
    OR UPPER(CSC_SMALL) LIKE '%':edtSearch'%')
end
		*/
		...
		sql += " WHERE 0=0";
		sql += " AND CSC_LARGE='HM05H'";
		sql += " AND CSC_ETC1_CD='1'";

		if(!EDTSEARCH.equals("")) {
			sql += " AND (UPPER(CSC_FULL_NM) LIKE '%" + EDTSEARCH + "%'";
			sql += " OR UPPER(CSC_SMALL) LIKE '%" + EDTSEARCH + "%')";
		}
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFind_Frst_Work_upGetDev_Rslt_001 \n";
			G_INFO += "설명 : 노동부판정 질병내역 조회 \n";
			G_INFO += "\n\n";
