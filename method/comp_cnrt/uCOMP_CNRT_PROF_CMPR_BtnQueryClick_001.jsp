...
SELECT '1' AS YH, ECI_ITEM_CD FROM ET_COMP_CNRT_PROF_ITEM
WHERE ECI_COMP_CD = :COMP_CD1
  AND ECI_MNGT_YR = :MNGT_YR1
  AND ECI_CNRT_SQ = :CNRT_SQ1
  AND ECI_PROF_SQ = :PROF_SQ1
UNION
SELECT '2' AS YH, ECI_ITEM_CD FROM ET_COMP_CNRT_PROF_ITEM
WHERE ECI_COMP_CD = :COMP_CD2
  AND ECI_MNGT_YR = :MNGT_YR2
  AND ECI_CNRT_SQ = :CNRT_SQ2
  AND ECI_PROF_SQ = :PROF_SQ2
		*/

		sql = " SELECT '1' AS YH, ECI_ITEM_CD";
		sql += " FROM ET_COMP_CNRT_PROF_ITEM";
		sql += " WHERE ECI_COMP_CD = '" + COMP_CD1 + "'";
		sql += " AND ECI_MNGT_YR = '" + MNGT_YR1 + "'";
		sql += " AND ECI_CNRT_SQ = '" + CNRT_SQ1 + "'";
		sql += " AND ECI_PROF_SQ = '" + PROF_SQ1 + "'";

		sql += " UNION";

		sql += " SELECT '2' AS YH, ECI_ITEM_CD";
		sql += " FROM ET_COMP_CNRT_PROF_ITEM";
		sql += " WHERE ECI_COMP_CD = '" + COMP_CD2 + "'";
		sql += " AND ECI_MNGT_YR = '" + MNGT_YR2 + "'";
		sql += " AND ECI_CNRT_SQ = '" + CNRT_SQ2 + "'";
		sql += " AND ECI_PROF_SQ = '" + PROF_SQ2 + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCOMP_CNRT_PROF_CMPR_BtnQueryClick_001 \n";
			G_INFO += "설명 : 검진유형 검사항목리스트 조회(비교대상1,2) \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ECI_ITEM_CD' rs:number='2' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_COMP_CNRT_PROF_ITEM'
