      ...
      WHERE A.EEA_EXAM_DT = :EXAM_DT
		...
		*/
		...
		if(G_ENCRT_KD.equals("1")) {
			...
			sql = " SELECT ECHELONDB.ECL_ENCRYP(B.ICR_PID_EN) AS ICR_PENL_DE,";
		} else {
			sql = " SELECT B.ICR_PENL_ID AS ICR_PENL_DE,";
		}        

		sql += " F_COMP_FIND(A.EEA_COMP_CD) EEA_COMP_NM, F_CODE_FIND('0908', A.EEA_CNTR_CD, '', '1') EEA_CNTR_NM, A.*, B.ICR_PENL_ID";
		...
		sql += " AND A.EEA_EXAM_SQ = '" + EXAM_SQ + "'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCancer_Quest_19_QRBand1BeforePrint_001 \n";
			G_INFO += "설명 : 인쇄를 위한 문진결과 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='EEA_CNTR_NM' rs:number='3' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		...
		<s:AttributeType name='ICR_PENL_ID' rs:number='147' rs:writeunknown='true' rs:basetable='IT_CUSTOMER' rs:basecolumn='ICR_PENL_ID'>
			<s:datatype dt:type='string' dt:maxLength='13' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='148' rs:rowid='true' rs:basetable='ET_EXAM_ACPT' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c148' rs:name='ROWID' rs:number='149' rs:rowid='true' rs:basetable='IT_CUSTOMER' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
...
<%
		...
		while(cRsList.next()) {
			...
			String c148_T = cRsList.getString("c148");
%>
			<z:row
<%
