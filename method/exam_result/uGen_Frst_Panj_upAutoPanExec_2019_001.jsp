...
SELECT ERI_EXAM_DT, ERI_EXAM_SQ, ERI_ITEM_CD, ERI_RSLT_VL
     , (ERI_RSLT_LOW ||' - '|| ERI_RSLT_HIGH) LH
     , ERI_HTSB_PANJ, ERI_VLDT_LH, ERI_HERT_CD, IIM_SNME_NM
     , EEA_SEX_CD, EEA_PSNL_AGE, EEA_EXAM_CD, EEA_HTSB_CD
  FROM ET_RSLT_ITEM A
  LEFT OUTER JOIN IT_ITEM B
    ON A.ERI_ITEM_CD = B.IIM_ITEM_CD
  LEFT OUTER JOIN ET_EXAM_ACPT C
    ON A.ERI_EXAM_DT = C.EEA_EXAM_DT
   AND A.ERI_EXAM_SQ = C.EEA_EXAM_SQ
 WHERE ERI_EXAM_DT = :EXAM_DT
   AND ERI_EXAM_SQ = :EXAM_SQ
   AND ERI_HLTH_KD <> '0'
   AND (ERI_HTSB_PANJ IS NOT NULL OR ERI_HERT_CD IS NOT NULL )
		*/

		sql = " SELECT ERI_EXAM_DT, ERI_EXAM_SQ, ERI_ITEM_CD, ERI_RSLT_VL , (ERI_RSLT_LOW ||' - '|| ERI_RSLT_HIGH) LH , ERI_HTSB_PANJ, ERI_VLDT_LH, ERI_HERT_CD, IIM_SNME_NM , EEA_SEX_CD, EEA_PSNL_AGE, EEA_EXAM_CD, EEA_HTSB_CD";
		sql += " FROM ET_RSLT_ITEM A LEFT OUTER JOIN IT_ITEM B";
		sql += " ON A.ERI_ITEM_CD = B.IIM_ITEM_CD LEFT OUTER JOIN ET_EXAM_ACPT C";
		sql += " ON A.ERI_EXAM_DT = C.EEA_EXAM_DT";
		sql += " AND A.ERI_EXAM_SQ = C.EEA_EXAM_SQ";
		sql += " WHERE ERI_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND ERI_EXAM_SQ = '" + EXAM_SQ + "'";
		sql += " AND ERI_HLTH_KD <> '0'";
		sql += " AND (ERI_HTSB_PANJ IS NOT NULL OR ERI_HERT_CD IS NOT NULL )";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uGen_Frst_Panj_upAutoPanExec_2019_001 \n";
			G_INFO += "설명 : 일검자동판정(일검검사항목조회) \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ERI_HTSB_PANJ' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_RSLT_ITEM'
			 rs:basecolumn='ERI_HTSB_PANJ'>
			<s:datatype dt:type='string' dt:maxLength='30'/>
		</s:AttributeType>
		<s:AttributeType name='ERI_VLDT_LH' rs:number='7' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_RSLT_ITEM'
			 rs:basecolumn='ERI_VLDT_LH'>
			<s:datatype dt:type='string' dt:maxLength='1'/>
		</s:AttributeType>
		<s:AttributeType name='ERI_HERT_CD' rs:number='8' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_RSLT_ITEM'
		...
		<s:AttributeType name='EEA_SEX_CD' rs:number='10' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
		...
		<s:AttributeType name='EEA_EXAM_CD' rs:number='12' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
			 rs:basecolumn='EEA_EXAM_CD'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='EEA_HTSB_CD' rs:number='13' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
			 rs:basecolumn='EEA_HTSB_CD'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='14' rs:rowid='true' rs:basetable='ET_RSLT_ITEM' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c14' rs:name='ROWID' rs:number='15' rs:rowid='true' rs:basetable='IT_ITEM' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c15' rs:name='ROWID' rs:number='16' rs:rowid='true' rs:basetable='ET_EXAM_ACPT' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
