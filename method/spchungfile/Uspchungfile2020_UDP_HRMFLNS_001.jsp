...
SELECT EEA_EXAM_DT,EEA_EXAM_SQ,EEA_PSNL_NM, SPG_EXAM_DT,
       SPG_MATT_CD, CASE WHEN EEA_NOTE_YN='Y' THEN '1' ELSE '0' END EEA_NOTE_YN ,'' BIZ_TRTMNT_YN
  FROM ET_EXAM_ACPT A LEFT OUTER JOIN  ST_PANJUNG  B ON B.SPG_EXAM_DT=A.EEA_EXAM_DT AND B.SPG_EXAM_sq=A.EEA_EXAM_SQ
 ...
 WHERE 0=0
   ...
   ||:sSQL_ADD
		...
		*/
		...
		sql = " SELECT EEA_EXAM_DT,EEA_EXAM_SQ,EEA_PSNL_NM, SPG_EXAM_DT, SPG_MATT_CD, CASE WHEN EEA_NOTE_YN='Y' THEN '1' ELSE '0' END EEA_NOTE_YN ,'' BIZ_TRTMNT_YN";
		sql += " FROM ET_EXAM_ACPT A LEFT OUTER JOIN ST_PANJUNG B";
		sql += " ON B.SPG_EXAM_DT=A.EEA_EXAM_DT";
		sql += " AND B.SPG_EXAM_SQ=A.EEA_EXAM_SQ LEFT OUTER JOIN ST_BILLHN_PR C";
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : Uspchungfile2020_UDP_HRMFLNS_001 \n";
			G_INFO += "설명 : 특검청구 개인별유해인자 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='SPG_EXAM_DT' rs:number='4' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PANJUNG'
			 rs:basecolumn='SPG_EXAM_DT'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='SPG_MATT_CD' rs:number='5' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PANJUNG'
			 rs:basecolumn='SPG_MATT_CD'>
			<s:datatype dt:type='string' dt:maxLength='5'/>
		</s:AttributeType>
		<s:AttributeType name='EEA_NOTE_YN' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
			 rs:basecolumn='EEA_NOTE_YN'>
			<s:datatype dt:type='string' dt:maxLength='1' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='BIZ_TRTMNT_YN' rs:number='7' rs:nullable='true'>
		...
		<s:AttributeType name='c9' rs:name='ROWID' rs:number='10' rs:rowid='true' rs:basetable='ST_BILLHN_PR' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
...
<%
		...
		while(cRsList.next()) {
			...
			String BIZ_TRTMNT_YN_T = cRsList.getString("BIZ_TRTMNT_YN");
...
<%
			...
			if(! BIZ_TRTMNT_YN_T.equals("")) {
%>
		 		BIZ_TRTMNT_YN='<%= BIZ_TRTMNT_YN_T%>'
<%
			}
%>
				ROWID='<%= cnt%>'
