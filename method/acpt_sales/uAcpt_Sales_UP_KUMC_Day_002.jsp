...
<%
		...
		sql = " SELECT SEX_KIND, SUM(EXAM_CNT1) EXAM_CNT1, SUM(EXAM_CNT2) EXAM_CNT2, ";
		sql += " 	SUM(EXAM_CNT1) - SUM(EXAM_CNT2) DIFF_CNT, ";
		sql += " 	CASE WHEN SUM(EXAM_CNT2) = 0 THEN 0 ";
		sql += " 			ELSE ROUND(ABS(SUM(EXAM_CNT1) - SUM(EXAM_CNT2)) / SUM(EXAM_CNT2) * 100, 1) END DIFF_RT ";
		sql += " FROM (SELECT '1' SEX_KIND, 0 EXAM_CNT1, 0 EXAM_CNT2 FROM DUAL ";
		sql += " 		UNION ALL ";
		sql += " 		SELECT '2' SEX_KIND, 0 EXAM_CNT1, 0 EXAM_CNT2 FROM DUAL ";
		sql += " 		UNION ALL ";
		sql += " 		SELECT EEA_SEX_CD SEX_KIND, 1 EXAM_CNT1, 0 EXAM_CNT2 ";
		sql += " 		FROM ET_EXAM_ACPT ";
		sql += " 		WHERE EEA_EXAM_DT BETWEEN '" + EXAM_SDT + "' AND '" + EXAM_EDT + "' ";
		sql += " 		AND EEA_ORDER_YN <> 'C' ";
		sql += PARAM_TX;
		sql += " 		UNION ALL ";
		sql += " 		SELECT EEA_SEX_CD SEX_KIND, 0 EXAM_CNT1, 1 EXAM_CNT2 ";
		sql += " 		FROM ET_EXAM_ACPT ";
		sql += " 		WHERE EEA_EXAM_DT BETWEEN '" + EXAM_SDT2 + "' AND '" + EXAM_EDT2 + "' ";
		sql += " 		AND EEA_ORDER_YN <> 'C' ";
		sql += PARAM_TX;
		sql += " 	) A ";
		sql += " GROUP BY SEX_KIND ";
		sql += " ORDER BY SEX_KIND ";

			//
		G_INFO += "<!-- \n";
		G_INFO += "서비스명 : uAcpt_Sales_UP_KUMC_Day_002 \n";
		G_INFO += "설명 : 성별 수검 현황 \n";
		G_INFO += "\n\n";
