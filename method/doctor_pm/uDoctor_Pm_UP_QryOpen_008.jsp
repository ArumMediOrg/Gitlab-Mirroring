...
SELECT A.* FROM ET_DENTAL_PANJ A
if :cmbPlaceFlg = '1' then
begin
        Inner Join ET_EXAM_ACPT B On B.EEA_EXAM_DT = A.EDP_EXAM_DT
                And B.EEA_EXAM_SQ = A.EDP_EXAM_SQ                      
                And B.EEA_ORDER_YN <> 'C'                                   
                And SUBSTR(B.EEA_PLCE_CD, 1, 2)= :cmbPlace
end;
WHERE 0=0 
        AND A.EDP_EXAM_DT >= :DTEdtEXAM_SDT
        AND A.EDP_EXAM_DT <= :DTEdtEXAM_EDT
        AND NVL(A.EDP_DOCT_CD, ' ') <> ' ' 
		*/

		sql = " SELECT A.*";
		sql += " FROM ET_DENTAL_PANJ A";

		if(CMBPLACEFLG.equals("1")) {
			sql += " INNER JOIN ET_EXAM_ACPT B";
			sql += " ON B.EEA_EXAM_DT = A.EDP_EXAM_DT";
			sql += " AND B.EEA_EXAM_SQ = A.EDP_EXAM_SQ";
			sql += " AND B.EEA_ORDER_YN <> 'C'";
			sql += " AND SUBSTR(B.EEA_PLCE_CD, 1, 2)= '" + CMBPLACE + "'";
		}

		sql += " WHERE 0=0";
		sql += " AND A.EDP_EXAM_DT >= '" + DTEDTEXAM_SDT + "'";
		sql += " AND A.EDP_EXAM_DT <= '" + DTEDTEXAM_EDT + "'";
		sql += " AND NVL(A.EDP_DOCT_CD, ' ') <> ' '";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UP_QryOpen_008 \n";
			G_INFO += "설명 : 구강검진정보 로딩 \n";
			G_INFO += "\n\n";
