...
<%
		...
		String MS_MATT_CD = htMethod.get("MS_MATT_CD");

		//
		if(MS_MATT_CD == null) { MS_MATT_CD = ""; }
...
FROM CT_SP_MATTER A INNER JOIN CT_SP_COMMON B ON A.CSM_COMM_CD = B.CSC_SMALL    
WHERE A.CSM_MATT_CD In (:Ms_Matt_Cd)                                         
AND   B.CSC_LARGE   = 'HM99A'      
		*/

		sql = " SELECT A.CSM_LARGE, A.CSM_MATT_CD, A.CSM_MATT_NM, B.CSC_ETC1_NM, B.CSC_ETC1_CD, B.CSC_ETC2_NM, B.CSC_ETC2_CD";
		sql += " FROM CT_SP_MATTER A INNER JOIN CT_SP_COMMON B";
		sql += " ON A.CSM_COMM_CD = B.CSC_SMALL";
		sql += " WHERE A.CSM_MATT_CD IN ('" + MS_MATT_CD + "')";
		sql += " AND B.CSC_LARGE = 'HM99A'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_PRINT_UP_ITEMDisplay_004 \n";
			G_INFO += "설명 : 표적장기별검사 묶음코드 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " MS_MATT_CD : " + MS_MATT_CD + " \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='CSC_ETC1_NM' rs:number='4' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC1_NM'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='CSC_ETC1_CD' rs:number='5' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC1_CD'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='CSC_ETC2_NM' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC2_NM'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='CSC_ETC2_CD' rs:number='7' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC2_CD'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='8' rs:rowid='true' rs:basetable='CT_SP_MATTER' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c8' rs:name='ROWID' rs:number='9' rs:rowid='true' rs:basetable='CT_SP_COMMON' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
