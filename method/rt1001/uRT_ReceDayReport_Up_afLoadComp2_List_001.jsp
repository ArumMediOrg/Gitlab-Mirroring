...
from
  (select
     ...
     EEA_EXAM_DT,
            ...
            EEA_PSNL_NM,
            ICR_BIRH_DT,
            (select CCN_FULL_NM from CT_COMMON where CCN_LARGE = '0401' and CCN_SMALL = c.EEA_EXAM_CD)  EEA_EXAM_NM,
            (select CCN_FULL_NM from CT_COMMON where CCN_LARGE = '0925' and CCN_SMALL = a.RRM_OCCU_KD)  RRM_OCCU_NM,
            RRM_RECE_DT,
      case when RRM_PAY_KD ='01' then (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) end cardpr,
      case when RRM_PAY_KD IN ('02','03')  then (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) end cashpr,
      case when RRM_PAY_KD IN ('04')  then (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) end bkbookpr,
      case when RRM_PAY_KD  NOT IN ('01','02','03','04')  then (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) end otherpr,
      RRM_RETURN_PR returnpr,
                   (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0) - NVL(RRM_RETURN_PR,0)) totpr,
   ...
   from RT_RECE_MEDICHK a left outer join IT_AUTH_USER  b on a.RRM_MODI_ID = b.IAU_EMP_NO
                          left outer join ET_EXAM_ACPT  c on a.RRM_RECE_NO = c.EEA_RECE_NO
                                                          left outer join IT_CUSTOMER   d on c.EEA_CUST_NO = d.ICR_CUST_NO
   where
          RRM_RECE_DT >= :dp_fromdt2 and RRM_RECE_DT <= :dp_todt2
      and RRM_OCCU_KD in ('20','70','71','72')
if :avsg2 <> '합계'  then
begin
      and RRM_MODI_ID = :MODI_ID
		...
		*/
		...
		sql += " FROM (";
		sql += "	SELECT RRM_MODI_ID, IAU_PENL_NM, EEA_EXAM_DT, EEA_CHART_NO, EEA_PSNL_NM, ICR_BIRH_DT";
		sql += "	, (SELECT CCN_FULL_NM FROM CT_COMMON WHERE CCN_LARGE = '0401' AND CCN_SMALL = C.EEA_EXAM_CD) EEA_EXAM_NM";
		sql += "	, (SELECT CCN_FULL_NM FROM CT_COMMON WHERE CCN_LARGE = '0925' AND CCN_SMALL = A.RRM_OCCU_KD) RRM_OCCU_NM";
		sql += "	, RRM_RECE_DT";
		sql += "	, CASE WHEN RRM_PAY_KD ='01' THEN (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) END CARDPR";
		sql += "	, CASE WHEN RRM_PAY_KD IN ('02','03') THEN (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) END CASHPR";
		sql += "	, CASE WHEN RRM_PAY_KD IN ('04') THEN (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) END BKBOOKPR";
		sql += "	, CASE WHEN RRM_PAY_KD NOT IN ('01','02','03','04') THEN (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0)) END OTHERPR";
		sql += "	, RRM_RETURN_PR RETURNPR";
		sql += "	, (NVL(RRM_RECE_PR,0) - NVL(RRM_RESEREPL_PR,0) - NVL(RRM_RETURN_PR,0)) TOTPR";
		sql += "	, TO_CHAR(RRM_INPUT_DTT, 'YYYY-MM-DD HH24:MI:SS') RRM_INPUT_DTT";
		sql += "	, NVL(";
		sql += "		(";
		...
		sql += " ON A.RRM_MODI_ID = B.IAU_EMP_NO LEFT OUTER JOIN ET_EXAM_ACPT C";
		sql += " ON A.RRM_RECE_NO = C.EEA_RECE_NO LEFT OUTER JOIN IT_CUSTOMER D";
		sql += " ON C.EEA_CUST_NO = D.ICR_CUST_NO";
		...
		sql += " ORDER BY EEA_PSNL_NM";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uRT_ReceDayReport_Up_afLoadComp2_List_001 \n";
			G_INFO += "설명 : 직불수납정보 로딩 \n";
			G_INFO += "\n\n";
