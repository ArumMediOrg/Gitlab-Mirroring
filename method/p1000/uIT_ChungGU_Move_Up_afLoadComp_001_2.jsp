...
<%
		...
		String HANYANG_GURI = htMethod.get("HANYANG_GURI");
		...
		if(HANYANG_GURI == null) { HANYANG_GURI = ""; }
		...
		if(RZ_CHKD.equals("0")) {
			...
			sql += " FROM ( SELECT A.EEA_COMP_CD, EEA_CUST_NO, EEA_EXAM_CD, 0 EEA_DISC_CPR, C.ICY_COMP_NM, C.ICY_ZIP_CD, C.ICY_ZIP_AR || ' ' || ICY_ROAD_AR ICY_ZIP_AR, D.IAU_PENL_NM BJ_NM, '일반' CHUNGGB, 1 CNT, SUM(B.EEP_COMP_PR) CHUNGAMT";
			...
			sql = " SELECT *";
			sql += " FROM ( SELECT EEA_COMP_CD, MIN(EEA_EXAM_CD) EEA_EXAM_CD, ICY_COMP_NM,ICY_ZIP_CD,ICY_ZIP_AR, BJ_NM,CHUNGGB,COUNT (CNT) CNT,SUM(CHUNGAMT) CHUNGAMT";
			sql += " FROM ( SELECT A.EEA_COMP_CD, EEA_CUST_NO, EEA_EXAM_CD, 0 EEA_DISC_CPR, C.ICY_COMP_NM, C.ICY_ZIP_CD, C.ICY_ZIP_AR || ' ' || ICY_ROAD_AR ICY_ZIP_AR, D.IAU_PENL_NM BJ_NM, '전체' CHUNGGB, 1 CNT, SUM(B.EEP_COMP_PR+EEP_COMPS_PR+EEP_PSNLP_PR) CHUNGAMT";
			sql += " FROM ET_EXAM_ACPT A LEFT OUTER JOIN ET_EXAM_PRICE B";
			sql += " ON A.EEA_EXAM_DT = B.EEP_EXAM_DT";
			sql += " AND A.EEA_EXAM_SQ = B.EEP_EXAM_SQ LEFT OUTER JOIN IT_COMPANY C";
			sql += " ON A.EEA_COMP_CD = C.ICY_COMP_CD LEFT OUTER JOIN IT_AUTH_USER D";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uIT_ChungGU_Move_Up_afLoadComp_001_2 \n";
			G_INFO += "설명 : 사업장명 자료정보 로딩 : 한양대(본원) 이 외 \n";
			G_INFO += "\n\n";
			...
			G_INFO += " HANYANG_GURI : " + HANYANG_GURI + " \n";
			G_INFO += "\n\n";
