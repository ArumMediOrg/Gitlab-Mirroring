...
<%
		...
		String CHKBOXCKD_KSCD_CD = htMethod.get("CHKBOXCKD_KSCD_CD");
		String EDTKCD_NAME1 = htMethod.get("EDTKCD_NAME1");
		String EDTKCD_NAME2 = htMethod.get("EDTKCD_NAME2");
		String EDTKCD_NAME3 = htMethod.get("EDTKCD_NAME3");
		String EDTKCD_NAME4 = htMethod.get("EDTKCD_NAME4");

		//
		if(CHKBOXCKD_KSCD_CD == null) { CHKBOXCKD_KSCD_CD = ""; }
		if(EDTKCD_NAME1 == null) { EDTKCD_NAME1 = ""; }
		if(EDTKCD_NAME2 == null) { EDTKCD_NAME2 = ""; }
		if(EDTKCD_NAME3 == null) { EDTKCD_NAME3 = ""; }
		if(EDTKCD_NAME4 == null) { EDTKCD_NAME4 = ""; }
...
FROM CT_KSCD A
WHERE 0 = 0
if :ChkBoxCKD_KSCD_CD = '1' then
begin
      AND (CKD_KSCD_CD = :EdtKCD_NAME1
       OR  CKD_KSCD_CD = :EdtKCD_NAME2
       OR  CKD_KSCD_CD = :EdtKCD_NAME3
       OR  CKD_KSCD_CD = :EdtKCD_NAME4)
end else
begin
      AND (CKD_KNME_NM LIKE '%' || :EdtKCD_NAME1 || '%'
       OR  CKD_ENME_NM LIKE '%' || :EdtKCD_NAME1 || '%'
       OR  CKD_SNME_NM LIKE '%' || :EdtKCD_NAME1 || '%')
end;
		*/

		sql = " SELECT CASE WHEN CKD_SUKN_CD IS NULL";
		sql += " OR CKD_SUKN_CD = '' THEN CKD_SUKN_CD ELSE '[' || CKD_SUKN_CD || ']' || GETCOMMON_LVCDTONM('0604',CKD_SUKN_CD,'5',1) END CKD_SUKN_NM, A.*";
		sql += " FROM CT_KSCD A";
		sql += " WHERE 0 = 0";

		if(CHKBOXCKD_KSCD_CD.equals("1")) {
			sql += " AND (CKD_KSCD_CD = '" + EDTKCD_NAME1 + "'";
			sql += " OR CKD_KSCD_CD = '" + EDTKCD_NAME2 + "'";
			sql += " OR CKD_KSCD_CD = '" + EDTKCD_NAME3 + "'";
			sql += " OR CKD_KSCD_CD = '" + EDTKCD_NAME4 + "')";
		} else {
			sql += " AND (CKD_KNME_NM LIKE '%' || '" + EDTKCD_NAME1 + "' || '%'";
			sql += " OR CKD_ENME_NM LIKE '%' || '" + EDTKCD_NAME1 + "' || '%'";
			sql += " OR CKD_SNME_NM LIKE '%' || '" + EDTKCD_NAME1 + "' || '%')";
		}
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCT_KSCD_UP_Display_Filter_001 \n";
			G_INFO += "설명 : 필터정보 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " CHKBOXCKD_KSCD_CD : " + CHKBOXCKD_KSCD_CD + " \n";
			G_INFO += " EDTKCD_NAME1 : " + EDTKCD_NAME1 + " \n";
			G_INFO += " EDTKCD_NAME2 : " + EDTKCD_NAME2 + " \n";
			G_INFO += " EDTKCD_NAME3 : " + EDTKCD_NAME3 + " \n";
			G_INFO += " EDTKCD_NAME4 : " + EDTKCD_NAME4 + " \n";
			G_INFO += "\n\n";
