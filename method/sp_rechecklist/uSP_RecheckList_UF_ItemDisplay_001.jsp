...
SELECT B.IIM_ITEM_CD, B.IIM_KNME_NM, C.ERI_RSLT_VL                                     
  FROM IT_ITEM B                                                
       LEFT OUTER JOIN ET_RSLT_ITEM C ON B.IIM_ITEM_CD = C.ERI_ITEM_CD                      
 WHERE B.IIM_ITEM_CD = :ITEM_CD
   AND C.ERI_EXAM_DT = :EXAM_DT 
   AND C.ERI_EXAM_SQ = :EXAM_SQ 
   AND C.ERI_CNCL_YN <> 'Y'
		*/

		sql = " SELECT B.IIM_ITEM_CD, B.IIM_KNME_NM, C.ERI_RSLT_VL";
		sql += " FROM IT_ITEM B LEFT OUTER JOIN ET_RSLT_ITEM C";
		sql += " ON B.IIM_ITEM_CD = C.ERI_ITEM_CD";
		sql += " WHERE B.IIM_ITEM_CD = '" + ITEM_CD + "'";
		...
		sql += " AND C.ERI_CNCL_YN <> 'Y'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_RecheckList_UF_ItemDisplay_001 \n";
			G_INFO += "설명 : 일검 1차 유소견항목 조회 \n";
			G_INFO += "\n\n";
