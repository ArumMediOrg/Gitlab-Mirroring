...
<%
		...
		sql = " SELECT A.* FROM ET_CANCER_LUNG A ";

		if(CMBPLACEFLG.equals("1")){
			sql += " INNER JOIN ET_EXAM_ACPT B ON B.EEA_EXAM_DT = A.ECV_EXAM_DT ";
		...
		sql += " WHERE 0=0 ";
		sql += " AND A.ECG_EXAM_DT >= '" + DTEDTEXAM_SDT + "' ";
		sql += " AND A.ECG_EXAM_DT <= '" + DTEDTEXAM_EDT + "' ";
		sql += " AND (NVL(A.ECG_LICEN_CD, ' ') <> ' ' OR  NVL(A.ECG_AFTE_LICEN_CD, ' ') <> ' ') ";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UP_QryOpen_012 \n";
			G_INFO += "설명 : 폐암 검진정보 로딩 \n";
			G_INFO += "\n\n";
