...
select a.*, epo_optn_pr * epo_optn_cnt optn_pr 
  from et_pack_optn a 
 where a.epo_exam_cd = :Exam_Cd
   and a.epo_pack_sq = :pack_sq 

		*/

		sql = " SELECT A.*, EPO_OPTN_PR * EPO_OPTN_CNT OPTN_PR";
		sql += " FROM ET_PACK_OPTN A";
		sql += " WHERE A.EPO_EXAM_CD = '" + EXAM_CD + "'";
		sql += " AND A.EPO_PACK_SQ = '" + PACK_SQ + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uPack_Each_AlvPackSelectItem_003 \n";
			G_INFO += "설명 : 묶음 옵션정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='OPTN_PR' rs:number='19' rs:nullable='true'>
			<s:datatype dt:type='number' rs:dbtype='varnumeric' dt:maxLength='38' rs:scale='255' rs:precision='38'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='20' rs:rowid='true' rs:basetable='ET_PACK_OPTN' rs:basecolumn='ROWID'
