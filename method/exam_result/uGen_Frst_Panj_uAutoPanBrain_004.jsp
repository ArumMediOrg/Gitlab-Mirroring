...
<%
		...
		String VRSLT_VAL = htMethod.get("VRSLT_VAL");

		//
		if(ITEM_CD == null) { ITEM_CD = ""; }
		if(SEX_KD == null) { SEX_KD = ""; }
		if(VRSLT_VAL == null) { VRSLT_VAL = ""; }
 ...
 SELECT * FROM CT_DANR_ITEM
  WHERE CDI_ITEM_CD = :ITEM_CD
    AND CDI_SEX_KD  = :SEX_KD
    AND CDI_DANR_KD = '1'
    And (TO_NUMBER(CDI_RSLT_LOW)  <= :vRSLT_VAL and
         TO_NUMBER(CDI_RSLT_HIGH) >= :vRSLT_VAL )
		*/

		sql = " SELECT *";
		sql += " FROM CT_DANR_ITEM";
		sql += " WHERE CDI_ITEM_CD = '" + ITEM_CD + "'";
		sql += " AND CDI_SEX_KD = '" + SEX_KD + "'";
		sql += " AND CDI_DANR_KD = '1'";
		sql += " AND (TO_NUMBER(CDI_RSLT_LOW) <= '" + VRSLT_VAL + "' AND TO_NUMBER(CDI_RSLT_HIGH) >= '" + VRSLT_VAL + "')";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uGen_Frst_Panj_uAutoPanBrain_004 \n";
			G_INFO += "설명 : 뇌심혈관판정(위험도평가CT_DANR_ITEM 조회 1) \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " ITEM_CD : " + ITEM_CD + " \n";
			G_INFO += " SEX_KD : " + SEX_KD + " \n";
			G_INFO += " VRSLT_VAL : " + VRSLT_VAL + " \n";
			G_INFO += "\n\n";
