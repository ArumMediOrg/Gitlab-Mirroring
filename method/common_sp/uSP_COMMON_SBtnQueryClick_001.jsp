...
<%
		...
		sql += " WHERE 0 = 0";
		if(!CMBKIND.equals("00000")) {
			sql += " AND CSC_LARGE = '" + CMBKIND + "'";
		}
		if(!EDTSEARCH.equals("")) {
			sql += " AND (CSC_FULL_NM LIKE '%" + EDTSEARCH + "%' Or CSC_SMALL LIKE '%" + EDTSEARCH + "%') ";
		}
		if(CHKBOXUSE.equals("1")) {
			sql += " AND CSC_USE_YN = 'Y'";
		}
		sql += " ORDER BY CSC_LARGE, CSC_SMALL, CSC_LEVEL ";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_COMMON_SBtnQueryClick_001 \n";
			G_INFO += "설명 : 특검코드 로딩 \n";
			G_INFO += "\n\n";
