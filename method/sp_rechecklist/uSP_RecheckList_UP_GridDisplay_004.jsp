...
SELECT A.EPI_ITEM_CD, B.IIM_KNME_NM 
  FROM ET_PACK_ITEM A 
       INNER JOIN IT_ITEM B ON B.IIM_ITEM_CD = A.EPI_ITEM_CD       
 WHERE A.EPI_EXAM_CD = :EXAM_CD
   AND A.EPI_PACK_SQ = :PACK_SQ
		*/

		sql = " SELECT A.EPI_ITEM_CD, B.IIM_KNME_NM";
		sql += " FROM ET_PACK_ITEM A INNER JOIN IT_ITEM B";
		sql += " ON B.IIM_ITEM_CD = A.EPI_ITEM_CD";
		sql += " WHERE A.EPI_EXAM_CD = '" + EXAM_CD + "'";
		sql += " AND A.EPI_PACK_SQ = '" + PACK_SQ + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_RecheckList_UP_GridDisplay_004 \n";
			G_INFO += "설명 : 특검용 개별묶음 검사항목 조회 \n";
			G_INFO += "\n\n";
