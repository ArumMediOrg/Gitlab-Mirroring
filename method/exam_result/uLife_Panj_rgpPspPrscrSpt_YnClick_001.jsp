...
SELECT
    NVL(MAX(HRunDay),0) HRunDay ,NVL(MAX(HRunHm),0) HRunHm ,NVL(MAX(HRunMm),0) HRunMm
   ,NVL(MAX(BRunDay),0) BRunDay ,NVL(MAX(BRunHm),0) BRunHm ,NVL(MAX(BRunMm),0) BRunMm
   ,NVL(MAX(MusDay), 0) MusDay
  FROM (
		...
		*/
		...
		sql = " SELECT NVL(MAX(HRUNDAY),0) HRUNDAY ,NVL(MAX(HRUNHM),0) HRUNHM ,NVL(MAX(HRUNMM),0) HRUNMM ,NVL(MAX(BRUNDAY),0) BRUNDAY ,NVL(MAX(BRUNHM),0) BRUNHM ,NVL(MAX(BRUNMM),0) BRUNMM ,NVL(MAX(MUSDAY), 0) MUSDAY";
		sql += " FROM (";
		sql += "	SELECT CASE WHEN QQN_QSTN_CD = '912' THEN NVL(QQN_RESULT, '0') ELSE '0' END HRUNDAY , CASE WHEN QQN_QSTN_CD = '914' THEN NVL(QQN_RESULT, '0') ELSE '0' END HRUNHM";
		...
		sql += ") DD";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uLife_Panj_rgpPspPrscrSpt_YnClick_001 \n";
			G_INFO += "설명 : 공통문진표 (운동관련) \n";
			G_INFO += "\n\n";
