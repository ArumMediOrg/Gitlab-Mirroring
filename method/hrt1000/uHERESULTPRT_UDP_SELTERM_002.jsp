...
select MIN(PSP_EXAM_DT) MINDT,MAX(PSP_EXAM_DT) MAXDT
  from PT_SED_PANJUNG A left outer join ET_EXAM_ACPT b on B.EEA_EXAM_DT = A.PSP_EXAM_DT AND B.EEA_EXAM_SQ = A.PSP_EXAM_SQ
...
if :RzCmb_place2_1 <> ' then
  AND EEA_PLCE_CD = RzCmb_place2_2
		...
		*/
		...
		sql = " SELECT MIN(PSP_EXAM_DT) MINDT,MAX(PSP_EXAM_DT) MAXDT";
		sql += " FROM PT_SED_PANJUNG A LEFT OUTER JOIN ET_EXAM_ACPT B";
		sql += " ON B.EEA_EXAM_DT = A.PSP_EXAM_DT";
		sql += " AND B.EEA_EXAM_SQ = A.PSP_EXAM_SQ";
		...
		if(!RZCMB_PLACE2_1.equals("")) {
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uHERESULTPRT_UDP_SELTERM_002 \n";
			G_INFO += "설명 : 2차판정 정보 로딩 \n";
			G_INFO += "\n\n";
