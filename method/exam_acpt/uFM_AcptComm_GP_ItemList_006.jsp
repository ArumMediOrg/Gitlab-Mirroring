...
SELECT A.IIM_EXGN_CD, A.IIM_SORT_SQ, 
       ...
       '00000000000' GBHC_KD, 
       CASE WHEN :SBSG_KD = 'Y' THEN B.IIP_SBSG_PR 
            ELSE CASE WHEN :CALC_CD = '4' THEN B.IIP_SBSG_PR ELSE B.IIP_TGSG_PR END END AS ITEM_PR, B.IIP_GASN_PR HOLI_PR 
 ...
 WHERE A.IIM_ITEM_CD = :ITEM_CD
   ...
   AND UPPER(B.IIP_USE_YN) = 'Y'
		*/
		...
		sql = " SELECT A.IIM_EXGN_CD, A.IIM_SORT_SQ, LTRIM(RTRIM(A.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) ITEM_NM, '00000000000' GBHC_KD, CASE WHEN '" + SBSG_KD + "' = 'Y' THEN B.IIP_SBSG_PR ELSE CASE WHEN '" + CALC_CD + "' = '4' THEN B.IIP_SBSG_PR ELSE B.IIP_TGSG_PR END END AS ITEM_PR, B.IIP_GASN_PR HOLI_PR";
		...
		sql += " AND UPPER(B.IIP_USE_YN) = 'Y'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_006 \n";
			G_INFO += "설명 : 일특 진찰 항목 및 수가 재산정 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " SBSG_KD : " + SBSG_KD + " \n";
