...
Select                                                                     
B.*, A.ERI_RSLT_VL,  A.ERI_RSLT_LOW, A.ERI_RSLT_HIGH                       
From ET_RSLT_ITEM A                                                        
Left Outer Join HEALTH_DIETITIAN_PANJUNG B On B.HDP_EXAM_DT = A.ERI_EXAM_DT
                                          And B.HDP_EXAM_SQ = A.ERI_EXAM_SQ
Where A.ERI_EXAM_DT =:EXAM_DT                                   
 And A.ERI_EXAM_SQ = :EXAM_SQ                                    
  And A.ERI_ITEM_CD = 'A0009'                                           
   And A.ERI_CNCL_YN = 'N'                                               


		*/

		sql = " SELECT B.*, A.ERI_RSLT_VL, A.ERI_RSLT_LOW, A.ERI_RSLT_HIGH";
		sql += " FROM ET_RSLT_ITEM A LEFT OUTER JOIN HEALTH_DIETITIAN_PANJUNG B";
		sql += " ON B.HDP_EXAM_DT = A.ERI_EXAM_DT";
		sql += " AND B.HDP_EXAM_SQ = A.ERI_EXAM_SQ";
		sql += " WHERE A.ERI_EXAM_DT ='" + EXAM_DT + "'";
		sql += " AND A.ERI_EXAM_SQ = '" + EXAM_SQ + "'";
		sql += " AND A.ERI_ITEM_CD = 'A0009'";
		sql += " AND A.ERI_CNCL_YN = 'N'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : upGetDietitian_001 \n";
			G_INFO += "설명 : BMI 수치 및 참고치 조회(고대) \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='HDP_EXAM_DT' rs:number='1' rs:nullable='true' rs:writeunknown='true' rs:basetable='HEALTH_DIETITIAN_PANJUNG'
			 rs:basecolumn='HDP_EXAM_DT'>
			<s:datatype dt:type='dateTime' rs:dbtype='timestamp' dt:maxLength='16' rs:scale='0' rs:precision='19' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='HDP_EXAM_SQ' rs:number='2' rs:nullable='true' rs:writeunknown='true' rs:basetable='HEALTH_DIETITIAN_PANJUNG'
		...
		<s:AttributeType name='ERI_RSLT_VL' rs:number='21' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_RSLT_ITEM'
			 rs:basecolumn='ERI_RSLT_VL'>
			<s:datatype dt:type='string' dt:maxLength='50'/>
		</s:AttributeType>
		<s:AttributeType name='ERI_RSLT_LOW' rs:number='22' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_RSLT_ITEM'
			 rs:basecolumn='ERI_RSLT_LOW'>
			<s:datatype dt:type='string' dt:maxLength='20'/>
		</s:AttributeType>
		<s:AttributeType name='ERI_RSLT_HIGH' rs:number='23' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_RSLT_ITEM'
			 rs:basecolumn='ERI_RSLT_HIGH'>
			<s:datatype dt:type='string' dt:maxLength='20'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='24' rs:rowid='true' rs:basetable='ET_RSLT_ITEM' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c24' rs:name='ROWID' rs:number='25' rs:rowid='true' rs:basetable='HEALTH_DIETITIAN_PANJUNG'
