...
SELECT * FROM ET_CLDR_MEMO
 WHERE ECM_MEMO_DT LIKE :EXAM_YR || '%'
   AND ECM_MEMO_SQ  > 0
   AND (ECM_MEMO_LT LIKE '%' || :MEMO_LT || '%'
    OR   ECM_MEMO_TX LIKE '%' || :MEMO_LT || '%')
 ORDER BY ECM_MEMO_DT DESC
		*/

		sql = " SELECT *";
		sql += " FROM ET_CLDR_MEMO";
		sql += " WHERE ECM_MEMO_DT LIKE '" + EXAM_YR + "' || '%'";
		sql += " AND ECM_MEMO_SQ > 0";
		sql += " AND (ECM_MEMO_LT LIKE '%' || '" + MEMO_LT + "' || '%'";
		sql += " OR ECM_MEMO_TX LIKE '%' || '" + MEMO_LT + "' || '%')";
		sql += " ORDER BY ECM_MEMO_DT DESC";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uMemo_BtnSearchClick_001 \n";
			G_INFO += "설명 : 선택연도 메모 조회 \n";
			G_INFO += "\n\n";
