...
SELECT B.EEA_CHART_NO, 
       ...
       A.* 
  FROM ST_ADVICE A INNER JOIN ET_EXAM_ACPT B ON A.SAE_EXAM_DT = B.EEA_EXAM_DT AND A.SAE_EXAM_SQ = B.EEA_EXAM_SQ 
 WHERE A.SAE_CUST_NO = :CUST_NO
		*/

		sql = " SELECT B.EEA_CHART_NO, GETCOMMON_LVCDTONM('0910', B.EEA_SPCL_CD, '5', 2) EEA_SPCL_NM, GETCOMMON_LVCDTONM('0427', A.SAE_ADVI_CD, '5', 1) SAE_ADVI_NM, F_USER_FIND(A.SAE_NURSE_ID) SAE_NURSE_NM, F_USER_FIND(A.SAE_DOCT_ID) SAE_DOCT_NM, A.*";
		...
		sql += " WHERE A.SAE_CUST_NO = '" + CUST_NO + "'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_PostManagement_CustList_APPLY_UP_AdviDisp_001 \n";
			G_INFO += "설명 : 상담 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='EEA_SPCL_NM' rs:number='2' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='SAE_ADVI_NM' rs:number='3' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
