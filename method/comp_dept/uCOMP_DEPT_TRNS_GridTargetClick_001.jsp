...
<%
		...
		String STEMP_T = htMethod.get("STEMP_T");

		//
		if(STEMP_T == null) { STEMP_T = ""; }
...
SELECT
CSM_MATT_CD, CSM_MATT_NM, CSM_COMM_CD, CSM_PERIOD_TM,
dbo.GetSP_COMMON_cdToNm('HM99B',CSM_PERIOD_TM) CSM_PERIOD_NM
FROM CT_SP_MATTER
 WHERE CSM_MATT_CD IN (:sTemp_T)

		*/

		sql = " SELECT CSM_MATT_CD, CSM_MATT_NM, CSM_COMM_CD, CSM_PERIOD_TM, GETSP_COMMON_CDTONM('HM99B',CSM_PERIOD_TM) CSM_PERIOD_NM";
		sql += " FROM CT_SP_MATTER";
		sql += " WHERE CSM_MATT_CD IN (" + STEMP_T + ")";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCOMP_DEPT_TRNS_GridTargetClick_001 \n";
			G_INFO += "설명 : 사업장부서관리 이월 취급물질 정보 조회 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " STEMP_T : " + STEMP_T + " \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='CSM_COMM_CD' rs:number='3' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_MATTER'
			 rs:basecolumn='CSM_COMM_CD'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='CSM_PERIOD_TM' rs:number='4' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_MATTER'
			 rs:basecolumn='CSM_PERIOD_TM'>
			<s:datatype dt:type='string' dt:maxLength='1'/>
		</s:AttributeType>
		<s:AttributeType name='CSM_PERIOD_NM' rs:number='5' rs:nullable='true'>
