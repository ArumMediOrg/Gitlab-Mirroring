...
select a.*
from et_exam_acpt b join et_rslt_item a
  on a.eri_exam_dt = b.eea_exam_dt
 and a.eri_exam_sq = b.eea_exam_sq
...
order by a.eri_exam_dt, a.eri_exam_sq, a.eri_sort_sq, a.eri_item_cd

		*/

		sql = " SELECT A.*";
		sql += " FROM ET_EXAM_ACPT B JOIN ET_RSLT_ITEM A";
		sql += " ON A.ERI_EXAM_DT = B.EEA_EXAM_DT";
		sql += " AND A.ERI_EXAM_SQ = B.EEA_EXAM_SQ";
		...
		sql += " ORDER BY A.ERI_EXAM_DT, A.ERI_EXAM_SQ, A.ERI_SORT_SQ, A.ERI_ITEM_CD";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uBusinessTrip_UP_btnDnLoadClick_004 \n";
			G_INFO += "설명 : 검사결과정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ROWID' rs:number='70' rs:rowid='true' rs:basetable='ET_EXAM_ACPT' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c70' rs:name='ROWID' rs:number='71' rs:rowid='true' rs:basetable='ET_RSLT_ITEM' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
...
<%
		...
		while(cRsList.next()) {
			...
			String c70_T = cRsList.getString("c70");
%>
			<z:row
<%
