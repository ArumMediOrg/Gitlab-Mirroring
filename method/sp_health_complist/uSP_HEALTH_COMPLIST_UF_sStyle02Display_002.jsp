...
SELECT A.EEA_COMP_CD, A.EEA_EXAM_DT, A.EEA_CUST_NO, A.EEA_CHART_NO, 
       A.EEA_EXAM_SQ, A.EEA_SPCL_LT, A.EEA_HTSB_CD, A.EEA_SPCL_CD, D.SPG_PANJ_CD
  FROM ET_EXAM_ACPT A
       LEFT OUTER JOIN IT_COMPANY B  ON A.EEA_COMP_CD = B.ICY_COMP_CD
              INNER JOIN ST_PANJUNG D ON A.EEA_EXAM_DT = D.SPG_EXAM_DT AND A.EEA_EXAM_SQ = D.SPG_EXAM_SQ
 WHERE 0 = 0 
...
:PARAM_TX
		...
		*/
		...
		sql = " SELECT A.EEA_COMP_CD, A.EEA_EXAM_DT, A.EEA_CUST_NO, A.EEA_CHART_NO, A.EEA_EXAM_SQ, A.EEA_SPCL_LT, A.EEA_HTSB_CD, A.EEA_SPCL_CD, D.SPG_PANJ_CD";
		...
		sql += " AND (A.EEA_EXAM_CD = '41001' OR A.EEA_EXAM_CD = '42001'";
		sql += " OR A.EEA_SPSB_YN = 'Y')";
		sql += PARAM_TX;

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_COMPLIST_UF_sStyle02Display_002 \n";
			G_INFO += "설명 : 02-특수검진실시사업장 판정조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='SPG_PANJ_CD' rs:number='9' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PANJUNG'
		...
		<s:AttributeType name='c11' rs:name='ROWID' rs:number='12' rs:rowid='true' rs:basetable='ST_PANJUNG' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
