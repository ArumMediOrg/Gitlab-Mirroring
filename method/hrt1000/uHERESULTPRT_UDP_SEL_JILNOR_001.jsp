...
<%
		...
		sql += " FROM ( SELECT CASE WHEN GB='1' THEN CNT END CNT, CASE WHEN GB='1' THEN AA END AA, CASE WHEN GB='1' THEN BB END AB, CASE WHEN GB='2' THEN CNT END CNT1, CASE WHEN GB='2' THEN AA END BA, CASE WHEN GB='2' THEN BB END BB";
		sql += " FROM ( SELECT MAX(GB) GB,MAX(CNT) CNT, MAX(AA) AA,MAX(BB) BB";
		sql += " FROM ( SELECT '1' GB, 0 CNT, 0 AA, 0 BB";
		sql += " FROM DUAL";
		sql += " UNION";
		sql += " ALL SELECT '1' GB, COUNT(*) CNT,CASE WHEN EEA_SEX_CD='1' THEN COUNT(*) END AA, CASE WHEN EEA_SEX_CD='2' THEN COUNT(*) END AB";
		sql += " FROM ( SELECT PLP_EXAM_DT,PLP_EXAM_SQ,PLP_JLPN_PNCD,EEA_SEX_CD";
		...
		sql += " AND SUBSTR(EEA_HTSB_CD, 1, 2) IN ('11', '12')";
		...
		sql += " GROUP BY PLP_EXAM_DT,PLP_EXAM_SQ,PLP_JLPN_PNCD,EEA_SEX_CD ) FF";
		sql += " GROUP BY EEA_SEX_CD ) DD";
		sql += " UNION";
		sql += " ALL SELECT MAX(GB) GB,MAX(CNT) CNT, MAX(AA) AA,MAX(BB) BB";
		sql += " FROM ( SELECT '2' GB, 0 CNT, 0 AA, 0 BB";
		sql += " FROM DUAL";
		sql += " UNION";
		sql += " ALL SELECT '2' GB, COUNT(*) CNT,CASE WHEN EEA_SEX_CD='1' THEN COUNT(*) END AA, CASE WHEN EEA_SEX_CD='2' THEN COUNT(*) END AB";
		sql += " FROM ( SELECT PLP_EXAM_DT,PLP_EXAM_SQ,PLP_JLPN_PNCD,EEA_SEX_CD";
		sql += " FROM PT_LABOR_PANJUNG A INNER JOIN ET_EXAM_ACPT B";
		sql += " ON A.PLP_EXAM_DT=B.EEA_EXAM_DT";
		sql += " AND A.PLP_EXAM_SQ=B.EEA_EXAM_SQ";
		...
		sql += " AND NVL(EEA_DIVI_CD,' ') <>  '1' ";
		sql += " GROUP BY PLP_EXAM_DT,PLP_EXAM_SQ,PLP_JLPN_PNCD,EEA_SEX_CD ) FF";
		sql += " GROUP BY EEA_SEX_CD ) DD ) T1) T2";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uHERESULTPRT_UDP_SEL_JILNOR_001 \n";
			G_INFO += "설명 : 질병정보 로딩 \n";
			G_INFO += "\n\n";
