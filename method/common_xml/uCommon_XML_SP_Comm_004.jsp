...
SELECT GETCOMMON_LVCDTONM('0427',A.SAE_ADVI_CD,'5',1) SAE_ADVI_NM, 
       F_USER_FIND(A.SAE_NURSE_ID) SAE_NURSE_NM, 
       F_USER_FIND(A.SAE_DOCT_ID) SAE_DOCT_NM, 
       A.* 
  FROM ST_ADVICE A 
 WHERE A.SAE_EXAM_DT = :EXAM_DT
   AND A.SAE_EXAM_SQ = :EXAM_SQ
   
IF (:NEXT_DT <> '') THEN 
   AND A.SAE_NEXT_DT = :NEXT_DT
   
IF (:ADVI_DT <> '') THEN 
   AND A.SAE_ADVI_DT = :ADVI_DT

IF (:ADVI_CD <> '0') THEN 
   AND A.SAE_ADVI_CD = :ADVI_CD
		*/

		sql = " SELECT GETCOMMON_LVCDTONM('0427',A.SAE_ADVI_CD,'5',1) SAE_ADVI_NM, F_USER_FIND(A.SAE_NURSE_ID) SAE_NURSE_NM, F_USER_FIND(A.SAE_DOCT_ID) SAE_DOCT_NM, A.*";
		sql += " FROM ST_ADVICE A";
		sql += " WHERE A.SAE_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND A.SAE_EXAM_SQ = '" + EXAM_SQ + "'";

		if(! NEXT_DT.equals("")) {
			sql += " AND A.SAE_NEXT_DT = '" + NEXT_DT + "'";
		}
		if(! ADVI_DT.equals("")) {
			sql += " AND A.SAE_ADVI_DT = '" + ADVI_DT + "'";
		}
		if(! ADVI_CD.equals("0")) {
			sql += " AND A.SAE_ADVI_CD = '" + ADVI_CD + "'";
		}

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCommon_XML_SP_Comm_004 \n";
			G_INFO += "설명 : 상담내역 조회 \n";
			G_INFO += "\n\n";
