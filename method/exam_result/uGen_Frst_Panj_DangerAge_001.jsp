...
<%
		...
		String MS_AREL = htMethod.get("MS_AREL");
		String SEX_KD = htMethod.get("SEX_KD");

		//
		if(MS_AREL == null) { MS_AREL = ""; }
...
SELECT * FROM (
SELECT CDR_DANR_AGE, ABS(CDR_DANR_VL - :Ms_Arel) as DANGER,CDR_DANR_VL
  FROM CT_DANGER
 WHERE CDR_DANR_KD = '2'
   AND CDR_SEX_KD = :SEX_KD
 Order by DANGER, CDR_DANR_AGE DESC
 ) WHERE ROWNUM = 1
		*/

		sql = " SELECT *";
		sql += " FROM (";
		sql += "	SELECT CDR_DANR_AGE, ABS(CDR_DANR_VL - '" + MS_AREL + "') AS DANGER,CDR_DANR_VL";
		sql += " 	FROM CT_DANGER";
		...
		sql += " WHERE ROWNUM = 1";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uGen_Frst_Panj_DangerAge_001 \n";
			G_INFO += "설명 : 뇌심혈관 나이 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " MS_AREL : " + MS_AREL + " \n";
