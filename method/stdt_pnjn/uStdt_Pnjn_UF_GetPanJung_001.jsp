...
<%
		...
		String AGE_FT = htMethod.get("AGE_FT");
...
SELECT *
  FROM IT_ITEM_VLDT 
 WHERE IIV_APLY_DT = GETITEM_VLDT_APPLYDATE(:ITEM_CD, :EXAM_DT, 'ITEM_CD', IIV_EXAM_CD) 
   AND IIV_ITEM_CD = :ITEM_CD
   AND IIV_SEX_CD  = :SEX_CD
   AND IIV_AGE_FR <= :AGE_FT 
   AND IIV_AGE_TO >= :AGE_FT
   AND IIV_EXAM_CD = '6'   

IF (:PANJ_CD <> '') THEN
   AND IIV_PANJ_CD = :PANJ_CD 

 ORDER BY IIV_APLY_DT DESC, IIV_PANJ_CD DESC
		*/

		sql = " SELECT *";
		sql += " FROM IT_ITEM_VLDT";
		sql += " WHERE IIV_APLY_DT = GETITEM_VLDT_APPLYDATE('" + ITEM_CD + "', '" + EXAM_DT + "', 'ITEM_CD', IIV_EXAM_CD)";
		sql += " AND IIV_ITEM_CD = '" + ITEM_CD + "'";
		sql += " AND IIV_SEX_CD = '" + SEX_CD + "'";
		sql += " AND IIV_AGE_FR <= '" + AGE_FT + "'";
		sql += " AND IIV_AGE_TO >= '" + AGE_FT + "'";
		sql += " AND IIV_EXAM_CD = '6'";

		if(! PANJ_CD.equals("")) {
			sql += " AND IIV_PANJ_CD = '" + PANJ_CD + "'";
		}

		sql += " ORDER BY IIV_APLY_DT DESC, IIV_PANJ_CD DESC";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStdt_Pnjn_UF_GetPanJung_001 \n";
			G_INFO += "설명 : 학생 검진 검사별 참고치 조회 \n";
			G_INFO += "\n\n";
