...
<%
		...
		String MS_MATT = htMethod.get("MS_MATT");

		//
		if(MS_MATT == null) { MS_MATT = ""; }
...
SELECT A.CSM_MATT_CD, A.CSM_MATT_NM, A.CSM_INJA_CD,
       GetSP_COMMON_cdToNm('HM06B',A.CSM_INJA_CD) CSM_INJA_NM,
       B.CSC_ETC1_NM, B.CSC_ETC1_CD, B.CSC_ETC2_NM, B.CSC_ETC2_CD
FROM CT_SP_MATTER A
 LEFT OUTER JOIN CT_SP_COMMON B ON A.CSM_COMM_CD = B.CSC_SMALL
                               AND B.CSC_LARGE = 'HM99A'
Where A.CSM_MATT_CD In (:Ms_Matt)

		*/

		sql = " SELECT A.CSM_MATT_CD, A.CSM_MATT_NM, A.CSM_INJA_CD, GETSP_COMMON_CDTONM('HM06B',A.CSM_INJA_CD) CSM_INJA_NM, B.CSC_ETC1_NM, B.CSC_ETC1_CD, B.CSC_ETC2_NM, B.CSC_ETC2_CD";
		sql += " FROM CT_SP_MATTER A LEFT OUTER JOIN CT_SP_COMMON B";
		sql += " ON A.CSM_COMM_CD = B.CSC_SMALL";
		sql += " AND B.CSC_LARGE = 'HM99A'";
		sql += " WHERE A.CSM_MATT_CD IN (" + MS_MATT + ")";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_PRINT_UP_MATTDisplay_001 \n";
			G_INFO += "설명 : 물질코드정보 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " MS_MATT : " + MS_MATT + " \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='CSM_INJA_CD' rs:number='3' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_MATTER'
			 rs:basecolumn='CSM_INJA_CD'>
			<s:datatype dt:type='string' dt:maxLength='2'/>
		</s:AttributeType>
		<s:AttributeType name='CSM_INJA_NM' rs:number='4' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='CSC_ETC1_NM' rs:number='5' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC1_NM'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='CSC_ETC1_CD' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC1_CD'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='CSC_ETC2_NM' rs:number='7' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC2_NM'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='CSC_ETC2_CD' rs:number='8' rs:nullable='true' rs:writeunknown='true' rs:basetable='CT_SP_COMMON'
			 rs:basecolumn='CSC_ETC2_CD'>
			<s:datatype dt:type='string' dt:maxLength='200'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='9' rs:rowid='true' rs:basetable='CT_SP_MATTER' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c9' rs:name='ROWID' rs:number='10' rs:rowid='true' rs:basetable='CT_SP_COMMON' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
