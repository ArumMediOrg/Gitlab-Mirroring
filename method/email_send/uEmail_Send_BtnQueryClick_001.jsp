...
<%
		...
		String ENCR_KD = htMethod.get("ENCR_KD");
		...
		String MAIL_MOBL = htMethod.get("MAIL_MOBL");
		String MAIL_CK = htMethod.get("MAIL_CK");

		//

		if(ENCR_KD == null) { ENCR_KD = ""; }
		...
		if(MAIL_MOBL == null) { MAIL_MOBL = ""; }
		if(MAIL_CK == null) { MAIL_CK = ""; }
		...
		else if (ENCR_KD.equals("2")){
			sql = " select ECHELONDB.DBO.ECL_DECRYPT(B.ICR_PID_EN) AS EEA_PSNL_ID, ";
		} else {
		...
		sql += " A.EEA_EXAM_CD, A.EEA_HTSB_CD,  A.EEA_SPSB_CD,  A.EEA_HDMD_YN,  A.EEA_CHART_NO, A.EEA_EMAIL_AR, ";
		sql += " f_tel_format(A.EEA_MOBL_NO) EEA_MOBL_NO FROM ET_EXAM_ACPT A ";
		...
		if (!SEND_CD.equals("00")){
			if (SEND_CD.equals("99")){
		...
		if (MAIL_MOBL.equals("1")){
			if (MAIL_CK.equals("1")){
				sql += " AND NVL(A.EEA_EMAIL_AR, ' ') NOT IN (' ', '@') ";
			}
			else {
				sql += " AND NVL(A.EEA_MOBL_NO, ' ') NOT IN (' ', '-') ";
			}
		...
		sql += " ORDER BY A.EEA_EXAM_DT, A.EEA_EXAM_SQ ";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uEmail_Send_BtnQueryClick_001 \n";
			G_INFO += "설명 : 발송대상 목록 조회 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " ENCR_KD : " + ENCR_KD + " \n";
			...
			G_INFO += " MAIL_MOBL : " + MAIL_MOBL + " \n";
			G_INFO += " MAIL_CK : " + MAIL_CK + " \n";
			G_INFO += "\n\n";
