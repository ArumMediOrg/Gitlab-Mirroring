...
if :SEARCH_TEXT <> '' then
begin
   AND (UPPER(IIM_ITEM_CD)  LIKE ''%''||:SEARCH_TEXT||''%''
    OR  UPPER(IIM_SBCD_CD)  LIKE ''%''||:SEARCH_TEXT||''%''
    OR  UPPER(IIM_OSCD_CD)  LIKE ''%''||:SEARCH_TEXT||''%''
    OR  UPPER(IIM_NITEM_CD) LIKE ''%''||:SEARCH_TEXT||''%''
    OR  UPPER(IIM_KNME_NM)  LIKE ''%''||:SEARCH_TEXT||''%''
    OR  UPPER(IIM_ENME_NM)  LIKE ''%''||:SEARCH_TEXT||''%''
    OR  UPPER(IIM_SNME_NM)  LIKE ''%''||:SEARCH_TEXT||''%'')
end;
ORDER BY IIM_KNME_NM
		...
		*/
		...
		sql += " ORDER BY IIM_KNME_NM";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uItem_Rltn_upGetItem_001 \n";
