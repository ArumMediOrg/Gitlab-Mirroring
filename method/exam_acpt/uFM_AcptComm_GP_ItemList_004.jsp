...
SELECT CSM_MATT_CD ITEM_CD, CSM_MATT_NM ITEM_NM, 
       '00000000000' GBHC_KD, 0 ITEM_PR, 0 HOLI_PR 
  FROM CT_SP_MATTER 
 WHERE CSM_MATT_CD IN (:MATT_CD) 
   AND UPPER(CSM_USE_YN) = 'Y'
 ORDER BY CSM_COMM_CD
		*/

		sql = " SELECT CSM_MATT_CD ITEM_CD, CSM_MATT_NM ITEM_NM, '00000000000' GBHC_KD, 0 ITEM_PR, 0 HOLI_PR";
		sql += " FROM CT_SP_MATTER";
		sql += " WHERE CSM_MATT_CD IN ('" + MATT_CD + "')";
		sql += " AND UPPER(CSM_USE_YN) = 'Y'";
		sql += " ORDER BY CSM_COMM_CD";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_004 \n";
			G_INFO += "설명 : 유해물질 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='ITEM_CD' rs:number='1' rs:writeunknown='true' rs:basetable='CT_SP_MATTER' rs:basecolumn='CSM_MATT_CD'>
			<s:datatype dt:type='string' dt:maxLength='10' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='ITEM_NM' rs:number='2' rs:writeunknown='true' rs:basetable='CT_SP_MATTER' rs:basecolumn='CSM_MATT_NM'>
