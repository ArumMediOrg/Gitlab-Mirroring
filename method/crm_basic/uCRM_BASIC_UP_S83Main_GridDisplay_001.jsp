...
<%
		...
		sql += " select DATE_LST from ";
		sql += " (SELECT ";
		sql += " 		TO_CHAR(TO_DATE('" + SRCH_DTS + "') + LEVEL - 1, 'YYYY-MM-DD') DATE_LST ";
		sql += " FROM    DUAL ";
		sql += " CONNECT BY LEVEL <= (TO_DATE('" + SRCH_DTE + "')+1) - TO_DATE('" + SRCH_DTS + "') ";
		sql += " ) a left outer join CR_DOCTOR_PLAN b ON ";
		sql += " a.DATE_LST = b.CDP_PLAN_DT ";
		sql += " where ";
		sql += " 1 = 1 ";
		...
		sql += SSQLADD;
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCRM_BASIC_UP_S83Main_GridDisplay_001 \n";
			G_INFO += "설명 : 일자별상담의사 일정리스트 \n";
			G_INFO += "\n\n";
