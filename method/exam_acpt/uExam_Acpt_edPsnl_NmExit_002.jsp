...
SELECT ICR_CUST_NO, ICR_PENL_NM,
       ICR_PENL_NM || ' : ' || F_TEL_FORMAT(ICR_MOBL_NO) || ' ' || ICR_ZIP_AR || ' ' || ICR_ROAD_AR SAME_NM
  FROM IT_CUSTOMER
 WHERE NVL(UPPER(ICR_USE_YN), 'Y') <> 'N';

if (:ENCRT_KD = '1') then
   AND ICR_PID_EN  = :PENL_ID
else if (:ENCRT_KD = '2') then
   AND ICR_PID_EN  = ECHELONDB.ECL_ENCRYPT(:PENL_ID)
else
   AND ICR_PENL_ID = :PENL_ID


 AND ICR_PENL_NM <> :PENL_NM;

		*/

		sql = " SELECT ICR_CUST_NO, ICR_PENL_NM, ICR_PENL_NM || ' : ' || F_TEL_FORMAT(ICR_MOBL_NO) || ' ' || ICR_ZIP_AR || ' ' || ICR_ROAD_AR SAME_NM";
		...
		if(ENCRT_KD.equals("1")) {
			sql += " AND ICR_PID_EN = '" + PENL_ID + "'";
   			...
   			sql += " AND ICR_PID_EN  = ECHELONDB.ECL_ENCRYPT('" + PENL_ID + "')";
		} else {
		...
		sql += " AND ICR_PENL_NM <> '" + PENL_NM + "'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uExam_Acpt_edPsnl_NmExit_002 \n";
			G_INFO += "설명 : 동번이명 여부 확인 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='SAME_NM' rs:number='3' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='4' rs:rowid='true' rs:basetable='IT_CUSTOMER' rs:basecolumn='ROWID'
