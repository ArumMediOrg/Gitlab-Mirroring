...
<%
		...
		sql = " SELECT A.* FROM ET_CANCER_STMC A ";

		if(CMBPLACEFLG.equals("1")){
			sql += " INNER JOIN ET_EXAM_ACPT B ON B.EEA_EXAM_DT = A.ECS_EXAM_DT ";
		...
		sql += " AND (NVL(A.ECS_CKUP_LICEN_CD, ' ') <> ' ' OR NVL(A.ECS_DECI_LICEN_CD, ' ') <> ' ' OR NVL(A.ECS_PATH_LICEN_CD, ' ') <> ' ') ";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UP_QryOpen_Chk_001 \n";
			G_INFO += "설명 : 위암 검진정보 로딩(검사/진단의) \n";
			G_INFO += "\n\n";
