...
<%
		...
		String S0243 = htMethod.get("S0243");
		String S0242 = htMethod.get("S0242");
		String S0240 = htMethod.get("S0240");
		String S0241 = htMethod.get("S0241");
...
WHERE 0=0
  ...
  ||:sSQL_ADD
		*/
		...
		sql += " ON E.IIP_APLY_DT = GETITEM_PRICE_APPLYDATE(B.ERI_ITEM_CD, A.EEA_EXAM_DT , 'ITEM_CD')";
		...
		sql += " AND B.ERI_ITEM_CD = F.SIC_ITEM_CD LEFT OUTER JOIN ST_BILLHN_PR G";
		sql += " ON A.EEA_EXAM_DT = G.SBP_EXAM_DT";
		sql += " AND A.EEA_EXAM_SQ = G.SBP_EXAM_SQ";
		sql += " AND G.SBP_CHUNG_KD = '" + CHUNG_KD + "' LEFT OUTER JOIN ST_BASE H";
		sql += " ON H.SBE_EXAM_DT = A.EEA_EXAM_DT";
		sql += " AND H.SBE_EXAM_SQ = A.EEA_EXAM_SQ LEFT OUTER JOIN IT_COMPANY I";
		sql += " ON A.EEA_COMP_CD=I.ICY_COMP_CD LEFT OUTER JOIN (SELECT SIR_EXAM_DT2,SIR_EXAM_SQ2,SIR_EXAM_DT,SIR_EXAM_SQ";
		sql += " FROM ST_ITEM_RECHECK";
		sql += " WHERE NVL(SIR_EXAM_DT2, ' ') <> ' '";
		sql += " GROUP BY SIR_EXAM_DT,SIR_EXAM_SQ, SIR_EXAM_DT2,SIR_EXAM_SQ2) K";
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : Uspchungfile2020_UDP_RESULT_003 \n";
			G_INFO += "설명 : 특검청구 검진결과 조회 1차(메인) \n";
			G_INFO += "\n\n";
