...
SELECT LTRIM(RTRIM(B.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(B.IIM_KNME_NM)) ITEM_NM, 
       '00000000000' GBHC_KD, 
       A.ECI_CNRT_PR ITEM_PR, 0 HOLI_PR 
  FROM ET_COMP_CNRT_PROF_ITEM A, IT_ITEM B 
 WHERE A.ECI_COMP_CD = :COMP_CD
   AND A.ECI_MNGT_YR = :MNGT_YR
   AND A.ECI_CNRT_SQ = :CNRT_SQ
   AND A.ECI_PROF_SQ = :PROF_SQ
   AND B.IIM_ITEM_CD = A.ECI_ITEM_CD 
 ORDER BY A.ECI_SORT_SQ 
		*/

		sql = " SELECT LTRIM(RTRIM(B.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(B.IIM_KNME_NM)) ITEM_NM, '00000000000' GBHC_KD, A.ECI_CNRT_PR ITEM_PR, 0 HOLI_PR";
		sql += " FROM ET_COMP_CNRT_PROF_ITEM A, IT_ITEM B";
		sql += " WHERE A.ECI_COMP_CD = '" + COMP_CD + "'";
		sql += " AND A.ECI_MNGT_YR = '" + MNGT_YR + "'";
		sql += " AND A.ECI_CNRT_SQ = '" + CNRT_SQ + "'";
		sql += " AND A.ECI_PROF_SQ = '" + PROF_SQ + "'";
		sql += " AND B.IIM_ITEM_CD = A.ECI_ITEM_CD";
		sql += " ORDER BY A.ECI_SORT_SQ";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_008 \n";
			G_INFO += "설명 : 사업장묶음항목별 수가 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ITEM_PR' rs:number='4' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_COMP_CNRT_PROF_ITEM'
		...
		<s:AttributeType name='ROWID' rs:number='6' rs:rowid='true' rs:basetable='ET_COMP_CNRT_PROF_ITEM' rs:basecolumn='ROWID'
