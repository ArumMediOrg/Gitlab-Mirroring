...
SELECT A.ERI_ITEM_CD, A.ERI_RSLT_VL, A.ERI_SPRT_CD, B.IIM_TRTP_CD, C.SIC_CANC_LT
  FROM ET_RSLT_ITEM A
       LEFT OUTER JOIN IT_ITEM B        ON A.ERI_ITEM_CD = B.IIM_ITEM_CD
       LEFT OUTER JOIN ST_ITEM_CANCEL C ON A.ERI_EXAM_DT = C.SIC_EXAM_DT AND A.ERI_EXAM_SQ = C.SIC_EXAM_SQ AND A.ERI_ITEM_CD = C.SIC_ITEM_CD
 WHERE A.ERI_EXAM_DT = :EXAM_DT 
   AND A.ERI_EXAM_SQ = :EXAM_SQ  
   AND NVL(A.ERI_CNCL_YN, 'N') <> 'Y'
   AND B.IIM_RSLT_KD IN ('1', '2', '4', '5', '7')
		*/

		sql = " SELECT A.ERI_ITEM_CD, A.ERI_RSLT_VL, A.ERI_SPRT_CD, B.IIM_TRTP_CD, C.SIC_CANC_LT";
		sql += " FROM ET_RSLT_ITEM A LEFT OUTER JOIN IT_ITEM B";
		sql += " ON A.ERI_ITEM_CD = B.IIM_ITEM_CD LEFT OUTER JOIN ST_ITEM_CANCEL C";
		sql += " ON A.ERI_EXAM_DT = C.SIC_EXAM_DT";
		sql += " AND A.ERI_EXAM_SQ = C.SIC_EXAM_SQ";
		sql += " AND A.ERI_ITEM_CD = C.SIC_ITEM_CD";
		sql += " WHERE A.ERI_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND A.ERI_EXAM_SQ = '" + EXAM_SQ + "'";
		sql += " AND NVL(A.ERI_CNCL_YN, 'N') <> 'Y'";
		sql += " AND B.IIM_RSLT_KD IN ('1', '2', '4', '5', '7')";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uIT_SPChungGU_Chk_UDP_RESULTERR_002 \n";
			G_INFO += "설명 : 2차검진결과 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ERI_SPRT_CD' rs:number='3' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_RSLT_ITEM'
		...
		<s:AttributeType name='c7' rs:name='ROWID' rs:number='8' rs:rowid='true' rs:basetable='ST_ITEM_CANCEL' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
