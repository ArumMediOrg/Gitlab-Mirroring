...
SELECT PlP_EXAM_DT, PLP_EXAM_SQ, PLP_JLPN_DSCD, PLP_JLPN_PNCD, PLP_JLPN_DOCV,
       ...
       CASE WHEN NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH03 = CODE ), ' ') <> ' ' THEN
                 NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH03 = CODE ), ' ') || CHR(13) ELSE '' END || NVL(PLP_ETC_EX, ' ') AS JLPN_EX,
  ...
  FROM PT_LABOR_PANJUNG
       ...
       LEFT OUTER JOIN (SELECT CAST(SUBSTR(CCN_SMALL,1,2) AS INT) AS PACK_CD,
                          ...
                          FROM CT_COMMON JOIN IT_ITEM ON IIM_ITEM_CD = SUBSTR(CCN_SMALL,3,5)
 ...
 :PARAM_TX
		...
		*/
		...
		sql += ", CASE WHEN ITEM_CD = 'A0014' AND NVL(PSP_HIBLOOD_VAL, ' ') <> ' ' THEN IIM_SNME_NM || ': 1차:' || ERI_RSLT_VL || ERI_RSLT_UNIT || ', 2차:' || PSP_HIBLOOD_VAL || ERI_RSLT_UNIT";
		sql += " 	WHEN ITEM_CD = 'A0015' AND NVL(PSP_LIBLOOD_VAL, ' ') <> ' ' THEN IIM_SNME_NM || ': 1차:' || ERI_RSLT_VL || ERI_RSLT_UNIT || ', 2차:' || PSP_LIBLOOD_VAL || ERI_RSLT_UNIT";
		sql += "	WHEN ITEM_CD = 'BA014' AND NVL(PSP_RVAL_GLO, ' ') <> ' ' THEN IIM_SNME_NM || ': 1차:' || ERI_RSLT_VL || ERI_RSLT_UNIT || ', 2차:' || PSP_RVAL_GLO || ERI_RSLT_UNIT";
		sql += "	ELSE IIM_SNME_NM || ': ' || ERI_RSLT_VL || NVL(ERI_RSLT_UNIT, ' ') END AS ERI_RSLT_VL";
		
		sql += ", PFP_RESULTR2_1, PFP_RESULTR2_2";
		
		sql += ", CASE WHEN NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH01 = CODE ), ' ') <> ' ' THEN NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH01 = CODE ), ' ') || CHR(13) ELSE '' END";
		sql += " || CASE WHEN NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH02 = CODE ), ' ') <> ' ' THEN NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH02 = CODE ), ' ') || CHR(13) ELSE '' END";
		sql += " || CASE WHEN NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH03 = CODE ), ' ') <> ' ' THEN NVL((SELECT JLPN_NAME FROM JLPN_TEMP WHERE PLP_JLPN_SH03 = CODE ), ' ') || CHR(13) ELSE '' END";
		sql += " || NVL(PLP_ETC_EX, ' ') AS JLPN_EX";
		
		sql += ", PLP_JLPN_WORK, PLP_JLPN_TTPN";

		sql += " FROM PT_LABOR_PANJUNG LEFT OUTER JOIN PT_FIR_PANJUNG";
		sql += " ON PFP_EXAM_DT = PLP_EXAM_DT";
		sql += " AND PFP_EXAM_SQ = PLP_EXAM_SQ";
		
		sql += " LEFT OUTER JOIN";
		
		sql += " (";
		sql += "	SELECT CAST(SUBSTR(CCN_SMALL,1,2) AS INT) AS PACK_CD, SUBSTR(CCN_SMALL,3,5) AS ITEM_CD, CCN_FULL_NM AS IIM_SNME_NM";
		...
		sql += " ) C";
		sql += " ON SUBSTR(PLP_JLPN_DSCD,1,1) = C.PACK_CD";
		
		sql += " LEFT OUTER JOIN ET_RSLT_ITEM";
		...
		sql += PARAM_TX;
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_POSTMANAGEMENT_UP_GridDisplay_001 \n";
			G_INFO += "설명 : 노동부 판정 조회 \n";
			G_INFO += "\n\n";
