...
SELECT EPI_ITEM_CD, IIM_KNME_NM
FROM ET_PACK_ITEM
  LEFT OUTER JOIN IT_ITEM
    ON IIM_ITEM_CD = EPI_ITEM_CD
WHERE EPI_EXAM_CD = :EXAM_CD
   AND EPI_PACK_SQ = :PACK_SQ
ORDER BY EPI_SORT_SQ
		*/

		sql = " SELECT EPI_ITEM_CD, IIM_KNME_NM";
		sql += " FROM ET_PACK_ITEM LEFT OUTER JOIN IT_ITEM";
		sql += " ON IIM_ITEM_CD = EPI_ITEM_CD";
		sql += " WHERE EPI_EXAM_CD = '" + EXAM_CD + "'";
		sql += " AND EPI_PACK_SQ = '" + PACK_SQ + "'";
		sql += " ORDER BY EPI_SORT_SQ";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uMake_Excel_Apply_btnAddClick_001 \n";
			G_INFO += "설명 : 검사정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ROWID' rs:number='3' rs:rowid='true' rs:basetable='ET_PACK_ITEM' rs:basecolumn='ROWID'
