...
Select SIR_ZERO_LT From ST_ITEM_RECHECK
 Where SIR_EXAM_DT = :EXAM_DT
   And SIR_EXAM_SQ = :EXAM_SQ
   And SIR_ZERO_LT Is Not Null
   And ROWNUM = 1
		*/

		sql = " SELECT SIR_ZERO_LT";
		sql += " FROM ST_ITEM_RECHECK";
		sql += " WHERE SIR_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND SIR_EXAM_SQ = '" + EXAM_SQ + "'";
		sql += " AND SIR_ZERO_LT IS NOT NULL";
		sql += " AND ROWNUM = 1";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_ITEM_RECHK_FormShow_001 \n";
			G_INFO += "설명 : 특검 2차대상항목 기선택한 미검사 조회(top1) 추가조건 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='SIR_ZERO_LT' rs:number='1' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_ITEM_RECHECK'
