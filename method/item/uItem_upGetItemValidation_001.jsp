...
<%
		...
		/*
			...
			ORDER BY IIV_APLY_DT DESC, IIV_EXAM_CD, IIV_SEX_CD ASC, IIV_AGE_FR, IIV_PANJ_CD

		*/

		sql = "SELECT CASE WHEN IIV_SEX_CD = '1' THEN '남' WHEN IIV_SEX_CD = '2' THEN '여' END AS SEX_CD";
		sql += " , (SELECT CCN_FULL_NM FROM CT_COMMON WHERE CCN_LARGE = '0512' AND CCN_LEVEL = '5' AND CCN_SMALL = IIV_LOW_KD) AS LOW_KD";
		sql += " , (SELECT CCN_FULL_NM FROM CT_COMMON WHERE CCN_LARGE = '0512' AND CCN_LEVEL = '5' AND CCN_SMALL = IIV_HIGH_KD) AS HIGH_KD";
		sql += " , (SELECT CCN_FULL_NM FROM CT_COMMON WHERE CCN_LARGE = '0511' AND CCN_LEVEL = '5' AND CCN_SMALL = IIV_EXAM_CD) AS EXAM_CD";
		sql += " , (SELECT CSC_FULL_NM FROM CT_SP_COMMON WHERE CSC_LARGE = 'HM06F' AND CSC_LEVEL = '5' AND CSC_SMALL = IIV_TREF_CD) AS TREF_CD";
		sql += " , IT_ITEM_VLDT.*";
		sql += " FROM IT_ITEM_VLDT";
		sql += " WHERE IIV_APLY_DT = '" + IIV_APLY_DT + "'";
		sql += " AND IIV_ITEM_CD =  '" + IIV_ITEM_CD + "'";
		sql += " ORDER BY IIV_APLY_DT DESC, IIV_EXAM_CD, IIV_SEX_CD ASC, IIV_AGE_FR, IIV_PANJ_CD";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uItem_upGetItemValidation_001 \n";
			G_INFO += "설명 : 검사항목 참고치 및 추가정보 조회 \n";
			G_INFO += "\n\n";
	...
	<resultXml>
		...
		<s:Schema id='RowsetSchema'>
			<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
				...
				<s:AttributeType name='ICD_TRNS_DTT' rs:number='32' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM_VLDT'
					 rs:basecolumn='ROWID'>
					<s:datatype dt:type='dateTime' rs:dbtype='timestamp' dt:maxLength='16' rs:scale='0' rs:precision='19' rs:fixedlength='true'/>
				</s:AttributeType>
				<s:AttributeType name='ROWID' rs:number='33' rs:rowid='true' rs:basetable='IT_ITEM_VLDT' rs:basecolumn='ROWID'
