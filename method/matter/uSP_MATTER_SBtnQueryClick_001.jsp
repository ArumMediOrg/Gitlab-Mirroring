...
<%
		...
		String EDTCSM_MATT_NM = htMethod.get("EDTCSM_MATT_NM");
		String CHKBOXUSE = htMethod.get("CHKBOXUSE");

		//
		if(EDTCSM_MATT_NM == null) { EDTCSM_MATT_NM = ""; }
...
SELECT GetSP_COMMON_cdToNm('HM05H',CSM_JIB_CD) CSM_JIB_NM, 
       GetSP_COMMON_cdToNm('HM06B',CSM_INJA_CD) CSM_INJA_NM, 
       GetSP_COMMON_cdToNm('HM99B',CSM_PERIOD_TM) CSM_PERIOD_NM, 
       GetSP_COMMON_cdToNm('HM99C',CSM_AFTER_TM) CSM_AFTER_NM, 
       GetSP_COMMON_cdToNm('HM99A',CSM_COMM_CD) CSM_COMM_NM, A.* 
FROM CT_SP_MATTER A 
WHERE 0 = 0 

if :EdtCSM_MATT_NM <> '' then
  AND CSM_MATT_NM like '%' || :EdtCSM_MATT_NM || '%'
...
ORDER BY CSM_MATT_CD
		*/
		...
		sql = " SELECT GETSP_COMMON_CDTONM('HM05H',CSM_JIB_CD) CSM_JIB_NM, GETSP_COMMON_CDTONM('HM06B',CSM_INJA_CD) CSM_INJA_NM, GETSP_COMMON_CDTONM('HM99B',CSM_PERIOD_TM) CSM_PERIOD_NM, GETSP_COMMON_CDTONM('HM99C',CSM_AFTER_TM) CSM_AFTER_NM, GETSP_COMMON_CDTONM('HM99A',CSM_COMM_CD) CSM_COMM_NM, A.*";
		sql += " FROM CT_SP_MATTER A";
		sql += " WHERE 0 = 0";

		if(! EDTCSM_MATT_NM.equals("")) {
			sql += " AND CSM_MATT_NM LIKE '%" + EDTCSM_MATT_NM + "%'";
		}
		...
		sql += " ORDER BY CSM_MATT_CD";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_MATTER_SBtnQueryClick_001 \n";
			G_INFO += "설명 : 특검문진정보 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " EDTCSM_MATT_NM : " + EDTCSM_MATT_NM + " \n";
