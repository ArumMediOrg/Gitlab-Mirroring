...
SELECT NVL(LICEN_CD, ' ') LICEN_CD, MAX(CASE WHEN DR_NM = ' ' THEN LICEN_CD ELSE DR_NM END) DR_NM,
  ...
  FROM (SELECT EEI_CAN_KD EXAM_CD, LICEN_CD,
               ...
               CASE WHEN CAN_KD = '1' THEN EXAM_PR ELSE 0 END AS STOMA_PR,
               CASE WHEN CAN_KD = '2' THEN EXAM_PR ELSE 0 END AS COLON_PR,
               CASE WHEN CAN_KD = '3' THEN EXAM_PR ELSE 0 END AS LIVER_PR,
               CASE WHEN CAN_KD = '4' THEN EXAM_PR ELSE 0 END AS BAST_PR,
               CASE WHEN CAN_KD = '5' THEN EXAM_PR ELSE 0 END AS UTER_PR,
               CASE WHEN CAN_KD = '6' THEN EXAM_PR ELSE 0 END AS LUNG_PR,
               CASE WHEN CAN_KD = '1' THEN CNT ELSE 0 END AS STOMA_CNT,
               CASE WHEN CAN_KD = '2' THEN CNT ELSE 0 END AS COLON_CNT,
               CASE WHEN CAN_KD = '3' THEN CNT ELSE 0 END AS LIVER_CNT,
               CASE WHEN CAN_KD = '4' THEN CNT ELSE 0 END AS BAST_CNT,
               CASE WHEN CAN_KD = '5' THEN CNT ELSE 0 END AS UTER_CNT,
               CASE WHEN CAN_KD = '6' THEN CNT ELSE 0 END AS LUNG_CNT,
...
IF (:PNJN_YN = 'Y') THEN
 ...
 ORDER BY DR_NM
	...
	sql += " FROM (SELECT EEI_CAN_KD EXAM_CD, LICEN_CD,";
	...
	sql += " CASE WHEN CAN_KD = '3' THEN EXAM_PR ELSE 0 END AS LIVER_PR,";
	sql += " CASE WHEN CAN_KD = '4' THEN EXAM_PR ELSE 0 END AS BAST_PR,";
	sql += " CASE WHEN CAN_KD = '5' THEN EXAM_PR ELSE 0 END AS UTER_PR,";
	sql += " CASE WHEN CAN_KD = '6' THEN EXAM_PR ELSE 0 END AS LUNG_PR,";
	sql += " CASE WHEN CAN_KD = '1' THEN CNT ELSE 0 END AS STOMA_CNT,";
	sql += " CASE WHEN CAN_KD = '2' THEN CNT ELSE 0 END AS COLON_CNT,";
	sql += " CASE WHEN CAN_KD = 'S' THEN CNT ELSE 0 END AS STOOL_CNT,";
	sql += " CASE WHEN CAN_KD = '3' THEN CNT ELSE 0 END AS LIVER_CNT,";
	sql += " CASE WHEN CAN_KD = '4' THEN CNT ELSE 0 END AS BAST_CNT,";
	...
	sql += " WHEN NVL(EEA_STOOL_CD, '0') <> '0' AND EEI_CAN_KD = '2' THEN ECC_STOOL_CD";
	sql += " WHEN NVL(EEA_LIVER_CD, '0') <> '0' AND EEI_CAN_KD = '3' THEN ECL_LICEN_CD";
	sql += " WHEN NVL(EEA_BAST_CD,  '0') <> '0' AND EEI_CAN_KD = '4' THEN ECB_LICEN_CD";
	sql += " WHEN NVL(EEA_UTER_CD,  '0') <> '0' AND EEI_CAN_KD = '5' THEN ECV_LICEN_CD";
	sql += " WHEN NVL(EEA_LUNG_CD,  '0') <> '0' AND EEI_CAN_KD = '6' THEN ECG_LICEN_CD END LICEN_CD,";
	sql += " SUM(CAN_PR+CASE WHEN EEA_HTSB_YN = 'Y' THEN HTSB_PR ELSE FST_PR END) EXAM_PR";
	sql += " FROM (SELECT EEA_EXAM_DT, EEA_EXAM_SQ, EEA_EXAM_CD, EEA_HTSB_YN, EEA_ORAL_YN, EEA_STOMA_CD, CASE WHEN INSTR(EEA_EXAM_LT, 'I933E0') > 0 THEN EEA_COLON_CD ELSE '0' END EEA_COLON_CD, CASE WHEN INSTR(EEA_EXAM_LT, 'I933BG') > 0 THEN EEA_COLON_CD ELSE '0' END EEA_STOOL_CD, EEA_LIVER_CD, EEA_BAST_CD, EEA_UTER_CD, EEA_LUNG_CD, B.ECS_LICEN_CD, CASE WHEN INSTR(EEA_EXAM_LT, 'I933E0') > 0 THEN C.ECC_LICEN_CD ELSE '' END ECC_LICEN_CD, CASE WHEN INSTR(EEA_EXAM_LT, 'I933BG') > 0 THEN C.ECC_LICEN_CD ELSE '' END ECC_STOOL_CD, D.ECL_LICEN_CD, E.ECB_LICEN_CD, F.ECV_LICEN_CD, G.ECG_LICEN_CD";
	...
	sql += " GROUP BY CASE WHEN EEA_STOOL_CD <> '0' AND EEI_CAN_KD = '2' THEN 'S' ELSE EEI_CAN_KD END, ";
	sql += " CASE WHEN NVL(EEA_STOMA_CD, '0') <> '0' AND EEI_CAN_KD = '1' THEN ECS_LICEN_CD ";
	sql += " WHEN NVL(EEA_COLON_CD, '0') <> '0' AND EEI_CAN_KD = '2' THEN ECC_LICEN_CD ";
	sql += " WHEN NVL(EEA_STOOL_CD, '0') <> '0' AND EEI_CAN_KD = '2' THEN ECC_STOOL_CD ";
	sql += " WHEN NVL(EEA_LIVER_CD, '0') <> '0' AND EEI_CAN_KD = '3' THEN ECL_LICEN_CD ";
	...
	sql += " LEFT OUTER JOIN (SELECT SSV_EXAM_DT, SSV_EXAM_SQ, MAX(NVL(SSV_DOCTOR, ' ')) SSV_DOCTOR_CD";
