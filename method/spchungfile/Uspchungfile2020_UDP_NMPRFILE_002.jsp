...
SELECT EEA_EXAM_DT, EEA_CUST_NO, EEA_CHART_NO, EEA_EXAM_SQ, EEA_SPCL_LT, EEA_HTSB_CD, EEA_SPCL_CD, EEA_SEX_CD, EEA_PSNL_AGE, EEA_ENTRY_DT
  FROM ET_EXAM_ACPT A INNER JOIN ST_BASE B      ON A.EEA_EXAM_DT = B.SBE_EXAM_DT AND A.EEA_EXAM_SQ = B.SBE_EXAM_SQ AND B.SBE_PANJ_YN = 'Y'
 ...
 WHERE 0=0
   ...
   ||:sSQL_ADD
		*/
		...
		sql = " SELECT EEA_EXAM_DT, EEA_CUST_NO, EEA_CHART_NO, EEA_EXAM_SQ, EEA_SPCL_LT, EEA_HTSB_CD, EEA_SPCL_CD, EEA_SEX_CD, EEA_PSNL_AGE, EEA_ENTRY_DT";
		sql += " FROM ET_EXAM_ACPT A INNER JOIN ST_BASE B";
		sql += " ON A.EEA_EXAM_DT = B.SBE_EXAM_DT";
		sql += " AND A.EEA_EXAM_SQ = B.SBE_EXAM_SQ";
		sql += " AND B.SBE_PANJ_YN = 'Y' LEFT OUTER JOIN ST_BILLHN_PR C";
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : Uspchungfile2020_UDP_NMPRFILE_002 \n";
			G_INFO += "설명 : 특검청구 건강진단현황 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='EEA_HTSB_CD' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
		...
		<s:AttributeType name='EEA_ENTRY_DT' rs:number='10' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
		...
		<s:AttributeType name='c12' rs:name='ROWID' rs:number='13' rs:rowid='true' rs:basetable='ST_BILLHN_PR' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c13' rs:name='ROWID' rs:number='14' rs:rowid='true' rs:basetable='IT_COMPANY' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
