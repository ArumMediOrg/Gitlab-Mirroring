...
SELECT A.* FROM ET_CANCER_BRST A
if :cmbPlaceFlg = '1' then
begin
        Inner Join ET_EXAM_ACPT B On B.EEA_EXAM_DT = A.ECB_EXAM_DT 
                And B.EEA_EXAM_SQ = A.ECB_EXAM_SQ                       
                And B.EEA_ORDER_YN <> 'C'                                    
                And SUBSTR(B.EEA_PLCE_CD, 1, 2)= :cmbPlace
end;
WHERE 0=0 
   AND A.ECB_EXAM_DT >= :DTEdtEXAM_SDT
   AND A.ECB_EXAM_DT <= :DTEdtEXAM_EDT
   AND NVL(A.ECB_LICEN_CD, ' ') <> ' ' 
		*/
		...
		if(CMBPLACEFLG.equals("1")) {
			...
			sql += " ON B.EEA_EXAM_DT = A.ECB_EXAM_DT";
			sql += " AND B.EEA_EXAM_SQ = A.ECB_EXAM_SQ";
			sql += " AND B.EEA_ORDER_YN <> 'C'";
			sql += " AND SUBSTR(B.EEA_PLCE_CD, 1, 2)= '" + CMBPLACE + "'";
		}

		sql += " WHERE 0=0";
		sql += " AND A.ECB_EXAM_DT >= '" + DTEDTEXAM_SDT + "'";
		sql += " AND A.ECB_EXAM_DT <= '" + DTEDTEXAM_EDT + "'";
		sql += " AND NVL(A.ECB_LICEN_CD, ' ') <> ' '";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UP_QryOpen_006 \n";
			G_INFO += "설명 : 유방암 검진정보 로딩 \n";
			G_INFO += "\n\n";
