...
<%
		...
		sql = " SELECT NVL(SUM(OLD_UNCHECK_PR),0) OLD_UNCHECK_PR, NVL(SUM(NEW_UNCHECK_PR),0) NEW_UNCHECK_PR FROM ( ";

		sql += " SELECT SUM(B.ITEM_PR) OLD_UNCHECK_PR, 0 NEW_UNCHECK_PR FROM ET_RSLT_ITEM A ";
		sql += " INNER JOIN ";
		...
		sql += " AND NVL(ERI_RSLT_VL, ' ') IN (' ', '미촬영') AND IIM_RSLT_KD <> '9' ";

		sql += " UNION ALL ";

		sql += " SELECT 0 OLD_UNCHECK_PR, SUM(B.ITEM_PR) NEW_UNCHECK_PR FROM ET_RSLT_ITEM A ";
		sql += " INNER JOIN ";
		sql += " (SELECT CASE WHEN C.IIM_EXGN_CD = 'I' THEN  'N0042' ELSE C.IIM_ITEM_CD END IIM_ITEM_CD, C.IIM_KNME_NM, C.IIM_EXGN_CD, C.IIM_RSLT_KD, ";
		sql += " CASE WHEN 'IIC_HLTH_KD' = 'IIC_HLTH_KD' OR 'IIC_HLTH_KD' = 'IIC_CAN_KD' OR 'IIC_HLTH_KD' = 'IIC_STDTO_KD' ";
		sql += " THEN B.IIP_GBSG_PR ";
		...
		sql += " ) C ";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uAcpt_Sales_upGetUnCheckOldAndNewPr_001 \n";
			G_INFO += "설명 : 전일, 현일 일반검진 미검수가 누계 조회 \n";
			G_INFO += "\n\n";
