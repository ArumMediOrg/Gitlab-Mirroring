...
SELECT CSP_JJ_CD, CSP_JJ_NM, CSP_JANGGI_CD, CSP_CHASU, 
       GetSP_COMMON_cdToNm('HM05D',CSP_JANGGI_CD) CSP_JANGGI_NM 
FROM CT_SP_PROF 
WHERE CSP_JANGGI_CD = :JANGGI_CD
  and CSP_USE_YN = 'Y'
GROUP BY CSP_JJ_CD, CSP_JJ_NM, CSP_JANGGI_CD, CSP_CHASU 
		*/

		sql = " SELECT CSP_JJ_CD, CSP_JJ_NM, CSP_JANGGI_CD, CSP_CHASU, GetSP_COMMON_cdToNm('HM05D',CSP_JANGGI_CD) CSP_JANGGI_NM";
		sql += " FROM CT_SP_PROF";
		sql += " WHERE CSP_JANGGI_CD = '" + JANGGI_CD + "'";
		sql += " AND CSP_USE_YN = 'Y'";
		sql += " GROUP BY CSP_JJ_CD, CSP_JJ_NM, CSP_JANGGI_CD, CSP_CHASU";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uST_COMMON_PYOJ_UP_ComboDisplay_001 \n";
			G_INFO += "설명 : 표적장기별 유형정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='CSP_CHASU' rs:number='4' rs:writeunknown='true' rs:basetable='CT_SP_PROF' rs:basecolumn='CSP_CHASU'>
