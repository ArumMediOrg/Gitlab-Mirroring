...
<%
		...
		String SALES_REP = htMethod.get("SALES_REP");

		//
		if(YEAR == null) { YEAR = ""; }
		if(SDT == null) { SDT = ""; }
		if(EDT == null) { EDT = ""; }
		if(SALES_REP == null) { SALES_REP = ""; }

		// DB객체
		stmtList = connect.createStatement();

		sql = " SELECT SALES_REP, NVL(SUM(ITEM_PR),0) NEW_UNCHECK_PR FROM ( ";
		sql += " SELECT NVL(D." + SALES_REP + ", ' ') SALES_REP, B.ITEM_PR FROM ET_RSLT_ITEM A ";
		sql += " INNER JOIN ";
		...
		sql += " LEFT OUTER JOIN ET_EXAM_ACPT D ON D.EEA_EXAM_DT = A.ERI_EXAM_DT AND D.EEA_EXAM_SQ = A.ERI_EXAM_SQ ";
		...
		sql += " GROUP BY A.SALES_REP ";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uAcpt_Sales_upGetSalesUnCheckNewPr_001 \n";
			G_INFO += "설명 : 영업담당자별 일반검진 미검수가 조회 \n";
			G_INFO += "\n\n";
