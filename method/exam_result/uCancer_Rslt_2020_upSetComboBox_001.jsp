...
<%
		...
		String SDOCCOLUMN = htMethod.get("SDOCCOLUMN");

		//
		if(SDOCCOLUMN == null) { SDOCCOLUMN = ""; }
...
Select * from IT_AUTH_USER
Where IAU_LICEN1_CD in( '11','12')
 AND   IAU_USE_YN  = 'Y'
 AND   (NVL('+:sDocColumn+', ' ') <> ' ' and '+:sDocColumn+' <> '')
 AND   IAU_EMP_NO = '+:sDocColumn+'


* (설명) sDocColumn : 조회대상 컬럼명을 parameter 로 넘겨 조회조건에 반영  
    AND   (NVL('+:sDocColumn+', ' ') <> ' ' and '+:sDocColumn+' <> '')      : sDocColumn 해당 하는 컬럼이 null 이나 공백이 아닌 경우
    AND   IAU_EMP_NO = ':sDocColumn'                                              :  IAU_EMP_NO  = '조회대상컬럼명' 의 값이 일치하는 경우
		*/
		...
		sql += " AND (NVL('" + SDOCCOLUMN + "', ' ') <> ' ' AND '" + SDOCCOLUMN + "' <> '')";
		sql += " AND IAU_EMP_NO = '" + SDOCCOLUMN + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCancer_Rslt_2020_upSetComboBox_001 \n";
			G_INFO += "설명 : 콤보리스트(해당 판정의사 조회) \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " SDOCCOLUMN : " + SDOCCOLUMN + " \n";
			G_INFO += "\n\n";
