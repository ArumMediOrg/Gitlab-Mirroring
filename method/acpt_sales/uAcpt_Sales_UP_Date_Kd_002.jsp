...
<%
		...
		sql = " SELECT B.EEP_ORDER_DT EXAM_DT, ";
		sql += " 	SUM(CNT) EXAM_CNT,       SUM(INSU) EXAM_PER, ";
		...
		sql += " 	SUM(B.PSNL_PR) PSNL_PR1, SUM(B.COMP_PR) COMP_PR1, ";
		sql += " 	SUM(B.HDMD_PR) HDMD_PR,  SUM(B.SDMD_PR) SDMD_PR, ";
		sql += " 	SUM(B.DISC_PR) DISC_PR,  SUM(B.DISC_CPR) DISC_CPR, ";
		sql += " 	SUM(B.TOTAL_PR) TOTAL_PR ";
		...
		sql += " GROUP BY B.EEP_ORDER_DT ";
		sql += " ORDER BY B.EEP_ORDER_DT ";


			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uAcpt_Sales_UP_Date_Kd_002 \n";
			G_INFO += "설명 : 검진일자별 현황(처리일자 기준) \n";
			G_INFO += "\n\n";
