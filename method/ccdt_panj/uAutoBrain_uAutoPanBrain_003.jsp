...
LEFT OUTER JOIN QT_QUESTION B ON A.IIQ_QGTN_CD = B.QQN_QSTN_CD                                  
                               AND B.QQN_EXAM_DT = :Ms_Date
                               AND B.QQN_EXAM_SQ = :Ms_Sq
LEFT OUTER JOIN CT_QUESTION C ON A.IIQ_QGTN_CD = C.CQN_QSTN_CD                                  
WHERE A.IIQ_ITEM_CD = 'M0018'                                 
   AND A.IIQ_APLY_DT = GetITEM_QUESTION_ApplyDate('M0018',:Ms_Date)                  
   AND A.IIQ_USE_YN = 'Y'                                                                       
ORDER BY A.IIQ_SORT_NO      
		*/
		...
		sql += " AND B.QQN_EXAM_DT = '" + MS_DATE + "'";
		sql += " AND B.QQN_EXAM_SQ = '" + MS_SQ + "' LEFT OUTER JOIN CT_QUESTION C";
		sql += " ON A.IIQ_QGTN_CD = C.CQN_QSTN_CD";
		sql += " WHERE A.IIQ_ITEM_CD = 'M0018'";
		sql += " AND A.IIQ_APLY_DT = GETITEM_QUESTION_APPLYDATE('M0018','" + MS_DATE + "')";
		sql += " AND A.IIQ_USE_YN = 'Y'";
		sql += " ORDER BY A.IIQ_SORT_NO";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uAutoBrain_uAutoPanBrain_003 \n";
			G_INFO += "설명 : 뇌심혈관 판정정보 로딩 \n";
			G_INFO += "\n\n";
