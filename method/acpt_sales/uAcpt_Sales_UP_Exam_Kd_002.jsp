...
<%
		...
		String EXAM_NM = htMethod.get("EXAM_NM");
		...
		sql = " SELECT D.*, F_COMP_FIND(D.EEA_COMP_CD) EEA_COMP_NM, F_PID(F.ICR_PENL_ID) EEA_PSNL_ID, EEA_CHART_NO";
		sql += " FROM (SELECT C.*, CASE WHEN C.EXAM_CD = '14001' THEN '구강검진' WHEN C.EXAM_CD = '31001' THEN '학생검진' WHEN C.EXAM_CD = '31005' THEN '학생구강' WHEN C.EXAM_CD = '31007' THEN '학교밖-구강' WHEN C.EXAM_CD = '99999' THEN '추가검사' ELSE F_CODE_FIND('0401', C.EXAM_CD, '5', '1') END || CASE WHEN C.ENTY_CD = '9' THEN CASE WHEN SUBSTR(C.EXAM_CD, 1, 2) = '11'";
		...
		sql += " WHERE C.EXAM_CD = '" + EXAM_CD + "'";
		sql += PARAM_TX1;
		sql += PARAM_TX2;
		sql += " ) D, ET_EXAM_ACPT E LEFT OUTER JOIN IT_CUSTOMER F";
		sql += " ON F.ICR_CUST_NO = E.EEA_CUST_NO";
		sql += " WHERE E.EEA_EXAM_DT = D.EEA_EXAM_DT";
		sql += " AND E.EEA_EXAM_SQ = D.EEA_EXAM_SQ";
		sql += " AND D.EXAM_NM = '" + EXAM_NM + "'";
		sql += " ORDER BY D.EEA_EXAM_DT, D.EEA_PSNL_NM";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uAcpt_Sales_UP_Exam_Kd_002 \n";
			G_INFO += "설명 : 검진유형별 상세 현황 \n";
			G_INFO += "\n\n";
