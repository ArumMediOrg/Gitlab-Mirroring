...
SELECT DBO.F_COMP_FIND(A.BEE_COMP_CD) BEE_COMP_NM, A.BEE_ESTI_NM, B.* 
  FROM BT_ESTIMATE A INNER JOIN BT_ESTI_DEPT B ON A.BEE_MNGT_YR = B.BED_MNGT_YR AND A.BEE_ESTI_CD = B.BED_ESTI_CD AND B.BED_USE_YN = 'Y'
 WHERE A.BEE_MNGT_YR = :MNGT_YR
   AND A.BEE_ESTI_CD = :ESTI_CD

		*/

		sql = " SELECT F_COMP_FIND(A.BEE_COMP_CD) BEE_COMP_NM, A.BEE_ESTI_NM, B.*";
		sql += " FROM BT_ESTIMATE A INNER JOIN BT_ESTI_DEPT B";
		sql += " ON A.BEE_MNGT_YR = B.BED_MNGT_YR";
		sql += " AND A.BEE_ESTI_CD = B.BED_ESTI_CD";
		sql += " AND B.BED_USE_YN = 'Y'";
		sql += " WHERE A.BEE_MNGT_YR = '" + MNGT_YR + "'";
		sql += " AND A.BEE_ESTI_CD = '" + ESTI_CD + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uBT_ESTIMATE_BtnSPPrintClick_001 \n";
			G_INFO += "설명 : 인쇄정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='BEE_COMP_NM' rs:number='1' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='BEE_ESTI_NM' rs:number='2' rs:writeunknown='true' rs:basetable='BT_ESTIMATE' rs:basecolumn='BEE_ESTI_NM'>
		...
		<s:AttributeType name='ROWID' rs:number='46' rs:rowid='true' rs:basetable='BT_ESTIMATE' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c46' rs:name='ROWID' rs:number='47' rs:rowid='true' rs:basetable='BT_ESTI_DEPT' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
...
<%
		...
		while(cRsList.next()) {
			...
			String c46_T = cRsList.getString("c46");
%>
			<z:row
<%
