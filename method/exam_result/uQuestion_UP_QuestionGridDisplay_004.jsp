...
LEFT OUTER JOIN CT_QUESTION C ON A.IIQ_QGTN_CD = C.CQN_QSTN_CD
 WHERE A.IIQ_ITEM_CD = :ITEM_CD
  AND A.IIQ_APLY_DT = GetITEM_QUESTION_ApplyDate( :ITEM_CD , '2018-12-31')
   AND A.IIQ_USE_YN = 'Y'
ORDER BY A.IIQ_SORT_NO
		*/
		...
		sql += " AND A.IIQ_APLY_DT = GETITEM_QUESTION_APPLYDATE( '" + ITEM_CD + "' , '2018-12-31')";
		sql += " AND A.IIQ_USE_YN = 'Y'";
		sql += " ORDER BY A.IIQ_SORT_NO";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uQuestion_UP_QuestionGridDisplay_004 \n";
			G_INFO += "설명 : 현문진가져오기(2019년초 특검/일검차이 관련) \n";
			G_INFO += "\n\n";
