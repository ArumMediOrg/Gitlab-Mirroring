...
<%
		...
		sql += " A.RRT_DISC_CD,  A.RRT_DISC_RT, A.RRT_DISC_PR, A.RRT_DISC_CPR, A.RRT_DISC_TX, A.RRT_OCS_NO, A.RRT_SALES_ID ";
		sql += " FROM RT_RSVT A LEFT OUTER JOIN IT_CUSTOMER B";
		sql += " ON B.ICR_CUST_NO = A.RRT_CUST_NO";
		sql += " INNER JOIN RT_RSVT_ITEM C";
		sql += " ON C.RRI_RSVN_NO = A.RRT_RSVN_NO";
		sql += " AND C.RRI_ITEM_CD = '" + ITEM_CD + "'";
		sql += " AND C.RRI_CNCL_YN = 'N'";
		sql += " WHERE A.RRT_EXAM_DT BETWEEN '" + EXAM_SDT + "' AND '" + EXAM_EDT + "'";
		sql += " AND SUBSTR(A.RRT_EXAM_CD, 1, 2) <> '31'";
		sql += " AND NVL(A.RRT_CNCL_YN, 'N') <> 'Y'";

		if(PARAM_TX.equals("")) {
			sql += "ORDER BY A.RRT_EXAM_DT, A.RRT_PSNL_NM";
		} else {
			sql += PARAM_TX;
		}
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uRsvn_List_AlvEquipClick_001 \n";
			G_INFO += "설명 : 검사별 예약 내역 \n";
			G_INFO += "\n\n";
