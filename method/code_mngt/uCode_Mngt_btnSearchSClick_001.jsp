...
<%
		...
		String EDSHRT_NM = htMethod.get("EDSHRT_NM");
		String CBUSE_YN = htMethod.get("CBUSE_YN");

		//
		if(STRLARGE == null) { STRLARGE = ""; }
		if(EDSHRT_NM == null) { EDSHRT_NM = ""; }
		if(CBUSE_YN == null) { CBUSE_YN = ""; }
...
select * 
from ct_common 
where ccn_large = :strLarge
      and ccn_level = '5'
      and ccn_full_nm like '%'||:edShrt_Nm||'%' 
if :cbUse_Yn = '1' then
...
order by ccn_small 
		*/
		...
		sql += " AND CCN_LEVEL = '5'";
		sql += " AND CCN_FULL_NM LIKE '%" + EDSHRT_NM + "%'";

		if(CBUSE_YN.equals("1")) {
			sql += " AND UPPER(CCN_USE_YN) = 'Y'";
		}
		sql += " ORDER BY CCN_SMALL";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCode_Mngt_btnSearchSClick_001 \n";
			G_INFO += "설명 : 코드정보 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " STRLARGE : " + STRLARGE + " \n";
			G_INFO += " EDSHRT_NM : " + EDSHRT_NM + " \n";
			G_INFO += " CBUSE_YN : " + CBUSE_YN + " \n";
			G_INFO += "\n\n";
