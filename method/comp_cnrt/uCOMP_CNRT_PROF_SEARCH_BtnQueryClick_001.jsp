...
<%
		...
		String SSQLSTR = htMethod.get("SSQLSTR");
		String COMP_CD = htMethod.get("COMP_CD");
		String MNGT_YR = htMethod.get("MNGT_YR");
		String CNRT_SQ = htMethod.get("CNRT_SQ");

		//
		if(SSQLSTR == null) { SSQLSTR = ""; }
 ...
 SELECT CASE WHEN ECP_SEX_CD IS NULL OR ECP_SEX_CD = '' THEN ECP_SEX_CD
             ELSE '[' || ECP_SEX_CD  || ']' || :sSqlStr END ECP_SEX_NM, A.*
 ...
 ORDER BY ECP_PROF_SQ
		...
		*/
		...
		sql = " SELECT CASE WHEN ECP_SEX_CD IS NULL OR ECP_SEX_CD = '' THEN ECP_SEX_CD ELSE '[' || ECP_SEX_CD || ']' || " + SSQLSTR + " END ECP_SEX_NM, A.*";
		...
		sql += " ORDER BY ECP_PROF_SQ";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCOMP_CNRT_PROF_SEARCH_BtnQueryClick_001 \n";
			G_INFO += "설명 : 사업장 계약 검진유형 리스트 조회 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " SSQLSTR : " + SSQLSTR + " \n";
