...
----------------------------------------------------
       CASE when SUBSTR(C.ECC_GIRT1_KD, 1,1) = '1' Then '이상소견없음'
            when SUBSTR(C.ECC_GIRT1_KD, 1,1) = '2' Then '대장용종'
                  when SUBSTR(C.ECC_GIRT1_KD, 1,1) = '3' Then '대장암의심'
                  when SUBSTR(C.ECC_GIRT1_KD, 1,1) = '4' Then '대장암'
                  when SUBSTR(C.ECC_GIRT1_KD, 1,1) = '5' Then '기타'
       End as ECC_GIRT1_KD,
       C.ECC_GIRT1S_KD As ECC_GIRT1S_KD_1,
       CASE when SUBSTR(C.ECC_GIRT2_KD, 1,1) = '1' Then '이상소견없음'
            when SUBSTR(C.ECC_GIRT2_KD, 1,1) = '2' Then '대장용종'
                  when SUBSTR(C.ECC_GIRT2_KD, 1,1) = '3' Then '대장암의심'
                  when SUBSTR(C.ECC_GIRT2_KD, 1,1) = '4' Then '대장암'
                  when SUBSTR(C.ECC_GIRT2_KD, 1,1) = '5' Then '기타'
       End as ECC_GIRT2_KD,
       C.ECC_GIRT2S_KD As ECC_GIRT2S_KD_2,
       CASE when SUBSTR(C.ECC_GIRT3_KD, 1,1) = '1' Then '이상소견없음'
            when SUBSTR(C.ECC_GIRT3_KD, 1,1) = '2' Then '대장용종'
                  when SUBSTR(C.ECC_GIRT3_KD, 1,1) = '3' Then '대장암의심'
                  when SUBSTR(C.ECC_GIRT3_KD, 1,1) = '4' Then '대장암'
                  when SUBSTR(C.ECC_GIRT3_KD, 1,1) = '5' Then '기타'
       End as ECC_GIRT3_KD,
       C.ECC_GIRT3S_KD As ECC_GIRT3S_KD_3,
       CASE when SUBSTR(C.ECC_GIETC_KD, 1,1) = '1' Then '치핵'
            when SUBSTR(C.ECC_GIETC_KD, 2,1) = '1' Then '비특이성 장염'
                   when SUBSTR(C.ECC_GIETC_KD, 3,1) = '1' Then '허혈성 장염'
                   when SUBSTR(C.ECC_GIETC_KD, 4,1) = '1' Then '궤양성 대장염'
                   when SUBSTR(C.ECC_GIETC_KD, 5,1) = '1' Then '크론병'
                   when SUBSTR(C.ECC_GIETC_KD, 6,1) = '1' Then '장결핵'
                   when SUBSTR(C.ECC_GIETC_KD, 7,1) = '1' Then '대장 게실증'
                   when SUBSTR(C.ECC_GIETC_KD, 8,1) = '1' Then '대장 점막하종양'
                   when SUBSTR(C.ECC_GIETC_KD, 9,1) = '1' Then '림프구 증식'
                   when SUBSTR(C.ECC_GIETC_KD,10,1) = '1' Then '직접기입'
       End as ECC_GIETC_KD,
       C.ECC_GIRT_EX,
       CASE when SUBSTR(C.ECC_RSLT_KD, 1,1) = '1' Then '이상소견없음'
            when SUBSTR(C.ECC_RSLT_KD, 1,1) = '2' Then '양성질환'
                   when SUBSTR(C.ECC_RSLT_KD, 1,1) = '3' Then '대장암의심'
                   when SUBSTR(C.ECC_RSLT_KD, 1,1) = '4' Then '대장암'
                   when SUBSTR(C.ECC_RSLT_KD, 1,1) = '5' Then '기타'
       End as ECC_RSLT_KD
---------------------------------------------------
        From ET_EXAM_ACPT A
       ...
       Inner Join ET_CANCER_COLO C On A.EEA_EXAM_DT = C.ECC_EXAM_DT And A.EEA_EXAM_SQ = C.ECC_EXAM_SQ
         WHERE 0=0
...
|| :sSQL_ADD
		*/
		...
		sql += ", CASE WHEN SUBSTR(C.ECC_GIRT1_KD, 1,1) = '1' THEN '이상소견없음' WHEN SUBSTR(C.ECC_GIRT1_KD, 1,1) = '2' THEN '대장용종' WHEN SUBSTR(C.ECC_GIRT1_KD, 1,1) = '3' THEN '대장암의심' WHEN SUBSTR(C.ECC_GIRT1_KD, 1,1) = '4' THEN '대장암' WHEN SUBSTR(C.ECC_GIRT1_KD, 1,1) = '5' THEN '기타' END AS ECC_GIRT1_KD";
		sql += ", C.ECC_GIRT1S_KD AS ECC_GIRT1S_KD_1";
		sql += ", CASE WHEN SUBSTR(C.ECC_GIRT2_KD, 1,1) = '1' THEN '이상소견없음' WHEN SUBSTR(C.ECC_GIRT2_KD, 1,1) = '2' THEN '대장용종' WHEN SUBSTR(C.ECC_GIRT2_KD, 1,1) = '3' THEN '대장암의심' WHEN SUBSTR(C.ECC_GIRT2_KD, 1,1) = '4' THEN '대장암' WHEN SUBSTR(C.ECC_GIRT2_KD, 1,1) = '5' THEN '기타' END AS ECC_GIRT2_KD";
		sql += ", C.ECC_GIRT2S_KD AS ECC_GIRT2S_KD_2";
		sql += ", CASE WHEN SUBSTR(C.ECC_GIRT3_KD, 1,1) = '1' THEN '이상소견없음' WHEN SUBSTR(C.ECC_GIRT3_KD, 1,1) = '2' THEN '대장용종' WHEN SUBSTR(C.ECC_GIRT3_KD, 1,1) = '3' THEN '대장암의심' WHEN SUBSTR(C.ECC_GIRT3_KD, 1,1) = '4' THEN '대장암' WHEN SUBSTR(C.ECC_GIRT3_KD, 1,1) = '5' THEN '기타' END AS ECC_GIRT3_KD";
		sql += ", C.ECC_GIRT3S_KD AS ECC_GIRT3S_KD_3";
		sql += ", CASE WHEN SUBSTR(C.ECC_GIETC_KD, 1,1) = '1' THEN '치핵' WHEN SUBSTR(C.ECC_GIETC_KD, 2,1) = '1' THEN '비특이성 장염' WHEN SUBSTR(C.ECC_GIETC_KD, 3,1) = '1' THEN '허혈성 장염' WHEN SUBSTR(C.ECC_GIETC_KD, 4,1) = '1' THEN '궤양성 대장염' WHEN SUBSTR(C.ECC_GIETC_KD, 5,1) = '1' THEN '크론병' WHEN SUBSTR(C.ECC_GIETC_KD, 6,1) = '1' THEN '장결핵' WHEN SUBSTR(C.ECC_GIETC_KD, 7,1) = '1' THEN '대장 게실증' WHEN SUBSTR(C.ECC_GIETC_KD, 8,1) = '1' THEN '대장 점막하종양' WHEN SUBSTR(C.ECC_GIETC_KD, 9,1) = '1' THEN '림프구 증식' WHEN SUBSTR(C.ECC_GIETC_KD,10,1) = '1' THEN '직접기입' END AS ECC_GIETC_KD";
		sql += ", C.ECC_GIRT_EX";
		sql += ", CASE WHEN SUBSTR(C.ECC_RSLT_KD, 1,1) = '1' THEN '이상소견없음' WHEN SUBSTR(C.ECC_RSLT_KD, 1,1) = '2' THEN '양성질환' WHEN SUBSTR(C.ECC_RSLT_KD, 1,1) = '3' THEN '대장암의심' WHEN SUBSTR(C.ECC_RSLT_KD, 1,1) = '4' THEN '대장암' WHEN SUBSTR(C.ECC_RSLT_KD, 1,1) = '5' THEN '기타' END AS ECC_RSLT_KD";
		sql += " FROM ET_EXAM_ACPT A LEFT JOIN IT_COMPANY B";
		sql += " ON A.EEA_COMP_CD = B.ICY_COMP_CD INNER JOIN ET_CANCER_HEADER D";
		sql += " ON A.EEA_EXAM_DT = D.ECH_EXAM_DT";
		sql += " AND A.EEA_EXAM_SQ = D.ECH_EXAM_SQ";
		sql += " AND ECH_CFRM_CD = '2' LEFT OUTER JOIN IT_COMPANY E";
		sql += " ON A.EEA_COMP_CD = E.ICY_COMP_CD INNER JOIN ET_CANCER_COLO C";
		sql += " ON A.EEA_EXAM_DT = C.ECC_EXAM_DT";
		sql += " AND A.EEA_EXAM_SQ = C.ECC_EXAM_SQ";
		sql += " WHERE 0=0";
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uChest_FilmDecipher_UF_SqlSetCancer_002 \n";
			G_INFO += "설명 : 대장 조영 영상판독 소견서 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ECC_GIRT1_KD' rs:number='16' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_CANCER_COLO'
			 rs:basecolumn='ECC_GIRT1S_KD'>
			<s:datatype dt:type='string' dt:maxLength='12'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_GIRT1S_KD_1' rs:number='17' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_CANCER_COLO'
			 rs:basecolumn='ECC_GIRT2S_KD'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_GIRT2_KD' rs:number='18' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_CANCER_COLO'
			 rs:basecolumn='ECC_GIRT3S_KD'>
			<s:datatype dt:type='string' dt:maxLength='12'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_GIRT2S_KD_2' rs:number='19' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_CANCER_COLO'
			 rs:basecolumn='ECC_GIRT_EX'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_GIRT3_KD' rs:number='20' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
			 rs:basecolumn='ROWID'>
			<s:datatype dt:type='string' dt:maxLength='12'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_GIRT3S_KD_3' rs:number='21' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_COMPANY'
			 rs:basecolumn='ROWID'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_GIETC_KD' rs:number='22' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_CANCER_HEADER'
			 rs:basecolumn='ROWID'>
			<s:datatype dt:type='string' dt:maxLength='15'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_GIRT_EX' rs:number='23' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_COMPANY'
			 rs:basecolumn='ROWID'>
			<s:datatype dt:type='string' dt:maxLength='40'/>
		</s:AttributeType>
		<s:AttributeType name='ECC_RSLT_KD' rs:number='24' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_CANCER_COLO'
			 rs:basecolumn='ROWID'>
			<s:datatype dt:type='string' dt:maxLength='12'/>
		</s:AttributeType>
...
<%
		...
		while(cRsList.next()) {
			...
			String ECC_GIRT1S_KD_1_T = cRsList.getString("ECC_GIRT1S_KD_1");
			String ECC_GIRT2_KD_T = cRsList.getString("ECC_GIRT2_KD");
			String ECC_GIRT2S_KD_2_T = cRsList.getString("ECC_GIRT2S_KD_2");
			String ECC_GIRT3_KD_T = cRsList.getString("ECC_GIRT3_KD");
			String ECC_GIRT3S_KD_3_T = cRsList.getString("ECC_GIRT3S_KD_3");
...
<%
			...
			if(! ECC_GIRT1S_KD_1_T.equals("")) {
%>
		 		ECC_GIRT1S_KD_1='<%= ECC_GIRT1S_KD_1_T%>'
<%
			}
...
<%
			...
			if(! ECC_GIRT2S_KD_2_T.equals("")) {
%>
		 		ECC_GIRT2S_KD_2='<%= ECC_GIRT2S_KD_2_T%>'
<%
			}
%>
				ECC_GIRT3_KD='<%= cnt%>'
