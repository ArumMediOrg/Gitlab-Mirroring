...
<%
		...
		String ARUM_PRO_CD = htMethod.get("ARUM_PRO_CD");

		//
		if(ARUM_PRO_CD == null) { ARUM_PRO_CD = ""; }
 ...
 WHERE ARUM_PRO_CD = :ARUM_PRO_CD
   ...
   AND UPPER(UB_USE_YN)   = 'Y' 
		...
		*/
		...
		sql += " WHERE ARUM_PRO_CD = '" + ARUM_PRO_CD + "'";
		sql += " AND UPPER(ARUM_USE_YN) = 'Y'";
		sql += " AND UPPER(UB_USE_YN) = 'Y'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uPack_Each_AlvPackSelectItem_002 \n";
			G_INFO += "설명 : 유비캐어용 묶음정보 로딩 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " ARUM_PRO_CD : " + ARUM_PRO_CD + " \n";
			G_INFO += "\n\n";
