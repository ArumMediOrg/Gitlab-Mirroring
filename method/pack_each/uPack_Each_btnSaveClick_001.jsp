...
select epk_pack_nm from et_pack 
 where epk_trns_id = :trns_id
   and (epk_exam_cd <> :exam_cd
    or  epk_pack_sq <> :pack_sq) 

		*/

		sql = " SELECT EPK_PACK_NM";
		sql += " FROM ET_PACK";
		sql += " WHERE EPK_TRNS_ID = '" + TRNS_ID + "'";
		sql += " AND (EPK_EXAM_CD <> '" + EXAM_CD + "' OR EPK_PACK_SQ <> '" + PACK_SQ + "')";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uPack_Each_btnSaveClick_001 \n";
			G_INFO += "설명 : 묶음이름 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='EPK_PACK_NM' rs:number='1' rs:writeunknown='true' rs:basetable='ET_PACK' rs:basecolumn='EPK_PACK_NM'>
			<s:datatype dt:type='string' dt:maxLength='200' rs:maybenull='false'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='2' rs:rowid='true' rs:basetable='ET_PACK' rs:basecolumn='ROWID' rs:keycolumn='true'
