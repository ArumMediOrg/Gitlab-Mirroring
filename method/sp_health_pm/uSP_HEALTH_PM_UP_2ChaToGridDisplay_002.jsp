...
SELECT A.EEA_CUST_NO,                                                                                  
       CASE WHEN A.EEA_SPCL_CD IS NULL OR A.EEA_SPCL_CD = '' THEN A.EEA_SPCL_CD                      
            ELSE '[' || A.EEA_SPCL_CD || ']' || GetCOMMON_LvCdToNm('0910',A.EEA_SPCL_CD,'5',2)
            END ICR_SPCL_NM,                                                                           
       CASE WHEN A.EEA_EXAM_CD = '41001' OR A.EEA_SPSB_YN = 'Y' THEN '1'                         
            ELSE '2' END ICR_SPCH_KD,                                                                
             A.EEA_EXAM_DT, A.EEA_EXAM_SQ, A.EEA_CHART_NO, E.SBE_TOBO_DT, E.SBE_PANJ_YN,                      
      E.SBE_TPAN_NM, E.SBE_TBUS_NM, B.SPG_PANJ_DT                                                      
FROM ET_EXAM_ACPT A                                                                                    
     LEFT OUTER JOIN ST_BASE E ON A.EEA_EXAM_DT = E.SBE_EXAM_DT                                             
                             AND A.EEA_EXAM_SQ = E.SBE_EXAM_SQ                                             
...
WHERE 0=0
      ...
      AND A.EEA_EXAM_SQ = :EXAM_SQ
		*/
		...
		sql = " SELECT A.EEA_CUST_NO, CASE WHEN A.EEA_SPCL_CD IS NULL";
		sql += " OR A.EEA_SPCL_CD = '' THEN A.EEA_SPCL_CD ELSE '[' || A.EEA_SPCL_CD || ']' || GETCOMMON_LVCDTONM('0910',A.EEA_SPCL_CD,'5',2) END ICR_SPCL_NM, CASE WHEN A.EEA_EXAM_CD = '41001'";
		sql += " OR A.EEA_SPSB_YN = 'Y' THEN '1' ELSE '2' END ICR_SPCH_KD, A.EEA_EXAM_DT, A.EEA_EXAM_SQ, A.EEA_CHART_NO, E.SBE_TOBO_DT, E.SBE_PANJ_YN, E.SBE_TPAN_NM, E.SBE_TBUS_NM, B.SPG_PANJ_DT";
		sql += " FROM ET_EXAM_ACPT A LEFT OUTER JOIN ST_BASE E";
		sql += " ON A.EEA_EXAM_DT = E.SBE_EXAM_DT";
		sql += " AND A.EEA_EXAM_SQ = E.SBE_EXAM_SQ INNER JOIN ST_PANJUNG B";
		...
		sql += " AND A.EEA_EXAM_SQ = '" + EXAM_SQ + "'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_PM_UP_2ChaToGridDisplay_002 \n";
			G_INFO += "설명 : 접수정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='SPG_PANJ_DT' rs:number='11' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_PANJUNG'
		...
		<s:AttributeType name='c12' rs:name='ROWID' rs:number='13' rs:rowid='true' rs:basetable='ST_BASE' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c13' rs:name='ROWID' rs:number='14' rs:rowid='true' rs:basetable='ST_PANJUNG' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
