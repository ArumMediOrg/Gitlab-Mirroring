...
<%
		...
		String IAU_DEPT_CD = htMethod.get("IAU_DEPT_CD");
		String IAU_USE_YN = htMethod.get("IAU_USE_YN");

		//
		if(IAU_DEPT_CD == null) { IAU_DEPT_CD = ""; }
...
if :IAU_DEPT_CD <> '' then
begin
  AND  IAU_DEPT_CD = :IAU_DEPT_CD
end
if :IAU_USE_YN  <> '' then
begin
  AND  ISNULL(IAU_USE_YN, ''N'') = :IAU_USE_YN
end
ORDER BY IAU_USE_YN DESC, IAU_EMP_NO
		*/
		...
		sql += ")";
		if(IAU_DEPT_CD != null && ! IAU_DEPT_CD.equals("")) {
			sql += " AND IAU_DEPT_CD = '" + IAU_DEPT_CD + "'";
		}
		if(IAU_USE_YN != null && ! IAU_USE_YN.equals("")) {
			sql += " AND NVL(IAU_USE_YN, 'N') = '" + IAU_USE_YN + "'";
		}
		sql += " ORDER BY IAU_USE_YN DESC, IAU_EMP_NO";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uCommon_XML_IAU_004 \n";
			G_INFO += "설명 : 사용자조회(의사리스트-특정부서 조건추가) \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " IAU_DEPT_CD : " + IAU_DEPT_CD + " \n";
