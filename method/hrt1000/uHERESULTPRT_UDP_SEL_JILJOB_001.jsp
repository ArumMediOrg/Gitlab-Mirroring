...
<%
		...
		sql += " FROM (SELECT MAX(GB) GB,SUM(CNT) CNT, MAX(AA) AA,MAX(BB) BB";
		...
		sql += " AND PLP_JLPN_PNCD IN ('D1','D2') ) FF";
		sql += " GROUP BY EEA_SEX_CD ) DD";
		sql += " UNION";
		sql += " ALL SELECT MAX(GB) GB,SUM(CNT) CNT, MAX(AA) AA,MAX(BB) BB";
		...
		sql += " GROUP BY EEA_SEX_CD) DD ) T1 ) T2";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uHERESULTPRT_UDP_SEL_JILJOB_001 \n";
			G_INFO += "설명 : 근로자 질병정보 로딩 \n";
			G_INFO += "\n\n";
