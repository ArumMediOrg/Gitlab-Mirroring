...
<%
		...
		/*
			...
			SELECT IIR_RSLT_CD, MMT_VIEW_NM, MMT_VIEW_TXT
			FROM IT_ITEM_RESULT
			JOIN MT_MEDI_EQUIP ON
			MMT_VIEW_CD = SUBSTR(IIR_RSLT_CD,1,10) AND
			MMT_VIEW_SQ = SUBSTR(IIR_RSLT_CD,11,10)
			WHERE IIR_ITEM_CD = :IIR_ITEM_CD
			AND IIR_RSLT_KD = ''3''

		*/

		sql = " SELECT IIR_RSLT_CD, MMT_VIEW_NM, MMT_VIEW_TXT";
		...
		sql += " AND IIR_RSLT_KD = '3'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uItem_upGetItemResult_001 \n";
			G_INFO += "설명 : 검사항목 소견조회 \n";
			G_INFO += "\n\n";
	...
	<resultXml>
		...
		<s:Schema id='RowsetSchema'>
			<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
				...
				<s:AttributeType name='ROWID' rs:number='4' rs:rowid='true' rs:basetable='IT_ITEM_RESULT' rs:basecolumn='ROWID'
					 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
					<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
				</s:AttributeType>
				<s:AttributeType name='c4' rs:name='ROWID' rs:number='5' rs:rowid='true' rs:basetable='MT_MEDI_EQUIP' rs:basecolumn='ROWID'
					 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
					<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
				</s:AttributeType>
