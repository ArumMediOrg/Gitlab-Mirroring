...
Select
  A.ERI_EXAM_DT, A.ERI_EXAM_SQ, A.ERI_ITEM_CD, A.ERI_RSLT_VL, B.IIM_SNME_NM
From ET_RSLT_ITEM A
Inner Join IT_ITEM B On B.IIM_SBCD_CD = '0606'
                    And B.IIM_USE_YN = 'Y'
 Where A.ERI_ITEM_CD = B.IIM_ITEM_CD
   And ERI_EXAM_DT = :EXAM_DT
   And ERI_EXAM_SQ = :EXAM_SQ

		*/

		sql = " SELECT A.ERI_EXAM_DT, A.ERI_EXAM_SQ, A.ERI_ITEM_CD, A.ERI_RSLT_VL, B.IIM_SNME_NM";
		sql += " FROM ET_RSLT_ITEM A INNER JOIN IT_ITEM B";
		sql += " ON B.IIM_SBCD_CD = '0606'";
		sql += " AND B.IIM_USE_YN = 'Y'";
		sql += " WHERE A.ERI_ITEM_CD = B.IIM_ITEM_CD";
		sql += " AND ERI_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND ERI_EXAM_SQ = '" + EXAM_SQ + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_Health_GrdEXAM_ACPTSelectionChanged_005 \n";
			G_INFO += "설명 : 검진결과조회(위장관계문진) \n";
			G_INFO += "\n\n";
