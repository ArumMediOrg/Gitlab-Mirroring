...
SELECT A.EEA_EXAM_DT, A.EEA_CUST_NO, A.EEA_PSNL_NM, A.EEA_EXAM_SQ, A.EEA_PSNL_AGE, A.EEA_COMP_CD,
       A.EEA_HTSB_CD, A.EEA_EXAM_CD, A.EEA_HDMD_YN, A.EEA_SDMD_YN, A.EEA_SPCL_LT,  A.EEA_SPCL_CD,
       A.EEA_SPHT_CD, A.EEA_CHART_NO,
       CASE WHEN A.EEA_EXAM_CD = '41001' OR A.EEA_SPSB_YN = 'Y' THEN '1'
            ELSE '2' END EEA_SPCH_KD,
       CASE WHEN A.EEA_SPCL_CD IS NULL OR A.EEA_SPCL_CD = '' THEN A.EEA_SPCL_CD
            ...
            END EEA_SPCL_NM,
       CASE WHEN A.EEA_SPHT_CD IS NULL OR A.EEA_SPHT_CD = '' THEN A.EEA_SPHT_CD
            ELSE '[' || A.EEA_SPHT_CD || ']' || GetCOMMON_LvCdToNm('0911',A.EEA_SPHT_CD,'5',1)
            END EEA_SPHT_MN,
       SUBSTR(B.ICR_PENL_ID,1,6) EEA_BIRTH_DT, B.ICR_BIRH_KD, B.ICR_TEL_NO, B.ICR_MOBL_NO,
       B.ICR_SEX_KD
 ...
 WHERE 0=0
   ...
   AND (A.EEA_EXAM_CD = '41001' OR A.EEA_EXAM_CD = '42001' OR A.EEA_SPSB_YN = 'Y')
 ORDER BY A.EEA_EXAM_DT DESC
		*/
		...
		sql += " ORDER BY A.EEA_EXAM_DT DESC";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_PAST_PANJ_FormShow_001 \n";
			G_INFO += "설명 : 과거판정 검진이력 조회(1건) \n";
			G_INFO += "\n\n";
