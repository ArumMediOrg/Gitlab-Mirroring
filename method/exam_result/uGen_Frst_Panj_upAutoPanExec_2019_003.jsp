...
select  QQN_RESULT
 ...
 where QQN_EXAM_DT = :EXAM_DT
   ...
   and IIQ_ITEM_CD = 'M0057'
   and QQN_QSTN_CD in ('930')
   and IIQ_APLY_DT = GetITEM_QUESTION_ApplyDate('M0057', :EXAM_DT )
   AND rownum <= 1

		*/

		sql = " SELECT QQN_RESULT";
		...
		sql += " AND IIQ_ITEM_CD = 'M0057'";
		sql += " AND QQN_QSTN_CD IN ('930')";
		sql += " AND IIQ_APLY_DT = GETITEM_QUESTION_APPLYDATE('M0057', '" + EXAM_DT + "')";
		sql += " AND ROWNUM <= 1";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uGen_Frst_Panj_upAutoPanExec_2019_003 \n";
			G_INFO += "설명 : 일검자동판정(우울증 자살생각) \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='QQN_RESULT' rs:number='1' rs:nullable='true' rs:writeunknown='true' rs:basetable='QT_QUESTION'
