...
<%
		...
		sql += " OR A.EEA_EXAM_CD = '' THEN A.EEA_EXAM_CD ELSE '[' || A.EEA_EXAM_CD || ']' || GETCOMMON_LVCDTONM('0401',A.EEA_EXAM_CD,'5',5) END ICR_EXAM_NM, A.EEA_EXAM_CD, A.EEA_HDMD_YN, A.EEA_SDMD_YN, A.EEA_RFID_NO";
		sql += " FROM ET_EXAM_ACPT A INNER JOIN IT_CUSTOMER B";
		sql += " ON A.EEA_CUST_NO = B.ICR_CUST_NO LEFT OUTER JOIN IT_COMPANY C";
		sql += " ON A.EEA_COMP_CD = C.ICY_COMP_CD LEFT OUTER JOIN ET_JOBSTRESS_SIMPLE J";
		...
		if(! CMBBOXEEA_EXAM_CD2_1.equals("00000") && ! CMBBOXEEA_EXAM_CD2_1.equals("")) {
			...
			if(CMBBOXEEA_EXAM_CD1.equals("11") || CMBBOXEEA_EXAM_CD1.equals("12")) {		// 일반검진, 생애검진
				...
				sql += " AND A.EEA_HTSB_CD = '" + CMBBOXEEA_EXAM_CD2_1 + "'))";
				...
				sql += " AND A.EEA_EMSB_CD = '" + CMBBOXEEA_EXAM_CD2_1 + "'))";
				...
				sql += " AND A.EEA_CVSB_CD = '" + CMBBOXEEA_EXAM_CD2_1 + "'))";
		...
		if(CMBBOXSORT_KD.equals("2")) {
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uJobStress_Simple_SBtnQueryClick_001 \n";
			G_INFO += "설명 : 직무스트레스(단축) 검사정보 로딩 \n";
			G_INFO += "\n\n";
