...
<%
		...
		if(CHBXRETRUN_PR.equals("Y")){
			if(CHBXPNT_GROUP.equals("Y")){
				...
				sql += " SUM(CASHAMT)  - NVL(CASE WHEN RRM_PAY_KD in ('02','04') THEN (select SUM(RRM_RETURN_PR) from RT_RECE_MEDICHK a where a.RRM_RECE_NO = FFF.RRM_RECE_NO and a.RRM_RETURN_RECE_SEQ = FFF.RRM_RECE_SEQ) else 0 END,0)  CASHAMT, ";
				...
				sql += " sum(CARDAMT) CARDAMT,  sum(CASREAMT) CASREAMT, sum(CASHAMT) CASHAMT, sum(OTHERAMT) OTHERAMT,";
			}
		}
		else {
			sql += " SUM(CARDAMT) CARDAMT, SUM(CASREAMT) CASREAMT, SUM(CASHAMT) CASHAMT, sum(OTHERAMT) OTHERAMT,";
		...
		sql += " (select SUM(RRM_RECE_PR) from RT_RECE_MEDICHK a where a.RRM_RECE_NO = FFF.RRM_RECE_NO and a.RRM_OCCU_KD <> '10')  SUM_RECE_PR, ";
		...
		sql += " 	MAX(CARD_APPRNO) CARD_APPRNO, MAX(CASH_APPRNO) CASH_APPRNO, MAX(CARDNO) CARDNO, MAX(CASHNO) CASHNO, MAX(RRM_PRINT_CNT) RRM_PRINT_CNT ";
		sql += " FROM ";
		...
		sql += " 			CASE WHEN RRM_PAY_KD not in ('01','02','03','04') THEN RRM_RECE_PR ELSE 0 END OTHERAMT, ";
		...
		sql += " 			CASE WHEN RRM_PAY_KD = '02' THEN RRM_APPR_NO END CASH_APPRNO, ";
		...
		sql += " 					) CARD_NM, ";
		sql += " 					RRM_SALE_PR, RRM_TRUNC_PR, RRM_APPR_NO, RRM_ISSUE_CD,RRM_RECE_PR,RRM_CARD_NO, EEA_CHART_NO, EEA_CUST_NO, RRM_RETURN_PR ";
		sql += " 				FROM RT_RECE_MEDICHK a ";
		sql += " 					LEFT OUTER  JOIN ET_EXAM_ACPT B ON A.RRM_RECE_NO  = B.EEA_RECE_NO ";
		...
		if(CHBXPNT_GROUP.equals("Y")){
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : ubT_Rece_RzBTN_RECEPRTClick_002 \n";
