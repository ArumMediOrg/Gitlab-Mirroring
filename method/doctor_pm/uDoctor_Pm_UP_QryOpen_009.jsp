...
SELECT A.* FROM ET_STDT_PNJN A
if :cmbPlaceFlg = '1' then
begin
        Inner Join ET_EXAM_ACPT B On B.EEA_EXAM_DT = A.ESP_EXAM_DT        
                And B.EEA_EXAM_SQ = A.ESP_EXAM_SQ            
                And B.EEA_ORDER_YN <> 'C'                         
                And SUBSTR(B.EEA_PLCE_CD, 1, 2)= :cmbPlace
end;
WHERE 0=0 ';
        AND A.ESP_EXAM_DT >= :DTEdtEXAM_SDT
        AND A.ESP_EXAM_DT <= :DTEdtEXAM_EDT
        AND (NVL(A.ESP_EXAM_DR, ' ') <> ' ' OR NVL(A.ESP_ORAL_DR, ' ') <> ' ') 
		*/
		...
		if(CMBPLACEFLG.equals("1")) {
			...
			sql += " ON B.EEA_EXAM_DT = A.ESP_EXAM_DT";
			sql += " AND B.EEA_EXAM_SQ = A.ESP_EXAM_SQ";
			sql += " AND B.EEA_ORDER_YN <> 'C'";
			sql += " AND SUBSTR(B.EEA_PLCE_CD, 1, 2)= '" + CMBPLACE + "'";
		}

		sql += " WHERE 0=0";
		sql += " AND A.ESP_EXAM_DT >= '" + DTEDTEXAM_SDT + "'";
		sql += " AND A.ESP_EXAM_DT <= '" + DTEDTEXAM_EDT + "'";
		sql += " AND (NVL(A.ESP_EXAM_DR, ' ') <> ' ' OR NVL(A.ESP_ORAL_DR, ' ') <> ' ')";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UP_QryOpen_009 \n";
			G_INFO += "설명 : 학생검진 검진정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ROWID' rs:number='44' rs:rowid='true' rs:writeunknown='true' rs:basetable='ET_STDT_PNJN'
