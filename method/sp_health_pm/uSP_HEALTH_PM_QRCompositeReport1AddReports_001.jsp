...
SELECT EEA_HCV_YN FROM ET_EXAM_ACPT
 WHERE EEA_EXAM_DT = :EXAM_DT
   and EEA_EXAM_SQ = :EXAM_SQ
   AND EEA_HCV_YN  = 'Y'
		*/

		sql = " SELECT EEA_HCV_YN";
		sql += " FROM ET_EXAM_ACPT";
		sql += " WHERE EEA_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND EEA_EXAM_SQ = '" + EXAM_SQ + "'";
		sql += " AND EEA_HCV_YN = 'Y'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_PM_QRCompositeReport1AddReports_001 \n";
			G_INFO += "설명 : C형감염 대상자정보 로딩 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='EEA_HCV_YN' rs:number='1' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
