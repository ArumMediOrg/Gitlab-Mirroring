...
SELECT A.IIM_EXGN_CD, A.IIM_SORT_SQ, 
       ...
       CASE WHEN SUBSTR(:CALC_CD, 1, 1) = 'Y' THEN B.IIP_SBSG_PR 
            ELSE CASE WHEN :MNGT_KD = '0' THEN 0 
 ...
 WHERE A.IIM_SBCD_CD = :SBCD_CD
   ...
   AND UPPER(B.IIP_USE_YN) = 'Y'               
		...
		*/
		...
		sql = " SELECT A.IIM_EXGN_CD, A.IIM_SORT_SQ, LTRIM(RTRIM(A.IIM_ITEM_CD)) ITEM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) ITEM_NM, '00000000000' GBHC_KD, CASE WHEN SUBSTR('" + CALC_CD + "', 1, 1) = 'Y' THEN B.IIP_SBSG_PR ELSE CASE WHEN '" + MNGT_KD + "' = '0' THEN 0 ELSE CASE WHEN SUBSTR('" + CALC_CD + "', 2, 1) = '4' THEN B.IIP_SBSG_PR ELSE B.IIP_TGSG_PR END END END ITEM_PR, B.IIP_GASN_PR HOLI_PR";
		sql += " FROM IT_ITEM A, IT_ITEM_PRICE B";
		sql += " WHERE A.IIM_SBCD_CD = '" + SBCD_CD + "'";
		...
		sql += " AND UPPER(B.IIP_USE_YN) = 'Y'";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_022 \n";
			G_INFO += "설명 : 특검 행정관리비 조회 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " CALC_CD : " + CALC_CD + " \n";
			G_INFO += " MNGT_KD : " + MNGT_KD + " \n";
