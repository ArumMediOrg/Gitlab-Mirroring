...
SELECT A.* FROM PT_SED_PANJUNG A
WHERE 0=0
 AND A.PSP_EXAM_DT >= :DTEdtEXAM_SDT
 AND A.PSP_EXAM_DT <= :DTEdtEXAM_EDT
 AND NVL(A.PSP_DOCTORSED, ' ') <> ' ' 

		*/

		sql = " SELECT A.*";
		sql += " FROM PT_SED_PANJUNG A";
		sql += " WHERE 0=0";
		sql += " AND A.PSP_EXAM_DT >= '" + DTEDTEXAM_SDT + "'";
		sql += " AND A.PSP_EXAM_DT <= '" + DTEDTEXAM_EDT + "'";
		sql += " AND NVL(A.PSP_DOCTORSED, ' ') <> ' '";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UP_QryOpen_002 \n";
			G_INFO += "설명 : 일반검진 2차 판정정보 로딩 \n";
			G_INFO += "\n\n";
