...
SELECT B.CSM_MATT_NM, A.*
  FROM ST_ITEM_RECHECK A
       INNER JOIN CT_SP_MATTER B ON A.SIR_MATT_CD = B.CSM_MATT_CD
 WHERE A.SIR_EXAM_DT  = :EXAM_DT
   AND A.SIR_EXAM_SQ  = :EXAM_SQ
   AND A.SIR_EXAM_CHA = '1'
   
IF (:MATT_CD <> '') THEN
  AND A.SIR_MATT_CD = :MATT_CD
		*/

		sql = " SELECT B.CSM_MATT_NM, A.*";
		sql += " FROM ST_ITEM_RECHECK A INNER JOIN CT_SP_MATTER B";
		sql += " ON A.SIR_MATT_CD = B.CSM_MATT_CD";
		sql += " WHERE A.SIR_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND A.SIR_EXAM_SQ = '" + EXAM_SQ + "'";
		sql += " AND A.SIR_EXAM_CHA = '1'";

		if(! MATT_CD.equals("")) {
			sql += " AND A.SIR_MATT_CD = '" + MATT_CD + "'";
		}

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_POSTMANAGEMENT_UF_Exam2ChaCheck_001 \n";
			G_INFO += "설명 : 2차 대상 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='CSM_MATT_NM' rs:number='1' rs:writeunknown='true' rs:basetable='CT_SP_MATTER' rs:basecolumn='CSM_MATT_NM'>
		...
		<s:AttributeType name='c23' rs:name='ROWID' rs:number='24' rs:rowid='true' rs:basetable='CT_SP_MATTER' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
