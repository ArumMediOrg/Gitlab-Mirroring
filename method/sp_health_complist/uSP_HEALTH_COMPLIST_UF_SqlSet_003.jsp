...
SELECT A.EEA_COMP_CD, B.ICY_COMP_NM, B.ICY_ZIP_AR || B.ICY_ROAD_AR AS ICR_ADRESS,
       B.ICY_TEL_NO, B.ICY_PRES_NM, C.ICP_TOT_M, C.ICP_TOT_F,
       GETCOMMON_LVCDTONM('0811', B.ICY_LABO_CD, '5', 3) ICY_LABO_NM, 
  ...
  FROM ET_EXAM_ACPT A 
       ...
       LEFT OUTER JOIN IT_COMPANY_PERS C ON A.EEA_COMP_CD = C.ICP_COMP_CD
                                        AND SUBSTR(A.EEA_MNGT_SPYM, 1, 4) = C.ICP_MNGT_YR
 WHERE 0 = 0
...
:ORDER_BY
		...
		*/
		...
		sql = " SELECT A.EEA_COMP_CD, B.ICY_COMP_NM, B.ICY_ZIP_AR || B.ICY_ROAD_AR AS ICR_ADRESS, B.ICY_TEL_NO, B.ICY_PRES_NM, C.ICP_TOT_M, C.ICP_TOT_F, GETCOMMON_LVCDTONM('0811', B.ICY_LABO_CD, '5', 3) ICY_LABO_NM, GETCOMMON_LVCDTONM('0805', B.ICY_TKGJ_CD, '5', 1) ICY_TKGJ_NM, MIN(A.EEA_EXAM_DT) AS EEA_MIN_DT, MAX(A.EEA_EXAM_DT) AS EEA_MAX_DT";
		sql += " FROM ET_EXAM_ACPT A LEFT OUTER JOIN IT_COMPANY B";
		sql += " ON A.EEA_COMP_CD = B.ICY_COMP_CD LEFT OUTER JOIN IT_COMPANY_PERS C";
		sql += " ON A.EEA_COMP_CD = C.ICP_COMP_CD";
		sql += " AND SUBSTR(A.EEA_MNGT_SPYM, 1, 4) = C.ICP_MNGT_YR";
		sql += " WHERE 0 = 0";
		...
		sql += ORDER_BY;
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_HEALTH_COMPLIST_UF_SqlSet_003 \n";
			G_INFO += "설명 : 직업병 03-유소견자(D1)/04-요관찰자(C1) 대상자 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='ICP_TOT_M' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_COMPANY_PERS'
			 rs:basecolumn='ICP_TOT_M'>
			<s:datatype dt:type='number' rs:dbtype='numeric' dt:maxLength='19' rs:scale='0' rs:precision='10' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='ICP_TOT_F' rs:number='7' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_COMPANY_PERS'
			 rs:basecolumn='ICP_TOT_F'>
			<s:datatype dt:type='number' rs:dbtype='numeric' dt:maxLength='19' rs:scale='0' rs:precision='10' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='ICY_LABO_NM' rs:number='8' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='ICY_TKGJ_NM' rs:number='9' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='4000'/>
		</s:AttributeType>
		<s:AttributeType name='EEA_MIN_DT' rs:number='10' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
		<s:AttributeType name='EEA_MAX_DT' rs:number='11' rs:nullable='true'>
			<s:datatype dt:type='string' dt:maxLength='10'/>
		</s:AttributeType>
