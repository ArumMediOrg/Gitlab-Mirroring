...
Select EEA_SPCL_LT From ET_EXAM_ACPT
 Where NVL(EEA_SPCL_LT, ' ') <> ' '
   And EEA_EXAM_DT = :EXAM_DT
   And EEA_EXAM_SQ = :EXAM_SQ
		*/

		sql = " SELECT EEA_SPCL_LT";
		sql += " FROM ET_EXAM_ACPT";
		sql += " WHERE NVL(EEA_SPCL_LT, ' ') <> ' '";
		sql += " AND EEA_EXAM_DT = '" + EXAM_DT + "'";
		sql += " AND EEA_EXAM_SQ = '" + EXAM_SQ + "'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_Health_NEWAutoPanjung_001 \n";
			G_INFO += "설명 : 특검자동판정-취급물질 유무 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='EEA_SPCL_LT' rs:number='1' rs:nullable='true' rs:writeunknown='true' rs:basetable='ET_EXAM_ACPT'
