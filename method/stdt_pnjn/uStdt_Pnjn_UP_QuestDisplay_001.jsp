...
SELECT A.IIQ_ITQU_LV1, A.IIQ_ITQU_LV2, A.IIQ_ITQU_SQ, C.CQN_QSTN_CD, C.CQN_QSKD_CD, C.CQN_QSTN_SQ, C.CQN_QSTN_LV, C.CQN_QSTN_NM, 
       C.CQN_ANSR_CD, C.CQN_ANSE_EX, C.CQN_QSTN_CP, C.CQN_DEFULT, NVL(B.QQN_RESULT,'') QQN_RESULT, 
       CASE WHEN C.CQN_ANSR_CD IS NULL OR C.CQN_ANSR_CD = '' THEN C.CQN_ANSR_CD ELSE C.CQN_ANSR_CD || '-' || GETCOMMON_LVCDTONM('0415',C.CQN_ANSR_CD,'5',1) END CQN_ANSR_NM, 
       C.CQN_SCOR_YN, C.CQN_SCOR_CNT, C.CQN_SCOR_NO, C.CQN_SCOR_SCR, B.QQN_SCORE_YN, B.QQN_SCORE  
  FROM IT_ITEM_QUESTION A 
       LEFT OUTER JOIN QT_QUESTION B ON A.IIQ_QGTN_CD = B.QQN_QSTN_CD AND B.QQN_EXAM_DT = :EXAM_DT AND B.QQN_EXAM_SQ = :EXAM_SQ
 ...
 ORDER BY A.IIQ_SORT_NO
		*/
		...
		sql += ", CASE WHEN C.CQN_ANSR_CD IS NULL OR C.CQN_ANSR_CD = '' THEN C.CQN_ANSR_CD ELSE C.CQN_ANSR_CD || '-' || GETCOMMON_LVCDTONM('0415',C.CQN_ANSR_CD,'5',1) END CQN_ANSR_NM";
		sql += ", C.CQN_SCOR_YN, C.CQN_SCOR_CNT, C.CQN_SCOR_NO, C.CQN_SCOR_SCR, B.QQN_SCORE_YN, B.QQN_SCORE";
		...
		sql += " ORDER BY A.IIQ_SORT_NO";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStdt_Pnjn_UP_QuestDisplay_001 \n";
			G_INFO += "설명 : 학생검진 문진 조회 \n";
			G_INFO += "\n\n";
