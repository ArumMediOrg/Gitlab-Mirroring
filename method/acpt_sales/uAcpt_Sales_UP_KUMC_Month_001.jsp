...
<%
		...
		sql = " SELECT ";
		sql += " 	SUM(EXAM_CNT1) EXAM_CNT1, SUM(EXAM_CNT2) EXAM_CNT2, SUM(EXAM_CNT3) EXAM_CNT3, ";
		...
		sql += " FROM (SELECT COUNT(*) EXAM_CNT1, SUM(EEA_PSNLR_PR+EEA_COMPR_PR+EEA_HLTHR_PR+EEA_SPCLR_PR) EXAM_AMT1, ";
		sql += " 			0        EXAM_CNT2, 0 EXAM_AMT2, ";
		sql += " 			0        EXAM_CNT3, 0 EXAM_AMT3, ";
		sql += " 			SUM(NVL(EEA_DISC_PR, 0)+NVL(EEA_DISC_CPR,0)) DISC_AMT1, ";
		sql += " 			0 DISC_AMT2, ";
		sql += " 			0 DISC_AMT3, ";
		sql += " 			SUM(EEA_PSNLR_PR+EEA_COMPR_PR+EEA_HLTHR_PR+EEA_SPCLR_PR-NVL(EEA_DISC_PR, 0)-NVL(EEA_DISC_CPR,0)) EXAM_DMT1, ";
		sql += " 			0 EXAM_DMT2, ";
		sql += " 			0 EXAM_DMT3 ";
		...
		sql += " 			COUNT(*) EXAM_CNT2, SUM(EEA_PSNLR_PR+EEA_COMPR_PR+EEA_HLTHR_PR+EEA_SPCLR_PR) EXAM_AMT2, ";
		sql += " 			0        EXAM_CNT3, 0 EXAM_AMT3, ";
		sql += " 			0 DISC_AMT1, ";
		sql += " 			SUM(NVL(EEA_DISC_PR, 0)+NVL(EEA_DISC_CPR,0)) DISC_AMT2, ";
		sql += " 			0 DISC_AMT3, ";
		sql += " 			0 EXAM_DMT1, ";
		sql += " 			SUM(EEA_PSNLR_PR+EEA_COMPR_PR+EEA_HLTHR_PR+EEA_SPCLR_PR-NVL(EEA_DISC_PR, 0)-NVL(EEA_DISC_CPR,0)) EXAM_DMT2, ";
		sql += " 			0 EXAM_DMT3 ";
		sql += " 		FROM ET_EXAM_ACPT ";
		...
		sql += " 		SELECT 0        EXAM_CNT1, 0 EXAM_AMT1, ";
		sql += " 			0        EXAM_CNT2, 0 EXAM_AMT2, ";
		sql += " 			COUNT(*) EXAM_CNT3, SUM(EEA_PSNLR_PR+EEA_COMPR_PR+EEA_HLTHR_PR+EEA_SPCLR_PR) EXAM_AMT3, ";
		sql += " 			0 DISC_AMT1, ";
		sql += " 			0 DISC_AMT2, ";
		sql += " 			SUM(NVL(EEA_DISC_PR, 0)+NVL(EEA_DISC_CPR,0)) DISC_AMT3, ";
		sql += " 			0 EXAM_DMT1, ";
		sql += " 			0 EXAM_DMT2, ";
		sql += " 			SUM(EEA_PSNLR_PR+EEA_COMPR_PR+EEA_HLTHR_PR+EEA_SPCLR_PR-NVL(EEA_DISC_PR, 0)-NVL(EEA_DISC_CPR,0)) EXAM_DMT3 ";
		sql += " 		FROM ET_EXAM_ACPT ";
		sql += " 		WHERE EEA_EXAM_DT BETWEEN '" + EXAM_SDT3 + "' AND '" + EXAM_EDT3 + "' ";
		sql += " 		AND EEA_ORDER_YN <> 'C' ";
		...
		sql += " 	)";
			...
			//
		G_INFO += "<!-- \n";
		G_INFO += "서비스명 : uAcpt_Sales_UP_KUMC_Month_001 \n";
		G_INFO += "설명 : 월말보고서(1. 수검현황, 2. 발생수가현황) \n";
		G_INFO += "\n\n";
