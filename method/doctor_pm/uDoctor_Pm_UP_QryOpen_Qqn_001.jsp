...
SELECT A.* FROM PT_FIR_PANJUNG A
if :cmbPlaceFlg = '1' then
begin
        Inner Join ET_EXAM_ACPT B On B.EEA_EXAM_DT = A.PFP_EXAM_DT
                And B.EEA_EXAM_SQ = A.PFP_EXAM_SQ
                And B.EEA_ORDER_YN <> 'C'
                And SUBSTR(B.EEA_PLCE_CD, 1, 2)= :cmbPlace
end;
WHERE 0=0
        AND A.PFP_EXAM_DT >= :DTEdtEXAM_SDT
        AND A.PFP_EXAM_DT <= :DTEdtEXAM_EDT
        AND NVL(A.PFP_DOCTORFIR, ' ') <> ' '
		...
		*/
		...
		if(CMBPLACEFLG.equals("1")) {
			sql += " BEGIN";
			sql += " INNER JOIN ET_EXAM_ACPT B ON B.EEA_EXAM_DT = A.PFP_EXAM_DT  ";
		...
		sql += " WHERE 0=0 ";
		sql += " AND A.PFP_EXAM_DT >= '" + DTEDTEXAM_SDT + "'";
		sql += " AND A.PFP_EXAM_DT <= '" + DTEDTEXAM_EDT + "'";
		sql += " AND ( NVL(A.PFP_DOCTORFIR, ' ') <> ' ' ";
		sql += " OR NVL(A.PFP_DOCTORQQN, ' ') <> ' ' )";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uDoctor_Pm_UP_QryOpen_Qqn_001 \n";
			G_INFO += "설명 : 일반검진 1차 판정정보(문진의사) 로딩 \n";
			G_INFO += "\n\n";
