...
SELECT LTRIM(RTRIM(IIM_ITEM_CD)) ITEM_CD 
  FROM IT_ITEM 
 WHERE IIM_SBCD_CD IN (:SBCD_CD) 
   AND UPPER(IIM_USE_YN) = 'Y'
   AND UPPER(IIM_TXRAY_YN) = 'Y'
		*/

		sql = " SELECT LTRIM(RTRIM(IIM_ITEM_CD)) ITEM_CD";
		sql += " FROM IT_ITEM";
		sql += " WHERE IIM_SBCD_CD IN ('" + SBCD_CD + "')";
		sql += " AND UPPER(IIM_USE_YN) = 'Y'";
		sql += " AND UPPER(IIM_TXRAY_YN) = 'Y'";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uFM_AcptComm_GP_ItemList_019 \n";
			G_INFO += "설명 : 특검 산보코드로 검사항목 찾기 \n";
			G_INFO += "\n\n";
