...
Select                                                                    
B.IIP_APLY_DT, A.IIM_ITEM_CD, A.IIM_SBCD_CD, A.IIM_KNME_NM, B.IIP_SBSG_PR,
B.IIP_TGSG_PR                                                             
From IT_ITEM A                                                            
Inner Join IT_ITEM_PRICE B On B.IIP_ITEM_CD = A.IIM_ITEM_CD               
                         And B.IIP_USE_YN = 'Y'                        
 Where A.IIM_SBCD_CD = '0542'                                           
  And A.IIM_USE_YN = 'Y'                                                
Order By B.IIP_APLY_DT Desc                                               

		*/

		sql = " SELECT B.IIP_APLY_DT, A.IIM_ITEM_CD, A.IIM_SBCD_CD, A.IIM_KNME_NM, B.IIP_SBSG_PR, B.IIP_TGSG_PR";
		sql += " FROM IT_ITEM A INNER JOIN IT_ITEM_PRICE B";
		sql += " ON B.IIP_ITEM_CD = A.IIM_ITEM_CD";
		sql += " AND B.IIP_USE_YN = 'Y'";
		sql += " WHERE A.IIM_SBCD_CD = '0542'";
		sql += " AND A.IIM_USE_YN = 'Y'";
		sql += " ORDER BY B.IIP_APLY_DT DESC";

			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uSP_ESTIMATE_RzBmpButton2Click_004 \n";
			G_INFO += "설명 : 행정관리비 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='IIP_SBSG_PR' rs:number='5' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM_PRICE'
			 rs:basecolumn='IIP_SBSG_PR'>
			<s:datatype dt:type='number' rs:dbtype='numeric' dt:maxLength='19' rs:scale='0' rs:precision='38' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='IIP_TGSG_PR' rs:number='6' rs:nullable='true' rs:writeunknown='true' rs:basetable='IT_ITEM_PRICE'
