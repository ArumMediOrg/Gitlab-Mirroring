...
<%
		...
		String ADD_SQL1 = htMethod.get("ADD_SQL1");

		//
		if(REQUEST_DATE == null) { REQUEST_DATE = ""; }
		if(EXAM_NAME == null) { EXAM_NAME = ""; }
		if(ADD_SQL1 == null) { ADD_SQL1 = ""; }
   ...
   FROM ARUM_TOTAL 
     WHERE REQUEST_DATE = :REQUEST_DATE
		...
		*/
		...
		sql += " AND EXAM_NAME = '" + EXAM_NAME + "'";
		sql += ADD_SQL1;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : MunJin_Viewselect_005 \n";
			G_INFO += "설명 : 각 날짜별로 해당 이미지 파일 이름, 이미지 데이터 가져오기 \n";
			G_INFO += "\n\n";

			G_INFO += "전달인자 : \n";
			G_INFO += " REQUEST_DATE : " + REQUEST_DATE + " \n";
			G_INFO += " EXAM_NAME : " + EXAM_NAME + " \n";
			G_INFO += " ADD_SQL1 : " + ADD_SQL1 + " \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='IMAGE_DATA' rs:number='25' rs:nullable='true' rs:writeunknown='true' rs:basetable='ARUM_TOTAL'
			 rs:basecolumn='IMAGE_DATA'>
			<s:datatype dt:type='bin.hex' dt:maxLength='2147483647' rs:long='true'/>
		</s:AttributeType>
		<s:AttributeType name='ROWID' rs:number='26' rs:rowid='true' rs:writeunknown='true' rs:basetable='ARUM_TOTAL'
...
<%
		...
		while(cRsList.next()) {
			...
			String IMAGE_DATA_T = "";
			byte[] buf_IMAGE_DATA = rsList.getBytes("IMAGE_DATA");
			if(buf_IMAGE_DATA != null) {
				IMAGE_DATA_T = new String(buf_IMAGE_DATA);
			}
