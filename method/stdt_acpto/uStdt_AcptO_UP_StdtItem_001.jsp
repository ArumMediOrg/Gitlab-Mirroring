...
SELECT SUBSTR(:EXAM_DT, 1, 4) EXAM_YR, LTRIM(RTRIM(A.IIM_ITEM_CD)) EXAM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) EXAM_NM, 
       SUBSTR(C.IIC_STDTO_KD, 1, 1) || '0000' AS EXAM_KD, 
 ...
 IF (:STDT_KD = '0') THEN
   AND SUBSTR(C.IIC_STDTO_KD, 1, 1) NOT IN ('0', '6', '9')
 ELSE
   AND SUBSTR(C.IIC_STDTO_KD, 1, 1) = :STDT_KD
 ...
 ORDER BY A.IIM_EXGN_CD, A.IIM_SORT_SQ, A.IIM_ITEM_CD
		*/
		...
		sql = " SELECT SUBSTR('" + EXAM_DT + "', 1, 4) EXAM_YR, LTRIM(RTRIM(A.IIM_ITEM_CD)) EXAM_CD, LTRIM(RTRIM(A.IIM_KNME_NM)) EXAM_NM, SUBSTR(C.IIC_STDTO_KD, 1, 1) || '0000' AS EXAM_KD, '" + EXAM_DT + "' PRCE_DT, '1' CALC_CD, B.IIP_GBSG_PR STDT_PR, B.IIP_GASN_PR GASN_PR";
		...
		sql += " WHERE C.IIC_EXAM_YR = SUBSTR('" + EXAM_DT + "', 1, 4)";
 		...
 		if(STDT_KD.equals("0")) {
 			sql += " AND SUBSTR(C.IIC_STDTO_KD, 1, 1) NOT IN ('0', '6', '9')";
		}
 		else {
			sql += " AND SUBSTR(C.IIC_STDTO_KD, 1, 1) = '" + STDT_KD + "'";
 		}
		...
		sql += " ORDER BY A.IIM_EXGN_CD, A.IIM_SORT_SQ, A.IIM_ITEM_CD";
			...
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uStdt_AcptO_UP_StdtItem_001 \n";
			G_INFO += "설명 : 학교밖 일반검진 검사항목 조회(일반, 암, 선택검사) \n";
			G_INFO += "\n\n";
