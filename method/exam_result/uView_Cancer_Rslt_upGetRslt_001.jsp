...
<%
		...
		String HOSP_SQ = htMethod.get("HOSP_SQ");

		//
		if(EXAM_DT == null) { EXAM_DT = ""; }
		if(EXAM_SQ == null) { EXAM_SQ = ""; }
		if(HOSP_SQ == null) { HOSP_SQ = ""; }

		// DB객체
		stmtList = connect.createStatement();


		sql = " SELECT ERI_ITEM_CD, IIM_KNME_NM, ERI_PACS_RSLT, CASE WHEN RTRIM(NVL(ERI_RSLT_EX,' ')) <> ' ' THEN ERI_RSLT_EX ELSE ERI_RSLT_VL END ERI_RSLT";
		...
		sql += " AND ERI_ITEM_CD IN (";
		sql += "	SELECT IIM_ITEM_CD";
		sql += " 	FROM IT_ITEM";
		sql += " 	WHERE IIM_CANCER_CD = '" + CANCER_CD + "'";
		sql += ")";

		if(HOSP_SQ.equals("447")){
			sql += " AND (NVL(ERI_HLTH_KD, '0') <> '0' OR (NVL(ERI_ADD_KD, '0') <> '0' AND IIM_CNIT_CD = '104')) ";
		}

		sql += " AND NVL(ERI_CNCL_YN, 'N') <> 'Y' ";
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uView_Cancer_Rslt_upGetRslt_001 \n";
			G_INFO += "설명 : 검진결과조회(대상암검사항목) \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		<s:AttributeType name='ERI_ITEM_CD' rs:number='1' rs:basetable='ET_RSLT_ITEM' rs:basecolumn='ERI_ITEM_CD'>
		...
		<s:AttributeType name='ERI_PACS_RSLT' rs:number='3' rs:nullable='true' rs:basetable='ET_RSLT_ITEM' rs:basecolumn='ERI_PACS_RSLT'>
