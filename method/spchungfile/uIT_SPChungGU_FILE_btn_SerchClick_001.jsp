...
<%
		...
		sql = " SELECT EEA_PSNL_NM, EEA_EXAM_DT, EEA_EXAM_SQ, EEA_DEPT_NM, SBE_EXAM_CHA,SBE_TPAN_NM, SBE_PANJ_YN, SBE_TOBO_DT, (SELECT MIN(NVL(SBE_PANJ_YN,' '))";
		sql += " FROM ST_ITEM_RECHECK SR, ST_BASE SB";
		sql += " WHERE SR.SIR_EXAM_DT = A.EEA_EXAM_DT";
		sql += " AND SR.SIR_EXAM_SQ = A.EEA_EXAM_SQ";
		sql += " AND SR.SIR_EXAM_DT2 = SB.SBE_EXAM_DT";
		sql += " AND SR.SIR_EXAM_SQ2 = SB.SBE_EXAM_SQ ) SBE_PANJ_YN2, (SELECT MAX(SIR_EXAM_DT2) SIR_EXAM_DT2";
		sql += " FROM ST_ITEM_RECHECK";
		sql += " WHERE SIR_EXAM_DT = A.EEA_EXAM_DT";
		sql += " AND SIR_EXAM_SQ = A.EEA_EXAM_SQ) SIR_EXAM_DT2";
		sql += " , EEA_EXAM_CD, EEA_HTSB_CD, EEA_ORAL_YN, EEA_EMSB_CD, EEA_ETSB_CD, EEA_CVSB_CD ";
      	sql += " , EEA_STOMA_CD, EEA_COLON_CD, EEA_LIVER_CD, EEA_BAST_CD, EEA_UTER_CD, EEA_LUNG_CD ";
		sql += " FROM ET_EXAM_ACPT A LEFT OUTER JOIN ST_BASE D";
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : uIT_SPChungGU_FILE_btn_SerchClick_001 \n";
			G_INFO += "설명 : 산보청구대상 내역 조회  메인 \n";
			G_INFO += "\n\n";
