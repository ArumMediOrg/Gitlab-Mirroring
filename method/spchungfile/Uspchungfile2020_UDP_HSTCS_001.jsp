...
SELECT EEA_EXAM_DT,EEA_EXAM_SQ,EEA_PSNL_NM, SSM_EXAM_DT, SSM_SYMP_SQ,'' SSM_SYSQ_CD,
       SSM_SYSQ_NM, SSM_ETC_TX HSTCS_REMARK
  FROM ET_EXAM_ACPT A LEFT OUTER JOIN ST_SYMPTOM B   ON B.SSM_EXAM_DT=A.EEA_EXAM_DT AND B.SSM_EXAM_SQ=A.EEA_EXAM_SQ AND SSM_EXAM_CHA = '1'  AND  SSM_SYMP_CD='1'
 ...
 WHERE 0=0
   ...
   ||:sSQL_ADD
		*/
		...
		sql = " SELECT EEA_EXAM_DT,EEA_EXAM_SQ,EEA_PSNL_NM, SSM_EXAM_DT, SSM_SYMP_SQ,'' SSM_SYSQ_CD, SSM_SYSQ_NM, SSM_ETC_TX HSTCS_REMARK";
		sql += " FROM ET_EXAM_ACPT A LEFT OUTER JOIN ST_SYMPTOM B";
		sql += " ON B.SSM_EXAM_DT=A.EEA_EXAM_DT";
		sql += " AND B.SSM_EXAM_SQ=A.EEA_EXAM_SQ";
		sql += " AND SSM_EXAM_CHA = '1'";
		sql += " AND SSM_SYMP_CD='1' LEFT OUTER JOIN ST_BASE D";
		...
		sql += SSQL_ADD;
			//
			G_INFO += "<!-- \n";
			G_INFO += "서비스명 : Uspchungfile2020_UDP_HSTCS_001 \n";
			G_INFO += "설명 : 특검청구 과거병력 조회 \n";
			G_INFO += "\n\n";
...
<s:Schema id='RowsetSchema'>
	<s:ElementType name='row' content='eltOnly' rs:updatable='true'>
		...
		<s:AttributeType name='SSM_SYMP_SQ' rs:number='5' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_SYMPTOM'
			 rs:basecolumn='SSM_SYMP_SQ'>
			<s:datatype dt:type='number' rs:dbtype='varnumeric' dt:maxLength='38' rs:scale='255' rs:precision='38'/>
		</s:AttributeType>
		<s:AttributeType name='SSM_SYSQ_CD' rs:number='6' rs:nullable='true'>
		...
		<s:AttributeType name='HSTCS_REMARK' rs:number='8' rs:nullable='true' rs:writeunknown='true' rs:basetable='ST_SYMPTOM'
		...
		<s:AttributeType name='c9' rs:name='ROWID' rs:number='10' rs:rowid='true' rs:basetable='ST_SYMPTOM' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c10' rs:name='ROWID' rs:number='11' rs:rowid='true' rs:basetable='ST_BASE' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c11' rs:name='ROWID' rs:number='12' rs:rowid='true' rs:basetable='ST_BILLHN_PR' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
		<s:AttributeType name='c12' rs:name='ROWID' rs:number='13' rs:rowid='true' rs:basetable='IT_COMPANY' rs:basecolumn='ROWID'
			 rs:keycolumn='true' rs:hidden='true' rs:autoincrement='true'>
			<s:datatype dt:type='string' rs:dbtype='str' dt:maxLength='18' rs:fixedlength='true'/>
		</s:AttributeType>
...
<%
		...
		while(cRsList.next()) {
			...
			String HSTCS_REMARK_T = cRsList.getString("HSTCS_REMARK");
...
<%
			...
			if(! HSTCS_REMARK_T.equals("")) {
%>
		 		HSTCS_REMARK='<%= HSTCS_REMARK_T%>'
<%
			}
%>
				ROWID='<%= cnt%>'
